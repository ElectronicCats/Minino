 ...
 *         The transmit result will be reported via
 * `esp_ieee802154_transmit_done()` or `esp_ieee802154_transmit_failed()`.
 *
 * @param[in]  frame  The pointer to the frame, the frame format:
 *                    |-----------------------------------------------------------------------|
 *                    | Len | MHR |              MAC Payload |  FCS  |
 *                    |-----------------------------------------------------------------------|
 * @param[in]  cca    Perform CCA before transmission if it's true, otherwise
 ...
 *
 * @param[in]  timeout  The time to wait for the ack frame, in symbol unit (16
 * us). Default: 0x006C, Range: 0x0000 - 0xFFFF.
 *
 * @return
 *      - ESP_OK on success.
 *      - ESP_FAIL on failure.
 ...
 *
 */
uint16_t esp_ieee802154_get_multipan_panid(
 ...
 *      - ESP_FAIL on failure.
 */
esp_err_t esp_ieee802154_set_multipan_panid(
    esp_ieee802154_multipan_index_t index,
    uint16_t panid);
 ...
 *
 */
uint16_t esp_ieee802154_get_multipan_short_address(
    esp_ieee802154_multipan_index_t index);
 ...
 *      - ESP_FAIL on failure.
 */
esp_err_t esp_ieee802154_set_multipan_short_address(
    esp_ieee802154_multipan_index_t index,
 ...
 *      - ESP_FAIL on failure.
 */
esp_err_t esp_ieee802154_get_multipan_extended_address(
    esp_ieee802154_multipan_index_t index,
    uint8_t* ext_addr);
 ...
 *      - ESP_FAIL on failure.
 */
esp_err_t esp_ieee802154_set_multipan_extended_address(
    esp_ieee802154_multipan_index_t index,
 ...
 *
 * As an example, call
 * `esp_ieee802154_set_multipan_enable(BIT(ESP_IEEE802154_MULTIPAN_0) |
 * BIT(ESP_IEEE802154_MULTIPAN_1));` to enable multipan interface 0 and 1.
 *
 ...
 *
 * @param[in]  pending_mode  The auto frame pending mode, refer to
 ...
 *      - ESP_FAIL on failure.
 */
esp_err_t esp_ieee802154_set_pending_mode(
 ...
 * @return
 *      - ESP_OK on success.
 *      - ESP_ERR_NOT_FOUND if the address was not found from the source
 * matching table.
 *
 */
 ...
 *
 * @param[in]  duration  The duration of energy detection, in symbol unit (16
 * us). The result will be reported via esp_ieee802154_energy_detect_done().
 *
 * @return
 *      - ESP_OK on success.
 *      - ESP_FAIL on failure due to invalid state.
...
/**
 * @brief  Notify the IEEE 802.15.4 Radio that the frame is handled done by
 * upper layer.
 *
 * @param[in]  frame  The pointer to the frame which was passed from the
 * function `esp_ieee802154_receive_done()` or ack frame from
...
/** Below are the events generated by IEEE 802.15.4 subsystem, which are in ISR
 * context **/
/**
 * @brief  A Frame was received.
 *
 * @note   User must call the function `esp_ieee802154_receive_handle_done()` to
 * notify 802.15.4 driver after the received frame is handled.
 *
 * @param[in]  frame  The point to the received frame, frame format:
 *                    |-----------------------------------------------------------------------|
 *                    | Len | MHR |              MAC Payload (no FCS)   |
 *                    |-----------------------------------------------------------------------|
 * @param[in]  frame_info  More information of the received frame, refer to
 ...
 *
 * @note   If the ack frame is not null, user must call the function
 * `esp_ieee802154_receive_handle_done()` to notify 802.15.4 driver after the
 * ack frame is handled.
 *
 * @param[in]  frame           The pointer to the transmitted frame.
 * @param[in]  ack             The received ACK frame, it could be NULL if the
 * transmitted frame's AR bit is not set.
 * @param[in]  ack_frame_info  More information of the ACK frame, refer to
 * esp_ieee802154_frame_info_t.
 *
 */
extern void esp_ieee802154_transmit_done(
    const uint8_t* frame,
    const uint8_t* ack,
 ...
 * @param[in]  error  The transmission failure reason, refer to
 ...
 *
 * @note   Radio will start receiving after the timestamp, and continue
 * receiving until it receives a valid frame. Refer to
 ...
 * `esp_ieee802154_transmit_done()` or `esp_ieee802154_transmit_failed()`.
 *
 * @param[in]  frame  The pointer to the frame. Refer to
 ...
 *
 */
esp_err_t esp_ieee802154_transmit_at(const uint8_t* frame,
                                     bool cca,
                                     uint32_t time);
 ...
 *
 * @param[in]  frame  A frame needs to be encrypted. Refer to
 * `esp_ieee802154_transmit()`.
 * @param[in]  key    A 16-bytes key for encryption.
 * @param[in]  addr   An 8-bytes addr for HW to generate nonce, in general, is
 * the device extended address.
 *
 * @return
 *      - ESP_OK on success.
 *      - ESP_FAIL on failure.
 */
esp_err_t esp_ieee802154_set_transmit_security(uint8_t* frame,
                                               uint8_t* key,
                                               uint8_t* addr);

/**
 * @brief  This function will be called when a received frame needs to be acked
 * with Enh-Ack, the upper layer should generate the Enh-Ack frame in this
 * callback function.
 *
 * @param[in]  frame          The received frame.
 * @param[in]  frame_info     The frame information. Refer to
 * `esp_ieee802154_frame_info_t`.
 * @param[out] enhack_frame   The Enh-ack frame need to be generated via this
 * function, HW will send it back after AIFS.
 *
 * @return
 ...
 *
 */
esp_err_t esp_ieee802154_enh_ack_generator(
    uint8_t* frame,
    esp_ieee802154_frame_info_t* frame_info,
