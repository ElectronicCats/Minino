...
static int iperf_show_socket_error_reason(const char* str, int sockfd) {
  int err = errno;
  if (err != 0) {
    ESP_LOGW(TAG, "%s error, error code: %d, reason: %s", str, err,
             strerror(err));
  }
...
static void iperf_report_task(void* arg) {
  ...
  while (!s_iperf_ctrl.finish) {
    ...
    actual_bandwidth =
        (s_iperf_ctrl.actual_len / coefficient[format] * 8) / interval;
    printf("%4" PRIi32 "-%4" PRIi32 " sec       %.2f %cbits/sec\n", cur,
           cur + interval, actual_bandwidth, unit[format]);
    cur += interval;
...
static esp_err_t iperf_start_report(void) {
  ...
  ret = xTaskCreatePinnedToCore(
      iperf_report_task, IPERF_REPORT_TASK_NAME, IPERF_REPORT_TASK_STACK, NULL,
      IPERF_REPORT_TASK_PRIORITY, NULL, portNUM_PROCESSORS - 1);
...
static void IRAM_ATTR socket_recv(int recv_socket,
  ...
  const char* error_log =
      (type == IPERF_TRANS_TYPE_TCP) ? "tcp server recv" : "udp server recv";
...
static void IRAM_ATTR socket_send(int send_socket,
...
#endif
  const char* error_log =
      (type == IPERF_TRANS_TYPE_TCP) ? "tcp client send" : "udp client send";
  ...
  while (!s_iperf_ctrl.finish) {
    if (period_us > 0) {
      ...
      if (actual_send > 0) {
        // Last packet "send" was successful, check how much off the previous
        // loop duration was to the ideal send period. Result will adjust the
        // next send delay.
        delay_us += period_us + (int32_t) (prev_time - send_time);
      } else {
        // Last packet "send" was not successful. Ideally we should try to catch
        // up the whole previous loop duration (e.g. prev_time - send_time).
        // However, that's not possible since the most probable reason why the
        // send was unsuccessful is the HW was not able to process the packet.
        // Hence, we cannot queue more packets with shorter (or no) delay to
        // catch up since we are already at the performance edge. The best we
        // can do is to reset the send delay (which is probably big negative
        // number) and start all over again.
        delay_us = 0;
      }
      prev_time = send_time;
    }
    *pkt_id_p = htonl(pkt_cnt++);  // datagrams need to be sequentially numbered
    ...
    // The send delay may be negative, it indicates we are trying to catch up
    // and hence to not delay the loop at all.
    if (delay_us > 0) {
...
static esp_err_t iperf_run_tcp_server(void) {
  ...
  ESP_GOTO_ON_FALSE((s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV6 ||
                     s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV4),
                    ESP_FAIL, exit, TAG, "Ivalid AF types");
#else
  ESP_GOTO_ON_FALSE((s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV4), ESP_FAIL,
                    exit, TAG, "Invalid AF types");
#endif
#ifdef CONFIG_LWIP_IPV6
  if (s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV6) {
    // The TCP server listen at the address "::", which means all addresses can
    // be listened to.
    ...
    ESP_GOTO_ON_FALSE((listen_socket >= 0), ESP_FAIL, exit, TAG,
    ...
    err = bind(listen_socket, (struct sockaddr*) &listen_addr6,
               sizeof(listen_addr6));
    ESP_GOTO_ON_FALSE((err == 0), ESP_FAIL, exit, TAG,
                      "Socket unable to bind: errno %d, IPPROTO: %d", errno,
                      AF_INET6);
  ...
  client_socket =
      accept(listen_socket, (struct sockaddr*) &remote_addr, &addr_len);
  ESP_GOTO_ON_FALSE((client_socket >= 0), ESP_FAIL, exit, TAG,
                    "Unable to accept connection: errno %d", errno);
  ESP_LOGI(TAG, "accept: %s,%d", inet_ntoa(remote_addr.sin_addr),
...
static esp_err_t iperf_run_tcp_client(void) {
  ...
  if (s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV6) {
    ...
    err = connect(client_socket, (struct sockaddr*) &dest_addr6,
                  sizeof(struct sockaddr_in6));
    ESP_GOTO_ON_FALSE((err == 0), ESP_FAIL, exit, TAG,
...
#else
  ESP_GOTO_ON_FALSE((s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV4), ESP_FAIL,
                    exit, TAG, "Ivalid AF types");
#endif
#ifdef CONFIG_LWIP_IPV6
  if (s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV6) {
    // The UDP server listen at the address "::", which means all addresses can
    // be listened to.
    inet6_aton("::", &listen_addr6.sin6_addr);
    listen_addr6.sin6_family = AF_INET6;
    listen_addr6.sin6_port = htons(s_iperf_ctrl.cfg.sport);
...
static esp_err_t iperf_run_udp_client(void) {
  ...
  if (s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV6) {
    ...
    ESP_GOTO_ON_FALSE((client_socket >= 0), ESP_FAIL, exit, TAG,
                      "Unable to create socket: errno %d", errno);
    ESP_LOGI(TAG, "Socket created, sending to %s:%" PRIu16,
...
#else
    return (s_iperf_ctrl.cfg.len_send_buf == 0 ? IPERF_DEFAULT_IPV4_UDP_TX_LEN
                                               : s_iperf_ctrl.cfg.len_send_buf);
#endif
  } else if (iperf_is_udp_server()) {
    return IPERF_DEFAULT_UDP_RX_LEN;
  } else if (iperf_is_tcp_client()) {
    return (s_iperf_ctrl.cfg.len_send_buf == 0 ? IPERF_DEFAULT_TCP_TX_LEN
                                               : s_iperf_ctrl.cfg.len_send_buf);
  } else {
...
esp_err_t iperf_start(iperf_cfg_t* cfg) {
  ...
  ret = xTaskCreatePinnedToCore(
      iperf_task_traffic, IPERF_TRAFFIC_TASK_NAME, IPERF_TRAFFIC_TASK_STACK,
      NULL, IPERF_TRAFFIC_TASK_PRIORITY, NULL, portNUM_PROCESSORS - 1);
  if (ret != pdPASS) {
