...
#include "ping/ping_sock.h"
   ...
   */
  #define VAR2IPSTR(var)                                 \
    (uint16_t) var & 0xFF, (uint16_t) (var >> 8) & 0xFF, \
        (uint16_t) (var >> 16) & 0xFF, (uint16_t) (var >> 24) & 0xFF
...
static int wifi_cmd_set_omc(int argc, char** argv) {
  ...
  do {
    if (!omctrl_args.ul_mu_disable->count &&
        !omctrl_args.ul_mu_data_disable->count) {
      omc.ul_mu_disable = 1;
    ...
    if (omctrl_args.resounding->count) {
      omc.dl_mu_mimo_resounding_recommendation =
          omctrl_args.resounding->dval[0];
    }
  } while (0);
  ...
  if (omctrl_args.report->count && omctrl_args.report->dval[0] == 0) {
    ...
    ESP_LOGW(TAG, "(omc)(internal)disable ul mu(%d, data:%d) successfully",
             omc.ul_mu_disable, omc.ul_mu_data_disable);
  } else {
    err = esp_wifi_set_htc_omc(&omc);
    if (err != ESP_OK) {
      ESP_LOGW(TAG, "(omc)disable ul mu(%d, data:%d) failed, err:0x%x",
               omc.ul_mu_disable, omc.ul_mu_data_disable, err);
    } else {
      ESP_LOGW(TAG, "(omc)disable ul mu(%d, data:%d) successfully",
               omc.ul_mu_disable, omc.ul_mu_data_disable);
    }
  }
  return 0;
...
static int wifi_cmd_reg_rw(int argc, char** argv) {
  ...
  if (reg_rw_args.read->count) {
    ...
    ESP_LOGW(TAG, "reg write 0x%8lx : 0x%8lx", addr,
             (uint32_t) reg_rw_args.value->dval[0]);
...
static int wifi_cmd_set_tf_padding(int argc, char** argv) {
  ...
  if (tf_padding_args.tf_padding->count) {
    ...
    ESP_LOGW(TAG, "(test)set trigger frame mac padding duration:%d",
             (int) tf_padding_args.tf_padding->dval[0]);
  } else {
    printf("Input Error\n");
  }
  return 0;
...
static int wifi_cmd_get_ps_state(int argc, char** argv) {
  ESP_LOGW(TAG, "ps: awake:%d, sleep:%d, dream:%d", pm_is_waked(),
           pm_is_sleeping(), pm_is_dream());
  return 0;
}

esp_err_t esp_netif_set_static_ip(esp_netif_t* netif_sta,
                                  uint32_t ip,
                                  uint32_t gw,
                                  uint32_t netmask) {
  esp_netif_dhcpc_stop(netif_sta);
  esp_netif_ip_info_t ip_info;
  esp_netif_set_ip4_addr(&ip_info.ip, ip & 0xFF, (ip >> 8) & 0xFF,
                         (ip >> 16) & 0xFF, (ip >> 24) & 0xFF);
  esp_netif_set_ip4_addr(&ip_info.gw, gw & 0xFF, (gw >> 8) & 0xFF,
                         (gw >> 16) & 0xFF, (gw >> 24) & 0xFF);
  esp_netif_set_ip4_addr(&ip_info.netmask, netmask & 0xFF,
  ...
  if (proto_args.proto->count) {
    if (!strcmp(proto_args.proto->sval[0], "ax")) {
      ESP_ERROR_CHECK(esp_wifi_set_protocol(
          ifx, WIFI_PROTOCOL_11B | WIFI_PROTOCOL_11G | WIFI_PROTOCOL_11N |
                   WIFI_PROTOCOL_11AX));
      ...
      ESP_ERROR_CHECK(
          esp_wifi_set_protocol(ifx, WIFI_PROTOCOL_11B | WIFI_PROTOCOL_11G));
      printf("(%s)set to bg\n", (ifx == WIFI_IF_STA) ? "sta" : "ap");
    } else {
...
static int wifi_cmd_inactive_time(int argc, char** argv) {
  ...
  if ((mode & WIFI_MODE_AP) && inactive_time_args.val->count) {
    err = esp_wifi_set_inactive_time(ESP_IF_WIFI_AP,
  ...
  if ((mode & WIFI_MODE_STA) && inactive_time_args.val->count) {
    err = esp_wifi_set_inactive_time(ESP_IF_WIFI_STA,
                                     inactive_time_args.val->ival[0]);
    if (err != ESP_OK) {
...
static int wifi_cmd_sounding_rate(int argc, char** argv) {
  ...
  if (wifi_sounding_rate_args.format->count &&
      wifi_sounding_rate_args.rate->count) {
    if (wifi_sounding_rate_args.format->ival[0] == SIG_MODE_LEGACY) {
      if (wifi_sounding_rate_args.rate->ival[0] < WIFI_PHY_RATE_MCS0_LGI &&
        ...
        hal_he_set_bf_report_rate(SIG_MODE_LEGACY,
      ...
      if (wifi_sounding_rate_args.rate->ival[0] >= WIFI_PHY_RATE_MCS0_LGI &&
        ...
        ESP_LOGW(TAG, "need correct mcs(%d-%d)", WIFI_PHY_RATE_MCS0_LGI,
  ...
  if (tx_pwr_args.mcs->count && tx_pwr_args.power->count) {
    if (tx_pwr_args.mcs->ival[0] <= 9 && tx_pwr_args.mcs->ival[0] >= 0) {
      if (tx_pwr_args.power->ival[0] >= -13 &&
        ...
        esp_test_set_tx_mcs_pwr(
            tx_pwr_args.mcs->ival[0] + WIFI_PHY_RATE_MCS0_LGI,
            tx_pwr_args.power->ival[0]);
        ESP_LOGW(TAG, "set MCS%d TX PWR to %d", tx_pwr_args.mcs->ival[0],
                 tx_pwr_args.power->ival[0]);
      } else if (tx_pwr_args.power->ival[0] == 0xff) {
        esp_test_set_tx_mcs_pwr(
            tx_pwr_args.mcs->ival[0] + WIFI_PHY_RATE_MCS0_LGI,
            tx_pwr_args.power->ival[0]);
        ESP_LOGW(TAG, "set MCS%d TX PWR to default value",
                 tx_pwr_args.mcs->ival[0]);
      }
    } else {
      ESP_LOGW(TAG,
               "Set TX power fail, MCS should in range [0,9], power should in "
               "range [-13, 30] or set 0xFF for default");
    }
  }
  return 0;
}
...
void register_wifi_cmd(void) {
  ...
  reg_rw_args.read =
      arg_dbl0("r", NULL, "<read_addr>", "read register address");
  reg_rw_args.write =
      arg_dbl0("w", NULL, "<write_addr>", "write register address");
  ...
  omctrl_args.ul_mu_data_disable =
      arg_dbl0("d", "uldata", "[uldata]", "disable ul mu data");
  omctrl_args.ersu_disable = arg_dbl0("e", "ersu", "[ersu]", "disable ersu");
  omctrl_args.report =
      arg_dbl0("r", "report", "[report]", "report om control to ap");
  omctrl_args.resounding = arg_dbl0("s", "resounding", "[resounding]",
                                    "DL MU-MIMO resound Recoummendation");
  ...
  const esp_console_cmd_t stbc_cmd = {.command = "stbc",
                                      .help = "configure stbc",
                                      .hint = NULL,
                                      .func = &wifi_cmd_stbc,
                                      .argtable = &stbc_args};
  ...
  const esp_console_cmd_t bmfmee_cmd = {.command = "bmfmee",
  ...
  tf_padding_args.tf_padding = arg_dbl0(
      "p", "padding", "[padding]", "set trigger frame mac padding duration");
  ...
  ping_args.interval =
      arg_dbl0("i", "interval", "<t>",
               "Wait interval seconds between sending each packet");
  ping_args.data_size = arg_int0("s", "size", "<n>",
                                 "Specify the number of data bytes to be sent");
  ping_args.count =
      arg_int0("c", "count", "<n>", "Stop after sending count packets");
  ping_args.tos = arg_int0("Q", "tos", "<n>",
                           "Set Type of Service related bits in IP datagrams");
  ...
  static_ip_args.netmask =
      arg_str0("n", "netmask", "<netmask>", "netmask addess");
  ...
  const esp_console_cmd_t proto_cmd = {.command = "proto",
  ...
  /* inactive time */
  inactive_time_args.val =
      arg_int0("t", "time", "time", "set inactive time, in seconds");
  ...
  wifi_sounding_rate_args.format =
      arg_int0("f", "format", "format", "set format");
  ...
  /* muedca */
  muedca_args.reset_timer =
      arg_int0("r", NULL, "reset timer", "reset muedca timer");
  muedca_args.read_timer =
      arg_int0("d", NULL, "read timer", "read muedca timer");
  ...
  tx_pwr_args.power =
      arg_int0("p", NULL, "[-13, 20]", "set max power, set 0xFF for default");
