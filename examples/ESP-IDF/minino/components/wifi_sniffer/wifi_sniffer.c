...
void wifi_sniffer_init(void) {
  ...
  xTaskCreate(&wifi_sniffer_task, "wifi_sniffer_task", 2048, NULL, 5,
...
static void get_ssid(unsigned char* data,
...
static void get_ht_capabilites_info(unsigned char* data,
                                    char htci[5],
                                    int pkt_len,
                                    int ssid_len) {
...
static void wifi_sniffer_run(void* buff, wifi_promiscuous_pkt_type_t type) {
  ...
  if ((fc & 0xFF00) == 0x4000) {  // only look for probe request packets
    ...
    wifi_sniffer_record_t record = {
        ...
        .addr = {mgmt->sa[0], mgmt->sa[1], mgmt->sa[2], mgmt->sa[3],
                 mgmt->sa[4], mgmt->sa[5]},
        .hash = hash,
