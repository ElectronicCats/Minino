...
class UartSpinelInterface : public ot::Spinel::SpinelInterface {
 public:
  ...
  /**
   * Encodes and sends a spinel frame to Radio Co-processor (RCP) over the
   * socket.
   *
   ...
   * @retval OT_ERROR_NO_BUFS  Insufficient buffer space available to encode the
   * frame.
   ...
   *
   */
  void RegisterRcpFailureHandler(esp_radio_spinel_rcp_failure_handler handler) {
  ...
  void RegisterUartDeinitHandler(esp_radio_spinel_uart_deinit_handler handler) {
    mUartDeinitHandler = handler;
  }

 private:
  enum {
    /**
     * Maximum wait time in Milliseconds for socket to become writable (see
     * `SendFrame`).
     *
     */
    kMaxWaitTime = 2000,
