...
static esp_openthread_circular_queue_info_t s_recv_queue = {.head = 0,
                                                            .tail = 0,
                                                            .used = 0};
...
esp_err_t esp_openthread_radio_init(
    const esp_openthread_platform_config_t* config) {
  ESP_RETURN_ON_FALSE(s_radio_event_fd == -1, ESP_ERR_INVALID_STATE,
                      OT_PLAT_LOG_TAG, "Radio was initalized already!");
  ...
  return esp_openthread_platform_workflow_register(
  ...
  if (get_event(EVENT_TX_DONE)) {
...
#endif
    {
      if (s_ack_frame.mPsdu == NULL) {
        ...
        otPlatRadioTxDone(aInstance, &s_transmit_frame, &s_ack_frame,
                          OT_ERROR_NONE);
...
#if OPENTHREAD_CONFIG_DIAG_ENABLE
    if (otPlatDiagModeGet()) {
      otPlatDiagRadioTransmitDone(aInstance, &s_transmit_frame,
    ...
    if (s_receive_frame[s_recv_queue.head].mPsdu != NULL) {
      ...
      if (otPlatDiagModeGet()) {
        otPlatDiagRadioReceiveDone(
            aInstance, &s_receive_frame[s_recv_queue.head], OT_ERROR_NONE);
      } else
#endif
      {
        otPlatRadioReceiveDone(aInstance, &s_receive_frame[s_recv_queue.head],
                               OT_ERROR_NONE);
      }
      esp_ieee802154_receive_handle_done(
          s_receive_frame[s_recv_queue.head].mPsdu - 1);
      s_receive_frame[s_recv_queue.head].mPsdu = NULL;
      s_recv_queue.head =
          (s_recv_queue.head + 1) % CONFIG_IEEE802154_RX_BUFFER_SIZE;
...
otError otPlatRadioTransmit(otInstance* aInstance, otRadioFrame* aFrame) {
  ...
  aFrame->mPsdu[-1] = aFrame->mLength;  // lenth locates one byte before the
                                        // psdu (esp_openthread_radio_tx_psdu);
  ...
  if (otMacFrameIsSecurityEnabled(aFrame) &&
    ...
    memcpy(s_security_key, s_current_key.mKeyMaterial.mKey.m8,
           sizeof(s_current_key.mKeyMaterial.mKey.m8));
    esp_ieee802154_set_transmit_security(&aFrame->mPsdu[-1], s_security_key,
                                         s_security_addr);
  }

  if (aFrame->mInfo.mTxInfo.mTxDelay != 0) {
    esp_ieee802154_transmit_at(&aFrame->mPsdu[-1],
                               aFrame->mInfo.mTxInfo.mCsmaCaEnabled,
                               (aFrame->mInfo.mTxInfo.mTxDelayBaseTime +
                                aFrame->mInfo.mTxInfo.mTxDelay));
  } else {
    esp_ieee802154_transmit(&aFrame->mPsdu[-1],
                            aFrame->mInfo.mTxInfo.mCsmaCaEnabled);
  }
...
static IRAM_ATTR uint16_t get_csl_phase() {
  ...
  uint32_t diff = (csl_period_us - (cur_time % csl_period_us) +
                   (s_csl_sample_time % csl_period_us)) %
                  csl_period_us;
...
otError otPlatRadioConfigureEnhAckProbing(otInstance* aInstance,
                                          ...
                                          const otExtAddress* aExtAddress) {
  otError error = otLinkMetricsConfigureEnhAckProbing(
      aShortAddress, aExtAddress, aLinkMetrics);
  return error;
}
...
convert_to_ot_frame(uint8_t* data,
...
enh_ack_set_security_addr_and_key(otRadioFrame* ack_frame) {
  ...
  esp_ieee802154_set_transmit_security(&ack_frame->mPsdu[-1], s_security_key,
                                       s_security_addr);
  return ESP_OK;
}
...
esp_ieee802154_enh_ack_generator(uint8_t* frame,
  ...
  link_metrics_data_len = otLinkMetricsEnhAckGenData(
      &mac_addr, esp_ieee802154_get_recent_lqi(),
      esp_ieee802154_get_recent_rssi(), link_metrics_data);
  if (link_metrics_data_len > 0) {
    offset += otMacFrameGenerateEnhAckProbingIe(ack_ie_data, link_metrics_data,
                                                link_metrics_data_len);
  }
#endif
  err = otMacFrameGenerateEnhAck(&ot_frame, frame_info->pending, ack_ie_data,
                                 offset, &ack_frame);
  ...
  if (otMacFrameIsSecurityEnabled(&ack_frame) &&
...
void IRAM_ATTR
ot_esp_ieee802154_receive_done(uint8_t* data,
                               esp_ieee802154_frame_info_t* frame_info) {
  otRadioFrame ot_frame;
  ...
  if (otMacFrameIsAckRequested(&ot_frame) &&
      otMacFrameIsVersion2015(&ot_frame)) {
    s_receive_frame[s_recv_queue.tail].mInfo.mRxInfo.mAckedWithSecEnhAck =
        s_with_security_enh_ack;
    s_receive_frame[s_recv_queue.tail].mInfo.mRxInfo.mAckFrameCounter =
        s_ack_frame_counter;
...
#endif  // OPENTHREAD_CONFIG_THREAD_VERSION >= OT_THREAD_VERSION_1_2
  s_recv_queue.tail =
      (s_recv_queue.tail + 1) % CONFIG_IEEE802154_RX_BUFFER_SIZE;
  atomic_fetch_add(&s_recv_queue.used, 1);
  set_event(EVENT_RX_DONE);
}

void IRAM_ATTR
ot_esp_ieee802154_transmit_failed(const uint8_t* frame,
...
void IRAM_ATTR ot_esp_ieee802154_receive_sfd_done(void) {}

void IRAM_ATTR ot_esp_ieee802154_transmit_sfd_done(uint8_t* frame) {
  ...
  if (frame == (uint8_t*) &s_transmit_psdu &&
      s_transmit_frame.mInfo.mTxInfo.mIeInfo->mTimeIeOffset != 0) {
    uint8_t* p_time_ie = s_transmit_frame.mPsdu +
                         s_transmit_frame.mInfo.mTxInfo.mIeInfo->mTimeIeOffset;
    uint64_t time =
        (uint64_t) ((int64_t) otPlatTimeGet() +
                    s_transmit_frame.mInfo.mTxInfo.mIeInfo->mNetworkTimeOffset);
...
void IRAM_ATTR ot_esp_ieee802154_energy_detect_done(int8_t power) {
  s_ed_power = power;

  set_event(EVENT_ENERGY_DETECT_DONE);
}

void IRAM_ATTR ot_esp_ieee802154_cca_done(bool channel_free) {}
