...
static void handle_ot_netif_state_change(otInstance* instance) {
  if (otLinkIsEnabled(instance)) {
    ...
    if (esp_event_post(OPENTHREAD_EVENT, OPENTHREAD_EVENT_IF_UP, NULL, 0, 0) !=
        ESP_OK) {
      ESP_LOGE(TAG, "Failed to post OpenThread if up event");
    }
  } else {
    ESP_LOGI(TAG, "netif down");
    if (esp_event_post(OPENTHREAD_EVENT, OPENTHREAD_EVENT_IF_DOWN, NULL, 0,
                       0) != ESP_OK) {
...
static void handle_ot_netdata_change(void) {
  ...
  if (esp_openthread_get_nat64_prefix(&dns_server_addr.u_addr.ip6) == ESP_OK) {
    ...
    dns_server_addr.u_addr.ip6.addr[3] =
        ipaddr_addr(CONFIG_OPENTHREAD_DNS_SERVER_ADDR);
    ...
    if (memcmp(dnsserver, &dns_server_addr, sizeof(ip_addr_t)) != 0) {
      ESP_LOGI(TAG, "Set dns server address: %s",
               ipaddr_ntoa(&dns_server_addr));
      dns_setserver(OPENTHREAD_DNS_SERVER_INDEX, &dns_server_addr);
      if (esp_event_post(OPENTHREAD_EVENT, OPENTHREAD_EVENT_SET_DNS_SERVER,
                         NULL, 0, 0) != ESP_OK) {
...
static void handle_ot_role_change(otInstance* instance) {
  ...
  if (esp_event_post(OPENTHREAD_EVENT, OPENTHREAD_EVENT_ROLE_CHANGED,
                     &event_data, sizeof(event_data), 0) != ESP_OK) {
    ESP_LOGE(TAG, "Failed to post OPENTHREAD_EVENT_ROLE_CHANGED");
  }
  if (role == OT_DEVICE_ROLE_CHILD || role == OT_DEVICE_ROLE_ROUTER ||
      role == OT_DEVICE_ROLE_LEADER) {
    if (s_previous_role == OT_DEVICE_ROLE_DETACHED ||
        s_previous_role == OT_DEVICE_ROLE_DISABLED) {
      otOperationalDataset dataset;
      ESP_GOTO_ON_FALSE(otDatasetGetActive(instance, &dataset) == OT_ERROR_NONE,
                        ESP_FAIL, exit, TAG,
                        "Failed to get the active dataset");
      ESP_GOTO_ON_ERROR(
          esp_event_post(OPENTHREAD_EVENT, OPENTHREAD_EVENT_ATTACHED, &dataset,
                         sizeof(dataset), 0),
          exit, TAG, "Failed to post OPENTHREAD_EVENT_ATTACHED. Err: %s",
          esp_err_to_name(ret));
    }
  } else if (role == OT_DEVICE_ROLE_DETACHED) {
    if (s_previous_role != OT_DEVICE_ROLE_DISABLED) {
      ESP_GOTO_ON_ERROR(
          esp_event_post(OPENTHREAD_EVENT, OPENTHREAD_EVENT_DETACHED,
                         &s_previous_role, sizeof(s_previous_role), 0),
          exit, TAG, "Failed to post OPENTHREAD_EVENT_DETACHED. Err: %s",
          esp_err_to_name(ret));
    }
  }
exit:
...
esp_err_t esp_openthread_state_event_init(otInstance* instance) {
  ESP_RETURN_ON_FALSE(
      otSetStateChangedCallback(instance, ot_state_change_callback, NULL) ==
          OT_ERROR_NONE,
