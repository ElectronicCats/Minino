...
void otPlatAlarmMilliStartAt(otInstance* aInstance,
  ...
  ESP_LOGD(OT_PLAT_LOG_TAG,
           "Millisecond timer alarm start running, t0=%" PRIu32 ", dt=%" PRIu32
           "",
           aT0, aDt);
}
...
void otPlatAlarmMicroStartAt(otInstance* aInstance,
  ...
  ESP_LOGD(OT_PLAT_LOG_TAG,
           "Microsecond timer alarm start running, t0=%" PRIu32 ", dt=%" PRIu32
           "",
           aT0, aDt);
}
...
esp_err_t esp_openthread_alarm_init(void) {
  return esp_openthread_platform_workflow_register(
      &esp_openthread_alarm_update, &esp_openthread_alarm_process,
      alarm_workflow);
}
...
void esp_openthread_alarm_update(esp_openthread_mainloop_context_t* mainloop) {
  ...
  if (s_is_ms_running) {
    remaining_us =
        calculate_duration(s_alarm_ms, otPlatAlarmMilliGetNow()) * US_PER_MS;
...
esp_err_t esp_openthread_alarm_process(
