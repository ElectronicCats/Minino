...
static esp_err_t get_next_empty_index(uint16_t aKey, uint8_t* index) {
  ESP_RETURN_ON_FALSE((s_ot_nvs_handle != 0), ESP_ERR_INVALID_STATE,
                      OT_PLAT_LOG_TAG, "OT NVS handle is invalid.");
  ...
  for (uint8_t i = 0; i != UINT8_MAX; i++) {
    s_unused_pos++;
    found = false;
    snprintf(ot_nvs_key, sizeof(ot_nvs_key), OT_KEY_INDEX_PATTERN,
             (uint8_t) aKey, s_unused_pos);
...
static esp_err_t find_target_key_using_index(uint16_t aKey,
                                             int aIndex,
                                             char* key,
                                             size_t key_len) {
  ESP_RETURN_ON_FALSE((s_ot_nvs_handle != 0), ESP_ERR_INVALID_STATE,
...
static esp_err_t erase_all_key(uint16_t aKey) {
  ...
  ESP_RETURN_ON_FALSE((ret == ESP_OK && nvs_it != NULL), ESP_FAIL,
                      OT_PLAT_LOG_TAG,
...
otError otPlatSettingsGet(otInstance* aInstance,
  ...
  ESP_RETURN_ON_FALSE((s_ot_nvs_handle != 0), OT_ERROR_NOT_FOUND,
                      OT_PLAT_LOG_TAG, "OT NVS handle is invalid.");
  esp_err_t ret = ESP_OK;
  char ot_nvs_key[OT_KEY_INDEX_PATTERN_LEN] = {0};

  ret = find_target_key_using_index(aKey, aIndex, ot_nvs_key,
                                    OT_KEY_INDEX_PATTERN_LEN);
  if (ret != ESP_OK) {
  ...
  *aValueLength = (uint16_t) length;
  ESP_RETURN_ON_FALSE((ret == ESP_OK), OT_ERROR_NOT_FOUND, OT_PLAT_LOG_TAG,
                      "Data not found, err: %d", ret);
...
otError otPlatSettingsSet(otInstance* aInstance,
  ...
  snprintf(ot_nvs_key, sizeof(ot_nvs_key), OT_KEY_INDEX_PATTERN, (uint8_t) aKey,
           0);
  ret = nvs_set_blob(s_ot_nvs_handle, ot_nvs_key, aValue, aValueLength);
  ESP_RETURN_ON_FALSE((ret == ESP_OK), OT_ERROR_NO_BUFS, OT_PLAT_LOG_TAG,
                      "No buffers, err: %d", ret);
  ret = nvs_commit(s_ot_nvs_handle);
  ESP_RETURN_ON_FALSE((ret == ESP_OK), OT_ERROR_NO_BUFS, OT_PLAT_LOG_TAG,
                      "OT NVS handle shut down, err: %d", ret);
  return OT_ERROR_NONE;
}
