...
#if !defined(MBEDTLS_PK_WRITE_C) || !defined(MBEDTLS_PEM_WRITE_C) || !defined(MBEDTLS_FS_IO) || \
...
int main(int argc, char *argv[])
{
    ...
    usage:
        ...
        while ((++curve_info)->name != NULL) mbedtls_printf("    %s\n", curve_info->name);
#endif /* MBEDTLS_ECP_C */
        goto exit;
    }
    ...
    {
        if ((ret = mbedtls_entropy_add_source(&entropy, dev_random_entropy_poll, NULL, DEV_RANDOM_THRESHOLD,
    ...
    {
        ret = mbedtls_ecp_gen_key((mbedtls_ecp_group_id)opt.ec_curve, mbedtls_pk_ec(key), mbedtls_ctr_drbg_random,
                                  &ctr_drbg);
        if (ret != 0)
        {
