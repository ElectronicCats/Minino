...
#if defined(MBEDTLS_AES_C) && defined(MBEDTLS_DHM_C) && defined(MBEDTLS_ENTROPY_C) && defined(MBEDTLS_NET_C) &&     \
    defined(MBEDTLS_RSA_C) && defined(MBEDTLS_SHA256_C) && defined(MBEDTLS_FS_IO) && defined(MBEDTLS_CTR_DRBG_C) && \
...
#if !defined(MBEDTLS_AES_C) || !defined(MBEDTLS_DHM_C) || !defined(MBEDTLS_ENTROPY_C) || !defined(MBEDTLS_NET_C) || \
    !defined(MBEDTLS_RSA_C) || !defined(MBEDTLS_SHA256_C) || !defined(MBEDTLS_FS_IO) ||                             \
    !defined(MBEDTLS_CTR_DRBG_C) || !defined(MBEDTLS_SHA1_C)
int main(void)
{
...
int main(void)
{
    ...
    if ((ret = mbedtls_rsa_pkcs1_verify(&rsa, NULL, NULL, MBEDTLS_RSA_PUBLIC, MBEDTLS_MD_SHA256, 0, hash, p)) != 0)
    {
    ...
    n = dhm.len;
    if ((ret = mbedtls_dhm_make_public(&dhm, (int)dhm.len, buf, n, mbedtls_ctr_drbg_random, &ctr_drbg)) != 0)
    {
    ...
    if ((ret = mbedtls_dhm_calc_secret(&dhm, buf, sizeof(buf), &n, mbedtls_ctr_drbg_random, &ctr_drbg)) != 0)
    {
        mbedtls_printf(" failed\n  ! mbedtls_dhm_calc_secret returned %d\n\n", ret);
        goto exit;
    }

    for (n = 0; n < 16; n++) mbedtls_printf("%02x", buf[n]);
