                                   ...
                                   size_t               dynamic_iv_len)
{
    ...
    for (i = 0; i < dynamic_iv_len; i++) dst_iv[i] ^= dynamic_iv[i];
}
                            ...
                            void *p_rng)
{
    ...
    {
        size_t padding = ssl_compute_padding_length(rec->data_len, MBEDTLS_SSL_TLS1_3_PADDING_GRANULARITY);
    ...
    if (rec->cid_len != 0)
    {
        size_t padding = ssl_compute_padding_length(rec->data_len, MBEDTLS_SSL_CID_PADDING_GRANULARITY);
        /*
                                        ...
                                        ))
    {
        ...
        if (transform->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0)
        {
            ...
            int           ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
            ret = ssl_mac(&transform->md_ctx_enc, transform->mac_enc, data, rec->data_len, rec->ctr, rec->type, mac);
            if (ret == 0)
        ...
        if (mode == MBEDTLS_MODE_CBC)
    {
        ...
        for (i = 0; i <= padlen; i++) data[rec->data_len + i] = (unsigned char)padlen;
...
int mbedtls_ssl_decrypt_buf(mbedtls_ssl_context const *ssl, mbedtls_ssl_transform *transform, mbedtls_record *rec)
{
        ...
        if (mode == MBEDTLS_MODE_CBC)
    {
        ...
        if (auth_done == 1)
        {
            const size_t mask = mbedtls_ct_size_mask_ge(rec->data_len, padlen + 1);
            correct &= mask;
            padlen &= mask;
        }
        else
        {
            ...
            const size_t mask = mbedtls_ct_size_mask_ge(rec->data_len, transform->maclen + padlen + 1);
            correct &= mask;
            padlen &= mask;
        }
            ...
            if (transform->minor_ver > MBEDTLS_SSL_MINOR_VERSION_0)
        {
            ...
            for (idx = start_idx; idx < rec->data_len; idx++)
            {
                ...
                const size_t equal = mbedtls_ct_size_bool_eq(check[idx], padlen - 1);
    ...
    if (auth_done == 0)
    {
        ...
        if (transform->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0)
        {
            ret = ssl_mac(&transform->md_ctx_dec, transform->mac_dec, data, rec->data_len, rec->ctr, rec->type,
                          mac_expect);
            if (ret != 0)
            {
                MBEDTLS_SSL_DEBUG_RET(1, "ssl_mac", ret);
            ...
            if (transform->minor_ver > MBEDTLS_SSL_MINOR_VERSION_0)
        {
            ...
            ret = mbedtls_ct_hmac(&transform->md_ctx_dec, add_data, add_data_len, data, rec->data_len, min_len, max_len,
                                  mac_expect);
            if (ret != 0)
            {
...
static void ssl_bitmask_set(unsigned char *mask, size_t offset, size_t len)
{
    ...
    if (start_bits != 8)
    {
        ...
        if (len <= start_bits)
        {
            for (; len != 0; len--) mask[first_byte_idx] |= 1 << (start_bits - len);
        ...
        for (; start_bits != 0; start_bits--) mask[first_byte_idx] |= 1 << (start_bits - 1);
    }
    ...
    if (end_bits != 0)
    {
        ...
        for (; end_bits != 0; end_bits--) mask[last_byte_idx] |= 1 << (8 - end_bits);
    }
...
static int ssl_check_client_reconnect(mbedtls_ssl_context *ssl)
{
     ...
     */
    if (rec_epoch == 0 && ssl->conf->endpoint == MBEDTLS_SSL_IS_SERVER && ssl->state == MBEDTLS_SSL_HANDSHAKE_OVER &&
...
void mbedtls_ssl_buffering_free(mbedtls_ssl_context *ssl)
{
    ...
    for (offset = 0; offset < MBEDTLS_SSL_MAX_BUFFERED_HS; offset++) ssl_buffering_free_slot(ssl, offset);
}
