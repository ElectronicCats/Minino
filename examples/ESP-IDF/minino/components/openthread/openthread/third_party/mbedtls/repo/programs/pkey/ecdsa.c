...
#if !defined(MBEDTLS_ECDSA_C) || !defined(MBEDTLS_SHA256_C) || !defined(MBEDTLS_ENTROPY_C) || \
...
static void dump_buf(const char *title, unsigned char *buf, size_t len)
{
    ...
    for (i = 0; i < len; i++) mbedtls_printf("%c%c", "0123456789ABCDEF"[buf[i] / 16], "0123456789ABCDEF"[buf[i] % 16]);
    mbedtls_printf("\n");
}
...
int main(int argc, char *argv[])
{
    ...
    if ((ret = mbedtls_ecdsa_genkey(&ctx_sign, ECPARAMS, mbedtls_ctr_drbg_random, &ctr_drbg)) != 0)
    {
