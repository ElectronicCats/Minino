...
static int key_type_is_raw_bytes(psa_key_type_t type) { return (PSA_KEY_TYPE_IS_UNSTRUCTURED(type)); }
...
/****************************************************************/
    ...
    defined(MBEDTLS_PSA_BUILTIN_ALG_ECDSA) || defined(MBEDTLS_PSA_BUILTIN_ALG_DETERMINISTIC_ECDSA) ||             \
...
psa_status_t psa_export_key(mbedtls_svc_key_id_t key, uint8_t *data, size_t data_size, size_t *data_length)
{
    ...
    *data_length = 0;
    ...
    status = psa_driver_wrapper_export_key(&attributes, slot->key.data, slot->key.bytes, data, data_size, data_length);
                                            ...
                                            size_t                     *data_length)
{
    ...
    if (PSA_KEY_TYPE_IS_RSA(type) || PSA_KEY_TYPE_IS_ECC(type))
    {
        ...
        {
#if defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_KEY_PAIR) || defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_PUBLIC_KEY)
            return (mbedtls_psa_rsa_export_public_key(attributes, key_buffer, key_buffer_size, data, data_size,
                                                      data_length));
#else
        ...
        * defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_PUBLIC_KEY) */
        ...
        {
#if defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR) || defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_PUBLIC_KEY)
            return (mbedtls_psa_ecp_export_public_key(attributes, key_buffer, key_buffer_size, data, data_size,
                                                      data_length));
#else
...
static psa_status_t psa_validate_optional_attributes(const psa_key_slot_t *slot, const psa_key_attributes_t *attributes)
{
    ...
    if (attributes->domain_parameters_size != 0)
    {
        ...
        if (PSA_KEY_TYPE_IS_RSA(slot->attr.type))
        {
            ...
            psa_status_t status =
                mbedtls_psa_rsa_load_representation(slot->attr.type, slot->key.data, slot->key.bytes, &rsa);
            if (status != PSA_SUCCESS)
                return (status);
            ...
            if (ret != 0)
                goto rsa_exit;
            ret = mbedtls_mpi_read_binary(&required, attributes->domain_parameters, attributes->domain_parameters_size);
            if (ret != 0)
                goto rsa_exit;
                          ...
                          mbedtls_svc_key_id_t       *target_key)
{
    ...
    *target_key = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    status = psa_restrict_key_policy(source_slot->attr.type, &actual_attributes.core.policy, &source_slot->attr.policy);
    if (status != PSA_SUCCESS)
        goto exit;
                              ...
                              size_t          hash_length)
{
    ...
    psa_status_t status = psa_driver_wrapper_hash_compute(alg, input, input_length, actual_hash, sizeof(actual_hash),
                                                          &actual_hash_length);
    if (status != PSA_SUCCESS)
        goto exit;
    if (actual_hash_length != hash_length)
                                  ...
                                  int                  is_sign)
{
    ...
    if (is_sign)
    {
        status = psa_driver_wrapper_mac_sign_setup(operation, &attributes, slot->key.data, slot->key.bytes, alg);
    }
    else
    {
        status = psa_driver_wrapper_mac_verify_setup(operation, &attributes, slot->key.data, slot->key.bytes, alg);
    }
                                             ...
                                             int                  is_sign)
{
    ...
    status = psa_driver_wrapper_mac_compute(&attributes, slot->key.data, slot->key.bytes, alg, input, input_length, mac,
                                      ...
                                      size_t              *signature_length)
{
    ...
    *signature_length = 0;
    ...
    if (input_is_message)
    {
        status = psa_driver_wrapper_sign_message(&attributes, slot->key.data, slot->key.bytes, alg, input, input_length,
                                                 signature, signature_size, signature_length);
    }
    else
    {
        status = psa_driver_wrapper_sign_hash(&attributes, slot->key.data, slot->key.bytes, alg, input, input_length,
                                              signature, signature_size, signature_length);
    }
                                        ...
                                        size_t               signature_length)
{
    ...
    {
        status = psa_driver_wrapper_verify_hash(&attributes, slot->key.data, slot->key.bytes, alg, input, input_length,
                                   ...
                                   size_t                     *signature_length)
{
    if (attributes->core.type == PSA_KEY_TYPE_RSA_KEY_PAIR)
    {
        if (PSA_ALG_IS_RSA_PKCS1V15_SIGN(alg) || PSA_ALG_IS_RSA_PSS(alg))
        {
            ...
            return (mbedtls_psa_rsa_sign_hash(attributes, key_buffer, key_buffer_size, alg, hash, hash_length,
                                              signature, signature_size, signature_length));
...
static void psa_rsa_oaep_set_padding_mode(psa_algorithm_t alg, mbedtls_rsa_context *rsa)
{
    ...
    if (PSA_KEY_TYPE_IS_RSA(slot->attr.type))
    {
        ...
        * defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_OAEP) */
        if (alg == PSA_ALG_RSA_PKCS1V15_CRYPT)
        {
            ...
            status =
                mbedtls_to_psa_error(mbedtls_rsa_pkcs1_encrypt(rsa, mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE,
                                                               MBEDTLS_RSA_PUBLIC, input_length, input, output));
#else
            status = PSA_ERROR_NOT_SUPPORTED;
        ...
        else if (PSA_ALG_IS_RSA_OAEP(alg))
        {
            ...
            status = mbedtls_to_psa_error(
                mbedtls_rsa_rsaes_oaep_encrypt(rsa, mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE,
                                               MBEDTLS_RSA_PUBLIC, salt, salt_length, input_length, input, output));
#else
            status = PSA_ERROR_NOT_SUPPORTED;
        ...
        * defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_OAEP) */
        ...
        if (alg == PSA_ALG_RSA_PKCS1V15_CRYPT)
        {
            ...
            status = mbedtls_to_psa_error(mbedtls_rsa_pkcs1_decrypt(rsa, mbedtls_psa_get_random,
                                                                    MBEDTLS_PSA_RANDOM_STATE, MBEDTLS_RSA_PRIVATE,
        ...
        else if (PSA_ALG_IS_RSA_OAEP(alg))
        {
            ...
            status = mbedtls_to_psa_error(mbedtls_rsa_rsaes_oaep_decrypt(
                rsa, mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE, MBEDTLS_RSA_PRIVATE, salt, salt_length,
                output_length, input, output, output_size));
#else
            status = PSA_ERROR_NOT_SUPPORTED;
#endif /* MBEDTLS_PSA_BUILTIN_ALG_RSA_OAEP */
                                     ...
                                     mbedtls_operation_t     cipher_operation)
{
    ...
    if (cipher_operation == MBEDTLS_ENCRYPT)
        status = psa_driver_wrapper_cipher_encrypt_setup(operation, &attributes, slot->key.data, slot->key.bytes, alg);
    else
        status = psa_driver_wrapper_cipher_decrypt_setup(operation, &attributes, slot->key.data, slot->key.bytes, alg);
                                ...
                                size_t              *output_length)
{
    ...
    status = psa_driver_wrapper_cipher_encrypt(&attributes, slot->key.data, slot->key.bytes, alg, local_iv,
                                ...
                                size_t              *output_length)
{
    ...
    status = psa_driver_wrapper_cipher_decrypt(&attributes, slot->key.data, slot->key.bytes, alg, input, input_length,
...
psa_status_t psa_key_derivation_abort(psa_key_derivation_operation_t *operation)
{
            ...
            PSA_ALG_IS_TLS12_PSK_TO_MS(kdf_alg))
    {
        ...
        {
            mbedtls_platform_zeroize(operation->ctx.tls12_prf.secret, operation->ctx.tls12_prf.secret_length);
                                                      ...
                                                      psa_key_derivation_operation_t *operation)
{
    ...
    status = psa_driver_wrapper_import_key(&attributes, data, bytes, slot->key.data, slot->key.bytes, &slot->key.bytes,
                                           &bits);
                                                   ...
                                                   size_t         *shared_secret_length)
{
    ...
    case PSA_ALG_ECDH:
        ...
        psa_status_t status      = mbedtls_psa_ecp_load_representation(private_key->attr.type, private_key->attr.bits,
...
psa_status_t psa_generate_key(const psa_key_attributes_t *attributes, mbedtls_svc_key_id_t *key)
{
    ...
    *key = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    if (slot->key.data == NULL)
    {
        ...
        {
            status = psa_validate_key_type_and_size_for_key_generation(attributes->core.type, attributes->core.bits);
            if (status != PSA_SUCCESS)
                goto exit;
