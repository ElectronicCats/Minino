...
static ip_addr_t map_openthread_addr_to_lwip_addr(const otIp6Address* address) {
  ...
  memcpy(ip_2_ip6(&addr)->addr, address->mFields.m8,
         sizeof(ip_2_ip6(&addr)->addr));
#if CONFIG_LWIP_IPV4
...
static void udp_recv_task(void* ctx) {
  ...
  otMessageSettings msg_settings = {.mLinkSecurityEnabled = false,
...
#endif
  memcpy(&message_info.mPeerAddr, ip_2_ip6(&task->addr)->addr,
         sizeof(message_info.mPeerAddr));
  ...
  VerifyOrExit(
      ...
      ESP_LOGE(
          OT_PLAT_LOG_TAG,
          "Failed to allocate data buf when receiving OpenThread plat UDP"));
  message = otUdpNewMessage(esp_openthread_get_instance(), &msg_settings);
  VerifyOrExit(message != NULL,
               ESP_LOGE(OT_PLAT_LOG_TAG,
                        "Failed to allocate OpenThread message when receiving "
                        "OpenThread plat UDP"));
  VerifyOrExit(
      otMessageAppend(message, data_buf, recv_buf->tot_len) == OT_ERROR_NONE,
      ESP_LOGE(OT_PLAT_LOG_TAG,
               "Failed to copy OpenThread message when receiving OpenThread "
               "plat UDP"));
...
static void handle_udp_recv(void* ctx,
                            struct udp_pcb* pcb,
                            struct pbuf* p,
  ...
  if (task == NULL) {
    ESP_LOGE(OT_PLAT_LOG_TAG,
             "Failed to allocate recv task when receiving OpenThread plat UDP");
  }
...
#if CONFIG_LWIP_IPV4
  task->hop_limit =
      (addr->type == IPADDR_TYPE_V6) ? IP6H_HOPLIM(ip6_hdr) : IPH_TTL(ip4_hdr);
#else
  task->hop_limit = IP6H_HOPLIM(ip6_hdr);
#endif
  task->is_host_interface =
      (netif_get_index(source_netif) ==
       esp_netif_get_netif_impl_index(esp_openthread_get_backbone_netif()));
...
otError otPlatUdpSocket(otUdpSocket* udp_socket) {
  ...
  udp_new_task_t task = {.source_task = xTaskGetCurrentTaskHandle(),
                         .socket = udp_socket};
...
otError otPlatUdpBind(otUdpSocket* udp_socket) {
  ...
  ESP_LOGI(OT_PLAT_LOG_TAG, "Platform UDP bound to port %d",
           udp_socket->mSockName.mPort);

#if CONFIG_LWIP_IPV4
  task.addr.type = IPADDR_TYPE_ANY;
#endif
  memcpy(ip_2_ip6(&task.addr)->addr, udp_socket->mSockName.mAddress.mFields.m8,
         sizeof(ip_2_ip6(&task.addr)->addr));
...
static uint8_t get_netif_index(otNetifIdentifier netif_identifier) {
  switch (netif_identifier) {
    ...
    case OT_NETIF_BACKBONE:
      return esp_netif_get_netif_impl_index(
...
otError otPlatUdpBindToNetif(otUdpSocket* udp_socket,
...
static void udp_send_task(void* ctx) {
  ...
  if (task->netif_index == get_netif_index(OT_NETIF_THREAD)) {
    // If the input arguments indicated the netif is OT, directly send the
    // message.
    err = udp_sendto_if_src(
        task->pcb, send_buf, &task->peer_addr, task->peer_port,
        netif_get_by_index(task->netif_index), &task->source_addr);
  } else {
...
static inline bool is_addr_ip6_any(const ip_addr_t* addr) {
  return ip_2_ip6(addr)->addr[0] == 0 && ip_2_ip6(addr)->addr[1] == 0 &&
         ip_2_ip6(addr)->addr[2] == 0 && ip_2_ip6(addr)->addr[3] == 0
#if CONFIG_LWIP_IPV4
         && addr->type == IPADDR_TYPE_V6
#endif
      ;
...
otError otPlatUdpSend(otUdpSocket* udp_socket,
                      otMessage* message,
  ...
  task->source_addr =
      map_openthread_addr_to_lwip_addr(&message_info->mSockAddr);
  task->peer_addr = map_openthread_addr_to_lwip_addr(&message_info->mPeerAddr);
#if CONFIG_LWIP_IPV4
  if (task->peer_addr.type == IPADDR_TYPE_V4 &&
      is_addr_ip6_any(&task->source_addr)) {
    task->source_addr.type = IPADDR_TYPE_ANY;
  }
#endif

  if (is_link_local(&message_info->mPeerAddr) ||
      is_multicast(&message_info->mPeerAddr)) {
    task->netif_index = get_netif_index(
        message_info->mIsHostInterface ? OT_NETIF_BACKBONE : OT_NETIF_THREAD);
  }

  if (is_openthread_internal_mesh_local_addr(&message_info->mPeerAddr)) {
    // If the destination address is a openthread mesh local address, set the
    // netif OT.
...
static void udp_multicast_join_leave_task(void* ctx) {
  udp_multicast_join_leave_task_t* task =
      (udp_multicast_join_leave_task_t*) ctx;
  struct netif* target = netif_get_by_index(task->netif_index);

  if (target == NULL) {
    ESP_LOGE(OT_PLAT_LOG_TAG,
             "Failed to %s multicast group, index%d netif is not ready",
...
otError otPlatUdpJoinMulticastGroup(otUdpSocket* socket,
                                    otNetifIdentifier netif_id,
                                    const otIp6Address* addr) {
  udp_multicast_join_leave_task_t* task =
      (udp_multicast_join_leave_task_t*) malloc(
          sizeof(udp_multicast_join_leave_task_t));
  otError error = OT_ERROR_NONE;
...
otError otPlatUdpLeaveMulticastGroup(otUdpSocket* socket,
                                     otNetifIdentifier netif_id,
                                     const otIp6Address* addr) {
  udp_multicast_join_leave_task_t* task =
      (udp_multicast_join_leave_task_t*) malloc(
