...
esp_err_t esp_openthread_task_queue_init(
  ...
  s_task_queue =
      xQueueCreate(config->port_config.task_queue_size, sizeof(task_storage_t));
  ...
  return esp_openthread_platform_workflow_register(
      &esp_openthread_task_queue_update, &esp_openthread_task_queue_process,
      task_queue_workflow);
}

esp_err_t IRAM_ATTR esp_openthread_task_queue_post(esp_openthread_task_t task,
                                                   void* arg) {
  ...
  if (!xPortCanYield()) {
    ESP_RETURN_ON_FALSE_ISR(
        xQueueSendFromISR(s_task_queue, &task_storage, &task_woken), ESP_FAIL,
        OT_PLAT_LOG_TAG, "Failed to post task to OpenThread task queue");
  } else {
    ESP_RETURN_ON_FALSE(xQueueSend(s_task_queue, &task_storage, portMAX_DELAY),
...
void esp_openthread_task_queue_update(
...
esp_err_t esp_openthread_task_queue_process(
  ...
  ESP_RETURN_ON_FALSE(s_task_queue != NULL, ESP_ERR_INVALID_STATE,
                      OT_PLAT_LOG_TAG, "OpenThread task queue not initialized");
