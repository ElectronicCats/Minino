...
class SpiSpinelInterface : public ot::Spinel::SpinelInterface {
 public:
   ...
   *
   * @note This method should be called before reading and sending spinel frames
   * to the interface.
   *
  ...
  /**
   * @brief   This method encodes and sends a spinel frame to Radio Co-processor
   * (RCP) over the socket.
   *
   * @param[in] frame     A pointer to buffer containing the spinel frame to
   * send.
   * @param[in] length    The length (number of bytes) in the frame.
   *
   * @return
   *      -OT_ERROR_NONE     Successfully encoded and sent the spinel frame.
   *      -OT_ERROR_NO_BUFS  Insufficient buffer space available to encode the
   * frame. -OT_ERROR_FAILED   Failed to send due to socket not becoming
  ...
  /**
   * This method waits for receiving part or all of spinel frame within
   * specified timeout.
   *
   ...
   *      -OT_ERROR_RESPONSE_TIMEOUT No spinel frame is received within @p
   * timeout_us.
   *
   */
  ...
  /**
   * This method is called when RCP failure detected and resets internal states
   * of the interface.
   *
   */
   ...
   *
   */
  void RegisterRcpFailureHandler(esp_openthread_rcp_failure_handler handler) {
    mRcpFailureHandler = handler;
  }
 ...
 private:
  ...
  static constexpr uint16_t kSPIFrameSize =
      ot::Spinel::SpinelInterface::kMaxFrameSize + kSPIFrameHeaderSize;
  ...
  esp_err_t ConductSPITransaction(bool reset,
