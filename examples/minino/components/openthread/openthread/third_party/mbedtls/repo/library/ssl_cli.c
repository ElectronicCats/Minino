...
static int ssl_write_alpn_ext(mbedtls_ssl_context *ssl, unsigned char *buf, const unsigned char *end, size_t *olen)
{
    ...
    *olen = 0;
    ...
    for (cur = ssl->conf->alpn_list; *cur != NULL; cur++) alpnlen += strlen(*cur) + 1;
...
static int ssl_write_client_hello(mbedtls_ssl_context *ssl)
{
    ...
    *p++ = (unsigned char)n;
    ...
    for (i = 0; i < n; i++) *p++ = ssl->session_negotiate->id[i];
    ...
    for (i = 0; ciphersuites[i] != 0; i++)
    {
        ...
        MBEDTLS_SSL_DEBUG_MSG(
            3, ("client hello, add ciphersuite: %#04x (%s)", (unsigned int)ciphersuites[i], ciphersuite_info->name));
...
static int ssl_parse_server_ecdh_params_psa(mbedtls_ssl_context *ssl, unsigned char **p, unsigned char *end)
{
    ...
    if (mbedtls_psa_tls_ecpoint_to_psa_ec(*p, ecpoint_len, handshake->ecdh_psa_peerkey,
                                          sizeof(handshake->ecdh_psa_peerkey), &handshake->ecdh_psa_peerkey_len) != 0)
    {
...
static int ssl_write_client_key_exchange(mbedtls_ssl_context *ssl)
{
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_DHE_RSA)
    {
        ...
        ret = mbedtls_dhm_make_public(&ssl->handshake->dhm_ctx, (int)mbedtls_mpi_size(&ssl->handshake->dhm_ctx.P),
                                      &ssl->out_msg[header_len], content_len, ssl->conf->f_rng, ssl->conf->p_rng);
        if (ret != 0)
        {
            ...
            ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_ECDSA)
    {
        ...
        status = psa_export_public_key(handshake->ecdh_psa_privkey, own_pubkey, sizeof(own_pubkey), &own_pubkey_len);
        if (status != PSA_SUCCESS)
            return (MBEDTLS_ERR_SSL_HW_ACCEL_FAILED);
...
#endif
        if ((ret =
                 mbedtls_ecdh_calc_secret(&ssl->handshake->ecdh_ctx, &ssl->handshake->pmslen, ssl->handshake->premaster,
        ...
        if (mbedtls_ssl_ciphersuite_uses_psk(ciphersuite_info))
    {
            ...
            if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_PSK)
        {
             ...
             */
            ret =
                mbedtls_ecdh_make_public(&ssl->handshake->ecdh_ctx, &content_len, &ssl->out_msg[header_len],
                                         MBEDTLS_SSL_OUT_CONTENT_LEN - header_len, ssl->conf->f_rng, ssl->conf->p_rng);
            if (ret != 0)
            {
                MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_ecdh_make_public", ret);
                return (ret);
        ...
        if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE)
    {
        ...
        ret = mbedtls_ecjpake_write_round_two(&ssl->handshake->ecjpake_ctx, ssl->out_msg + header_len,
                                              MBEDTLS_SSL_OUT_CONTENT_LEN - header_len, &content_len, ssl->conf->f_rng,
                                              ssl->conf->p_rng);
        if (ret != 0)
        {
