...
static void module_list_cb_event(uint8_t button_name, uint8_t button_event) {
  ...
  switch (button_name) {
    ...
    case BUTTON_RIGHT:
      tracker_information_free();
      xSemaphoreTake(trackers_mutex, portMAX_DELAY);
      if (scanned_airtags == NULL || current_item >= trackers_count) {
        xSemaphoreGive(trackers_mutex);
        break;
      }
      char tracker_mac[20];
      ...
      tracker_information[0] = strdup(tracker_name);
      tracker_information[1] = strdup(tracker_rssi);
      tracker_information[3] = strdup(tracker_mac);
      xSemaphoreGive(trackers_mutex);
