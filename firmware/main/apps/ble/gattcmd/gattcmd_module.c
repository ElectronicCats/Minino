...
static uint16_t count_char_elements = 0;
...
static uint16_t serviceCount = 0;
  ...
  switch (event) {
    ...
    case ESP_GATTC_DIS_SRVC_CMPL_EVT:
      if (param->dis_srvc_cmpl.status != ESP_GATT_OK) {
        ESP_LOGE(GATTC_TAG, "Service discover failed, status %d",
      ...
      esp_ble_gattc_get_attr_count(
          gattc_if, param->dis_srvc_cmpl.conn_id, ESP_GATT_DB_CHARACTERISTIC,
          gl_profile_tab[PROFILE_A_APP_ID].service_start_handle,
          gl_profile_tab[PROFILE_A_APP_ID].service_end_handle,
          gl_profile_tab[PROFILE_A_APP_ID].char_handle, &serviceCount);

      ESP_LOGI(GATTC_TAG, "Characteristic count: %d", serviceCount);
      break;
    case ESP_GATTC_CFG_MTU_EVT:
      ESP_LOGI(GATTC_TAG, "MTU exchange, status %d, MTU %d",
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT:
      if (p_data->search_cmpl.status != ESP_GATT_OK) {
        ESP_LOGE(GATTC_TAG, "Service search failed, status %x",
      ...
      if (get_server) {
        ...
        if (count > 0) {
          ESP_LOGI(GATTC_TAG, "ESP_GATTC_SEARCH_CMPL_EVT Count: %d", count);
          count_char_elements = count;
          ...
          if (!char_elem_result) {
            ...
            ESP_LOGW(GATTC_TAG, "Locking for all char");
            ...
            for (int i = 0; i < count; i++) {
              ESP_LOGI(GATTC_TAG, "Char %d", i);
              ESP_LOGI(GATTC_TAG, "UUID16 (%d): %x",
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
      if (p_data->reg_for_notify.status != ESP_GATT_OK) {
        ESP_LOGE(GATTC_TAG, "Notification register failed, status %d",
    ...
    case ESP_GATTC_WRITE_DESCR_EVT:
      if (p_data->write.status != ESP_GATT_OK) {
        ESP_LOGE(GATTC_TAG, "Descriptor write failed, status %x",
                 p_data->write.status);
        break;
      }
      ...
      esp_ble_gattc_write_char(
          gattc_if, gl_profile_tab[PROFILE_A_APP_ID].conn_id,
          gl_profile_tab[PROFILE_A_APP_ID].char_handle, sizeof(write_char_data),
          write_char_data, ESP_GATT_WRITE_TYPE_RSP, ESP_GATT_AUTH_REQ_NONE);
      break;
    case ESP_GATTC_SRVC_CHG_EVT: {
      esp_bd_addr_t bda;
      memcpy(bda, p_data->srvc_chg.remote_bda, sizeof(esp_bd_addr_t));
      ESP_LOGI(GATTC_TAG, "Service change from " ESP_BD_ADDR_STR "",
               ESP_BD_ADDR_HEX(bda));
      break;
    }
    case ESP_GATTC_WRITE_CHAR_EVT:
      if (p_data->write.status != ESP_GATT_OK) {
        ESP_LOGE(GATTC_TAG, "Characteristic write failed, status %x)",
                 p_data->write.status);
        break;
      }
  ...
  switch (event) {
    ...
    case ESP_GAP_BLE_SCAN_START_COMPLETE_EVT:
      ...
      if (param->scan_start_cmpl.status != ESP_BT_STATUS_SUCCESS) {
        ESP_LOGE(GATTC_TAG, "Scanning start failed, status %x",
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
          ...
          if (adv_name != NULL) {
            if (memcmp(target, scan_result->scan_rst.bda, 6) == 0) {
              ESP_LOGE(GATTC_TAG, "I Found you");
    ...
    case ESP_GAP_BLE_SCAN_STOP_COMPLETE_EVT:
      if (param->scan_stop_cmpl.status != ESP_BT_STATUS_SUCCESS) {
        ESP_LOGE(GATTC_TAG, "Scanning stop failed, status %x",
    ...
    case ESP_GAP_BLE_ADV_STOP_COMPLETE_EVT:
      if (param->adv_stop_cmpl.status != ESP_BT_STATUS_SUCCESS) {
        ESP_LOGE(GATTC_TAG, "Advertising stop failed, status %x",
...
void gattcmd_module_disconnect() {
  initialized = false;
  esp_err_t err = esp_ble_gattc_close(gl_profile_tab[PROFILE_NUM].gattc_if,
                                      gl_profile_tab[PROFILE_NUM].conn_id);
  if (err != ESP_OK) {
    ESP_LOGE(GATTC_TAG, "Error with the disconnection: %d", err);
    return;
  }
