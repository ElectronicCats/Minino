...
#define GATTC_WRITE_TAG "GATT_SWRITE"
...
uint16_t to_bettercap(uint16_t wireshark_value) {
  return 0x10000 - wireshark_value;
}
        ...
        if (count > 0) {
          ...
          for (int i = 0; i < count; i++) {
            ...
            if (char_elem_result[i].uuid.uuid.uuid16 == gatt_target_uuid) {
              for (int j = 0; j < idx; j++) {
                printf("Value[%d]: %s\n", j, values_array[j]);
                gatt_target_value_len =
                    hex_string_to_bytes(values_array[j], gatt_target_value,
                ...
                if (res == ESP_OK) {
                  printf("[" GATTC_WRITE_TAG "] Write %d done\n", j);
                } else {
                  printf("[" GATTC_WRITE_TAG "] Write %d error: %d\n", j, res);
                }
...
void gatt_read_file() {
  ...
  while (fgets(line, sizeof(line), file)) {
    ...
    if (addr && handle_str && value) {
      ...
      if (!configured & (idx > 0)) {
        gatt_target_uuid = hex_string_to_uint16(handle_str);
        parse_address_colon(addr, target_bda);
        configured = true;
        printf("Addr: %s - ", addr);
        printf("Handle: %s (0x%04x) - ", handle_str, gatt_target_uuid);
        printf("Value: %s\n", value);
      }
      idx++;
    } else {
