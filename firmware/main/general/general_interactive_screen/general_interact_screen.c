...
static void input_cb(uint8_t button_name, uint8_t button_event) {
  ...
  switch (button_name) {
    case BUTTON_LEFT:
      screen_back_selection_handler_t exit_cb = interactive_screen_ctx->select_back_cb;
      ...
      break;
    case BUTTON_RIGHT:
      screen_select_selection_handler_t select_select_cb = interactive_screen_ctx->select_select_cb;
      if (select_select_cb) {
    ...
    case BUTTON_UP:
      counter = counter == interactive_screen_ctx->range_high
                    ...
                    : counter + 1;
      screen_up_selection_handler_t select_up_cb = interactive_screen_ctx->select_up_cb;
      if (select_up_cb) {
    ...
    case BUTTON_DOWN:
      counter = counter == interactive_screen_ctx->range_low
                    ...
                    : counter - 1;
      screen_down_selection_handler_t select_down_cb= interactive_screen_ctx->select_down_cb;
      if (select_down_cb) {
