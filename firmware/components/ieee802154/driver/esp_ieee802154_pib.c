...
static uint8_t ieee802154_txpower_convert(int8_t txpower) {
  ...
  if (txpower >= IEEE802154_TXPOWER_VALUE_MAX) {
    ...
    ieee820154_txpower_index =
        (uint8_t) ((txpower - IEEE802154_TXPOWER_VALUE_MIN) / 3) +
...
void ieee802154_pib_update(void) {
  if (ieee802154_pib_is_pending()) {
    ieee802154_ll_set_freq(
        ieee802154_channel_to_freq(s_ieee802154_pib.channel));
    ieee802154_ll_set_power(
        ieee802154_txpower_convert(s_ieee802154_pib.txpower));
    ...
    ieee802154_ll_set_pending_mode(s_ieee802154_pib.pending_mode ==
                                   IEEE802154_AUTO_PENDING_ENHANCED);
...
void ieee802154_pib_set_pending_mode(
    ieee802154_ll_pending_mode_t pending_mode) {
