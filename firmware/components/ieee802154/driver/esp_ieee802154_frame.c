...
IEEE802154_STATIC IEEE802154_INLINE bool is_security_enabled(
    const uint8_t* frame) {
  return frame[IEEE802154_FRAME_SECURITY_OFFSET] &
         IEEE802154_FRAME_SECURITY_BIT;
}
...
IEEE802154_STATIC IEEE802154_INLINE uint8_t
dst_addr_mode(const uint8_t* frame) {
  return frame[IEEE802154_FRAME_DST_MODE_OFFSET] &
         IEEE802154_FRAME_DST_MODE_MASK;
}

IEEE802154_STATIC IEEE802154_INLINE uint8_t
src_addr_mode(const uint8_t* frame) {
  return frame[IEEE802154_FRAME_SRC_MODE_OFFSET] &
         IEEE802154_FRAME_SRC_MODE_MASK;
}

IEEE802154_STATIC IEEE802154_INLINE bool is_panid_compression(
    const uint8_t* frame) {
  return frame[IEEE802154_FRAME_PANID_COMP_OFFSET] &
         IEEE802154_FRAME_PANID_COMP_BIT;
}

IEEE802154_STATIC IEEE802154_INLINE bool is_suported_frame_type(
    uint8_t frame_type) {
  // HW supports 4 kinds of frame type: Beacon, Ack, Data, Command.
  return (frame_type == IEEE802154_FRAME_TYPE_BEACON ||
          frame_type == IEEE802154_FRAME_TYPE_DATA ||
          frame_type == IEEE802154_FRAME_TYPE_ACK ||
          frame_type == IEEE802154_FRAME_TYPE_COMMAND);
}
...
IEEE802154_STATIC bool is_dst_panid_present(const uint8_t* frame) {
  ...
  if (ieee802154_frame_get_version(frame) == IEEE802154_FRAME_VERSION_2) {
    ...
    if (dst_mode != IEEE802154_FRAME_DST_MODE_NONE) {  // dest address is
                                                       // present/short/extented
      if ((src_mode == IEEE802154_FRAME_SRC_MODE_NONE && panid_compression) ||
          (dst_mode == IEEE802154_FRAME_DST_MODE_EXT &&
           src_mode == IEEE802154_FRAME_SRC_MODE_EXT && panid_compression)) {
...
IEEE802154_STATIC bool is_src_panid_present(const uint8_t* frame) {
  ...
  if (ieee802154_frame_get_version(frame) == IEEE802154_FRAME_VERSION_2) {
    ...
    if (src_mode != IEEE802154_FRAME_SRC_MODE_NONE) {
      if (dst_mode == IEEE802154_FRAME_DST_MODE_EXT &&
          src_mode == IEEE802154_FRAME_SRC_MODE_EXT && !panid_compression) {
...
IEEE802154_STATIC uint8_t IEEE802154_INLINE
ieee802154_frame_address_offset(const uint8_t* frame) {
  return IEEE802154_FRAME_PHR_SIZE + IEEE802154_FRAME_FCF_SIZE +
         (is_dsn_present(frame) ? IEEE802154_FRAME_DSN_SIZE : 0);
}

IEEE802154_STATIC IRAM_ATTR uint8_t
ieee802154_frame_address_size(const uint8_t* frame) {
...
IEEE802154_STATIC uint8_t
ieee802154_frame_security_header_offset(const uint8_t* frame) {
  ESP_RETURN_ON_FALSE_ISR(
      is_suported_frame_type(ieee802154_frame_get_type(frame)),
      IEEE802154_FRAME_INVALID_ADDR_MODE, TAG, "invalid frame type");
  uint8_t offset = ieee802154_frame_address_offset(frame);
  uint8_t address_size = ieee802154_frame_address_size(frame);

  ESP_RETURN_ON_FALSE_ISR(offset != IEEE802154_FRAME_INVALID_OFFSET,
                          IEEE802154_FRAME_INVALID_OFFSET, TAG,
                          "invalid offset");
  ESP_RETURN_ON_FALSE_ISR(address_size != IEEE802154_FRAME_INVALID_OFFSET,
                          IEEE802154_FRAME_INVALID_OFFSET, TAG,
                          "invalid offset");
...
IEEE802154_STATIC uint8_t
ieee802154_frame_get_security_field_len(const uint8_t* frame) {
  ESP_RETURN_ON_FALSE_ISR(
      is_suported_frame_type(ieee802154_frame_get_type(frame)),
      IEEE802154_FRAME_INVALID_OFFSET, TAG, "invalid frame type");
...
IEEE802154_STATIC uint8_t
ieee802154_frame_ie_header_offset(const uint8_t* frame) {
  uint8_t offset = ieee802154_frame_security_header_offset(frame);
...
IEEE802154_STATIC uint8_t
ieee802154_frame_get_ie_field_len(const uint8_t* frame) {
  uint8_t offset = ieee802154_frame_ie_header_offset(frame);
  uint8_t ie_field_len = 0;
  uint8_t frame_footer_len =
      ieee802154_frame_get_mic_len(frame) + IEEE802154_FRAME_FCS_SIZE;
  ...
  /* If the `offset + frame_footer_len == frame_len`, we exit the `while()`
     ...
     footer length. (for details, please reference 2015 - spec table 7 - 6 in
     page 169) */

  while (frame[0] > offset + ie_field_len + frame_footer_len) {
    uint16_t ie_header =
        frame[offset + ie_field_len + 1] << 8 | frame[offset + ie_field_len];
    // Header Termination IE 2 is used in to signal end of the MHR and beginning
    // of the MAC Payload.
    if ((ie_header & IEEE802154_FRAME_IE_HEAD_ID_MASK) ==
        IEEE802154_IE_TYPE_HT2) {
      ie_field_len += IEEE802154_FRAME_IE_HEAD_LEN;
      break;
    } else {
      uint8_t ie_subfield_len =
          (ie_header & IEEE802154_FRAME_IE_SUBFIELD_LEN_MASK);
...
IEEE802154_STATIC IRAM_ATTR uint8_t
ieee802154_frame_payload_offset(const uint8_t* frame) {
  uint8_t offset = ieee802154_frame_security_header_offset(frame);
  ...
  if (ieee802154_frame_get_version(frame) == IEEE802154_FRAME_VERSION_0 ||
...
uint8_t ieee802154_frame_get_dst_addr(const uint8_t* frame, uint8_t* addr) {
  ...
  ESP_RETURN_ON_FALSE_ISR(dst_mode == IEEE802154_FRAME_DST_MODE_SHORT ||
                              dst_mode == IEEE802154_FRAME_DST_MODE_EXT,
                          dst_mode, TAG, "invalid address mode");

  addr_size = (dst_mode == IEEE802154_FRAME_DST_MODE_SHORT)
...
uint8_t ieee802154_frame_get_src_addr(const uint8_t* frame, uint8_t* addr) {
  ...
  ESP_RETURN_ON_FALSE_ISR(src_mode == IEEE802154_FRAME_SRC_MODE_SHORT ||
                              src_mode == IEEE802154_FRAME_SRC_MODE_EXT,
                          src_mode, TAG, "invalid address mode");

  addr_size = (src_mode == IEEE802154_FRAME_SRC_MODE_SHORT)
                  ? IEEE802154_FRAME_SHORT_ADDR_SIZE
                  : IEEE802154_FRAME_EXT_ADDR_SIZE;
...
esp_err_t ieee802154_frame_get_dest_panid(const uint8_t* frame,
                                          uint8_t* panid) {
  uint8_t offset = ieee802154_frame_address_offset(frame);
