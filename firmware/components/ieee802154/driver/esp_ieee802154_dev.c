...
// STUB : Stub buffer, used when all valid buffers are under processing, the
// received frame will be dropped.
static uint8_t s_rx_frame[CONFIG_IEEE802154_RX_BUFFER_SIZE + 1]
                         [IEEE802154_RX_FRAME_SIZE];
static esp_ieee802154_frame_info_t
    s_rx_frame_info[CONFIG_IEEE802154_RX_BUFFER_SIZE + 1];
...
static void ieee802154_receive_done(uint8_t* data,
  ...
  if (s_rx_index == CONFIG_IEEE802154_RX_BUFFER_SIZE) {
    ...
    switch (radio_selector_get_selected_option()) {
      ...
      case RADIO_SELECT_ZIGBEE_SNIFFER:
        sniffer_esp_ieee802154_receive_done(data, frame_info);
        break;
      case RADIO_SELECT_THREAD:
...
static void ieee802154_transmit_done(
    const uint8_t* frame,
    const uint8_t* ack,
...
static void ieee802154_rx_frame_info_update(void) {
  ...
  int8_t rssi =
      s_rx_frame[s_rx_index][len - 1];  // crc is not written to rx buffer
  uint8_t lqi = s_rx_frame[s_rx_index][len];

  s_rx_frame_info[s_rx_index].channel =
      ieee802154_freq_to_channel(ieee802154_ll_get_freq());
...
IEEE802154_STATIC void set_next_rx_buffer(void) {
  ...
  if (s_rx_index != CONFIG_IEEE802154_RX_BUFFER_SIZE &&
      s_rx_frame_info[s_rx_index].process == false) {
  ...
  // If all buffer is under processing by the upper layer, we set the stub
  // buffer, and will not post the received frame to the upper layer.
  if (!next_rx_buffer) {
...
static bool stop_rx(void) {
  ...
  if (events & IEEE802154_EVENT_RX_DONE) {
    ieee802154_receive_done((uint8_t*) s_rx_frame[s_rx_index],
                            &s_rx_frame_info[s_rx_index]);
  }

  ieee802154_ll_clear_events(IEEE802154_EVENT_RX_DONE |
                             IEEE802154_EVENT_RX_ABORT |
                             IEEE802154_EVENT_RX_SFD_DONE);
...
static bool stop_tx_ack(void) {
  ...
  ieee802154_ll_clear_events(
      IEEE802154_EVENT_ACK_TX_DONE | IEEE802154_EVENT_RX_ABORT |
      IEEE802154_EVENT_TX_SFD_DONE);  // ZB-81: clear TX_SFD_DONE event
...
static bool stop_tx(void) {
  ...
  if (s_ieee802154_state == IEEE802154_STATE_TX_ENH_ACK) {
    // if current operation is sending 2015 Enh-ack, SW should create the
    // receive-done event.
    ieee802154_receive_done((uint8_t*) s_rx_frame[s_rx_index],
                            &s_rx_frame_info[s_rx_index]);
    ieee802154_ll_clear_events(IEEE802154_EVENT_ACK_TX_DONE);
  } else if ((events & IEEE802154_EVENT_TX_DONE) &&
             (!ieee802154_frame_is_ack_required(s_tx_frame) ||
              !ieee802154_ll_get_rx_auto_ack())) {
    // if the tx is already done, and the frame is not ack request OR auto ack
    // rx is disabled.
  ...
  ieee802154_ll_clear_events(IEEE802154_EVENT_TX_DONE |
                             IEEE802154_EVENT_TX_ABORT |
                             IEEE802154_EVENT_TX_SFD_DONE);
...
static bool stop_cca(void) {
  ieee802154_set_cmd(IEEE802154_CMD_STOP);
  ieee802154_ll_clear_events(IEEE802154_EVENT_ED_DONE |
                             IEEE802154_EVENT_RX_ABORT);
  return true;
}
...
static IRAM_ATTR void next_operation(void) {
  ...
  if (s_pending_tx.frame) {
    // Here the driver needs to recover the setting of rx aborts, see function
    // `ieee802154_transmit`.
    ieee802154_ll_enable_rx_abort_events(
        BIT(IEEE802154_RX_ABORT_BY_TX_ACK_TIMEOUT - 1) |
        BIT(IEEE802154_RX_ABORT_BY_TX_ACK_COEX_BREAK - 1));
...
static IRAM_ATTR void isr_handle_tx_done(void) {
  ...
  if (s_ieee802154_state == IEEE802154_STATE_TX_ENH_ACK) {
    ...
    if (s_ieee802154_state == IEEE802154_STATE_TEST_TX) {
      ieee802154_transmit_done(s_tx_frame, NULL, NULL);
      next_operation();
    } else if (s_ieee802154_state == IEEE802154_STATE_TX ||
               s_ieee802154_state == IEEE802154_STATE_TX_CCA) {
      if (ieee802154_frame_is_ack_required(s_tx_frame) &&
          ieee802154_ll_get_rx_auto_ack()) {
        ieee802154_set_state(IEEE802154_STATE_RX_ACK);
#if !CONFIG_IEEE802154_TEST
        receive_ack_timeout_timer_start(
            200000);  // 200ms for receive ack timeout
#endif
      } else {
        ieee802154_transmit_done(s_tx_frame, NULL, NULL);
        next_operation();
...
static IRAM_ATTR void isr_handle_rx_done(void) {
  ...
  if (s_ieee802154_state == IEEE802154_STATE_RX) {
    if (ieee802154_frame_is_ack_required(s_rx_frame[s_rx_index]) &&
        ieee802154_frame_get_version(s_rx_frame[s_rx_index]) <=
            IEEE802154_FRAME_VERSION_1 &&
        ieee802154_ll_get_tx_auto_ack()) {
      // auto tx ack only works for the frame with version 0b00 and 0b01
      s_rx_frame_info[s_rx_index].pending =
          ieee802154_ack_config_pending_bit(s_rx_frame[s_rx_index]);
      ieee802154_set_state(IEEE802154_STATE_TX_ACK);
    } else if (ieee802154_frame_is_ack_required(s_rx_frame[s_rx_index]) &&
               ieee802154_frame_get_version(s_rx_frame[s_rx_index]) ==
                   IEEE802154_FRAME_VERSION_2 &&
               ieee802154_ll_get_tx_enhance_ack()) {
      s_rx_frame_info[s_rx_index].pending =
          ieee802154_ack_config_pending_bit(s_rx_frame[s_rx_index]);
      // For 2015 enh-ack, SW should generate an enh-ack then send it manually
      if (esp_ieee802154_enh_ack_generator(s_rx_frame[s_rx_index],
                                           &s_rx_frame_info[s_rx_index],
                                           s_enh_ack_frame) == ESP_OK) {
#if !CONFIG_IEEE802154_TEST
...
static IRAM_ATTR void isr_handle_rx_abort(void) {
  ...
  ieee802154_ll_rx_abort_reason_t rx_abort_reason =
  ...
  switch (rx_abort_reason) {
    ...
    case IEEE802154_RX_ABORT_BY_ED_COEX_REJECT:
      IEEE802154_ASSERT(s_ieee802154_state == IEEE802154_STATE_ED ||
    ...
    case IEEE802154_RX_ABORT_BY_TX_ACK_COEX_BREAK:
      IEEE802154_ASSERT(s_ieee802154_state == IEEE802154_STATE_TX_ACK ||
...
static IRAM_ATTR void isr_handle_tx_abort(void) {
  event_end_process();
  ieee802154_ll_tx_abort_reason_t tx_abort_reason =
...
static void ieee802154_isr(void* arg) {
  ...
  if (events & IEEE802154_EVENT_RX_SFD_DONE) {
    // IEEE802154_STATE_TX && IEEE802154_STATE_TX_CCA &&
    // IEEE802154_STATE_TX_ENH_ACK for isr processing delay
    IEEE802154_ASSERT(s_ieee802154_state == IEEE802154_STATE_RX ||
  ...
  if (events & IEEE802154_EVENT_TIMER0_OVERFLOW) {
    ...
    IEEE802154_ASSERT(ieee802154_timer0_test ||
                      s_ieee802154_state == IEEE802154_STATE_RX_ACK);
#endif
...
esp_err_t ieee802154_mac_init(void) {
  ...
  modem_clock_module_mac_reset(
      PERIPH_IEEE802154_MODULE);  // reset ieee802154 MAC
  ieee802154_pib_init();
...
#if !CONFIG_IEEE802154_TEST
  ieee802154_ll_disable_events((IEEE802154_EVENT_TIMER0_OVERFLOW) |
                               (IEEE802154_EVENT_TIMER1_OVERFLOW));
#endif
  ieee802154_ll_enable_tx_abort_events(
      BIT(IEEE802154_TX_ABORT_BY_RX_ACK_TIMEOUT - 1) |
      BIT(IEEE802154_TX_ABORT_BY_TX_COEX_BREAK - 1) |
      BIT(IEEE802154_TX_ABORT_BY_TX_SECURITY_ERROR - 1) |
      BIT(IEEE802154_TX_ABORT_BY_CCA_FAILED - 1) |
      BIT(IEEE802154_TX_ABORT_BY_CCA_BUSY - 1));
  ieee802154_ll_enable_rx_abort_events(
      BIT(IEEE802154_RX_ABORT_BY_TX_ACK_TIMEOUT - 1) |
      BIT(IEEE802154_RX_ABORT_BY_TX_ACK_COEX_BREAK - 1));

  ieee802154_ll_set_ed_sample_mode(IEEE802154_ED_SAMPLE_AVG);
#if !CONFIG_IEEE802154_TEST && CONFIG_ESP_COEX_SW_COEXIST_ENABLE || \
  ...
  ret = esp_intr_alloc(ieee802154_periph.irq_id, 0, ieee802154_isr, NULL,
                       &s_ieee802154_isr_handle);
  ESP_RETURN_ON_FALSE(ret == ESP_OK, ESP_FAIL, IEEE802154_TAG,
                      "IEEE802154 MAC init failed");

  ESP_RETURN_ON_FALSE(ieee802154_sleep_init() == ESP_OK, ESP_FAIL,
                      IEEE802154_TAG, "IEEE802154 MAC sleep init failed");
...
static inline esp_err_t ieee802154_transmit_internal(const uint8_t* frame,
                                                     bool cca) {
...
esp_err_t ieee802154_transmit(const uint8_t* frame, bool cca) {
  ...
  if ((s_ieee802154_state == IEEE802154_STATE_RX &&
      ...
      s_ieee802154_state == IEEE802154_STATE_TX_ACK ||
      s_ieee802154_state == IEEE802154_STATE_TX_ENH_ACK) {
    // If the current radio is processing an RX frame or sending an ACK, do not
    // shut down the ongoing process. Instead, defer the transmission of the
    // pending TX frame. Once the current process is completed, the pending
    // transmit frame will be initiated.
    s_pending_tx.frame = frame;
    s_pending_tx.cca = cca;
    IEEE802154_TX_DEFERRED_NUMS_UPDATE();
    // Here we enable all rx interrupts due to the driver needs to know when the
    // current RX has finished. Will recover the setting of rx abort in function
    // `next_operation`.
...
esp_err_t ieee802154_transmit_at(const uint8_t* frame,
  ...
  if (cca) {
    ...
    ieee802154_set_state(IEEE802154_STATE_TX_CCA);
    ieee802154_enter_critical();
    ieee802154_etm_set_event_task(IEEE802154_ETM_CHANNEL0,
                                  ETM_EVENT_TIMER0_OVERFLOW,
                                  ETM_TASK_ED_TRIG_TX);
    current_time = (uint32_t) esp_timer_get_time();
    ieee802154_timer0_set_threshold(
        (is_target_time_expired(tx_target_time, current_time)
             ? 0
             : (tx_target_time - current_time)));  // uint: 1us
    ieee802154_timer0_start();
    ieee802154_exit_critical();
  } else {
    ...
    ieee802154_enter_critical();
    ieee802154_etm_set_event_task(IEEE802154_ETM_CHANNEL0,
                                  ETM_EVENT_TIMER0_OVERFLOW, ETM_TASK_TX_START);
    current_time = (uint32_t) esp_timer_get_time();
    ieee802154_timer0_set_threshold(
        (is_target_time_expired(tx_target_time, current_time)
             ? 0
...
esp_err_t ieee802154_receive_at(uint32_t time) {
  ...
  ieee802154_set_state(IEEE802154_STATE_RX);
  ieee802154_enter_critical();
  ieee802154_etm_set_event_task(IEEE802154_ETM_CHANNEL1,
                                ETM_EVENT_TIMER0_OVERFLOW, ETM_TASK_RX_START);
  current_time = (uint32_t) esp_timer_get_time();
  ieee802154_timer0_set_threshold(
      (is_target_time_expired(rx_target_time, current_time)
           ? 0
           : (rx_target_time - current_time)));  // uint: 1us
  ieee802154_timer0_start();
  ieee802154_exit_critical();
  return ESP_OK;
}
...
static esp_err_t ieee802154_sleep_retention_init(void* arg) {
  #define N_REGS_IEEE802154() \
    (((IEEE802154_MAC_DATE_REG - IEEE802154_REG_BASE) / 4) + 1)
  const static sleep_retention_entries_config_t
      ieee802154_mac_regs_retention[] = {
          [0] = {.config = REGDMA_LINK_CONTINUOUS_INIT(
                     REGDMA_MODEM_IEEE802154_LINK(0x00), IEEE802154_REG_BASE,
                     IEEE802154_REG_BASE, N_REGS_IEEE802154(), 0, 0),
                 .owner = IEEE802154_LINK_OWNER},
      };
  esp_err_t err = sleep_retention_entries_create(
      ieee802154_mac_regs_retention, ARRAY_SIZE(ieee802154_mac_regs_retention),
      REGDMA_LINK_PRI_IEEE802154, SLEEP_RETENTION_MODULE_802154_MAC);
  ESP_RETURN_ON_ERROR(err, IEEE802154_TAG,
                      "failed to allocate memory for ieee802154 mac retention");
...
static esp_err_t ieee802154_sleep_init(void) {
  ...
  sleep_retention_module_init_param_t init_param = {
      .cbs = {.create = {.handle = ieee802154_sleep_retention_init,
                         .arg = NULL}},
  ...
  ESP_RETURN_ON_ERROR(err, IEEE802154_TAG,
                      "failed to create sleep retention linked list for "
                      "ieee802154 mac retention");
  #if SOC_PM_RETENTION_HAS_CLOCK_BUG && CONFIG_MAC_BB_PD
  sleep_modem_register_mac_bb_module_prepare_callback(
      sleep_modem_mac_bb_power_down_prepare,
...
static esp_err_t ieee802154_sleep_deinit(void) {
  ...
  #if SOC_PM_RETENTION_HAS_CLOCK_BUG && CONFIG_MAC_BB_PD
  sleep_modem_unregister_mac_bb_module_prepare_callback(
      sleep_modem_mac_bb_power_down_prepare,
      sleep_modem_mac_bb_power_up_prepare);
  #endif  // SOC_PM_RETENTION_HAS_CLOCK_BUG && CONFIG_MAC_BB_PD
#endif  // SOC_PM_MODEM_RETENTION_BY_REGDMA && CONFIG_FREERTOS_USE_TICKLESS_IDLE
