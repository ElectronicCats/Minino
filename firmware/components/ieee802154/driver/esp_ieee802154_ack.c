...
static IRAM_ATTR bool ieee802154_addr_in_pending_table(const uint8_t* addr,
                                                       bool is_short) {
  bool ret = false;
  if (is_short) {
    for (uint8_t index = 0; index < CONFIG_IEEE802154_PENDING_TABLE_SIZE;
         index++) {
      if (BIT_IST(ieee802154_pending_table.short_addr_mask, index) &&
          memcmp(addr, ieee802154_pending_table.short_addr[index],
                 IEEE802154_FRAME_SHORT_ADDR_SIZE) == 0) {
        ret = true;
        break;
      }
    }
    ...
    for (uint8_t index = 0; index < CONFIG_IEEE802154_PENDING_TABLE_SIZE;
      ...
      if (BIT_IST(ieee802154_pending_table.ext_addr_mask, index) &&
          memcmp(addr, ieee802154_pending_table.ext_addr[index],
                 IEEE802154_FRAME_EXT_ADDR_SIZE) == 0) {
        ret = true;
        break;
      }
    }
  ...
  if (is_short) {
    for (uint8_t index = 0; index < CONFIG_IEEE802154_PENDING_TABLE_SIZE;
      ...
      if (!BIT_IST(ieee802154_pending_table.short_addr_mask, index)) {
        ...
        first_empty_index =
            (first_empty_index == -1 ? index : first_empty_index);
      } else if (memcmp(addr, ieee802154_pending_table.short_addr[index],
                        IEEE802154_FRAME_SHORT_ADDR_SIZE) == 0) {
    ...
    if (first_empty_index != -1) {
      memcpy(ieee802154_pending_table.short_addr[first_empty_index], addr,
    ...
    for (uint8_t index = 0; index < CONFIG_IEEE802154_PENDING_TABLE_SIZE;
      ...
      if (!BIT_IST(ieee802154_pending_table.ext_addr_mask, index)) {
        first_empty_index =
            (first_empty_index == -1 ? index : first_empty_index);
      } else if (memcmp(addr, ieee802154_pending_table.ext_addr[index],
                        IEEE802154_FRAME_EXT_ADDR_SIZE) == 0) {
    ...
    if (first_empty_index != -1) {
      memcpy(ieee802154_pending_table.ext_addr[first_empty_index], addr,
...
esp_err_t ieee802154_clear_pending_addr(const uint8_t* addr, bool is_short) {
  esp_err_t ret = ESP_FAIL;
  // Consider this function may be called in ISR, only clear the mask bits for
...
bool ieee802154_ack_config_pending_bit(const uint8_t* frame) {
  ...
  switch (pending_mode) {
    ...
    case IEEE802154_AUTO_PENDING_ENHANCED:
      ...
      if (src_mode == IEEE802154_FRAME_SRC_MODE_SHORT ||
          src_mode == IEEE802154_FRAME_SRC_MODE_EXT) {
        if (ieee802154_addr_in_pending_table(
                addr, src_mode == IEEE802154_FRAME_SRC_MODE_SHORT)) {
          pending_bit = true;
        }
      }
      break;
    case IEEE802154_AUTO_PENDING_ZIGBEE:
      // If the address type is short and in pending table, set 'pending_bit'
      // false, otherwise set true.
      src_mode = ieee802154_frame_get_src_addr(frame, addr);
      pending_bit = true;
      if (src_mode == IEEE802154_FRAME_SRC_MODE_SHORT &&
