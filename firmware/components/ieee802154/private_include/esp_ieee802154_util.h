...
#define IEEE802154_PROBE(a)       \
  ...
  #define IEEE802154_ASSERT_RECORD_EVENT_SIZE \
  ...
  #define IEEE802154_RECORD_EVENT(a)                                         \
    do {                                                                     \
      ...
      g_ieee802154_probe.event[g_ieee802154_probe.event_index].state =       \
          ieee802154_get_state();                                            \
      if (a == IEEE802154_EVENT_RX_ABORT) {                                  \
        g_ieee802154_probe.event[g_ieee802154_probe.event_index]             \
            .abort_reason.rx = ieee802154_ll_get_rx_abort_reason();          \
      } else if (a == IEEE802154_EVENT_TX_ABORT) {                           \
        g_ieee802154_probe.event[g_ieee802154_probe.event_index]             \
            .abort_reason.tx = ieee802154_ll_get_tx_abort_reason();          \
      }                                                                      \
      g_ieee802154_probe.event[g_ieee802154_probe.event_index++].timestamp = \
          esp_timer_get_time();                                              \
      g_ieee802154_probe.event_index = (g_ieee802154_probe.event_index ==    \
                                        IEEE802154_ASSERT_RECORD_EVENT_SIZE) \
                                           ? 0                               \
...
typedef struct {
  ...
  #define IEEE802154_ASSERT_RECORD_STATE_SIZE \
    CONFIG_IEEE802154_RECORD_STATE_SIZE
  #define ieee802154_set_state(a)                                            \
    do {                                                                     \
      s_ieee802154_state = a;                                                \
      sprintf(                                                               \
          g_ieee802154_probe.state[g_ieee802154_probe.state_index].line_str, \
          "%d", __LINE__);                                                   \
      g_ieee802154_probe.state[g_ieee802154_probe.state_index].timestamp =   \
          esp_timer_get_time();                                              \
      g_ieee802154_probe.state[g_ieee802154_probe.state_index++].state = a;  \
      g_ieee802154_probe.state_index = (g_ieee802154_probe.state_index ==    \
                                        IEEE802154_ASSERT_RECORD_STATE_SIZE) \
                                           ? 0                               \
 ...
 */
typedef struct {
  char line_str[5]; /*!< record which line in esp_ieee802154_dev.c changes the
                       state */
  ieee802154_state_t state; /*!< record current radio state */
  uint64_t timestamp;       /*!< record timestamp */
  ...
  #define ieee802154_set_cmd(a)                                               \
    do {                                                                      \
      ieee802154_ll_set_cmd(a);                                               \
      sprintf(g_ieee802154_probe.cmd[g_ieee802154_probe.cmd_index].line_str,  \
              "%d", __LINE__);                                                \
      g_ieee802154_probe.cmd[g_ieee802154_probe.cmd_index].timestamp =        \
          esp_timer_get_time();                                               \
      g_ieee802154_probe.cmd[g_ieee802154_probe.cmd_index++].cmd = a;         \
      g_ieee802154_probe.cmd_index =                                          \
          (g_ieee802154_probe.cmd_index == IEEE802154_ASSERT_RECORD_CMD_SIZE) \
              ? 0                                                             \
              : g_ieee802154_probe.cmd_index;                                 \
    } while (0)
 ...
 */
typedef struct {
  char line_str[5]; /*!< record which line in esp_ieee802154_dev.c set the
                       command */
  ...
  #define IEEE802154_ASSERT_RECORD_ABORT_SIZE \
    CONFIG_IEEE802154_RECORD_ABORT_SIZE
  #define ieee802154_record_abort(a)                                           \
    do {                                                                       \
      if (a == IEEE802154_EVENT_RX_ABORT) {                                    \
        g_ieee802154_probe.abort[g_ieee802154_probe.abort_index]               \
            .abort_reason.rx = ieee802154_ll_get_rx_abort_reason();            \
        g_ieee802154_probe.abort[g_ieee802154_probe.abort_index].is_tx_abort = \
            0;                                                                 \
        g_ieee802154_probe.abort[g_ieee802154_probe.abort_index++].timestamp = \
            esp_timer_get_time();                                              \
        g_ieee802154_probe.abort_index = (g_ieee802154_probe.abort_index ==    \
                                          IEEE802154_ASSERT_RECORD_ABORT_SIZE) \
                                             ? 0                               \
...
#if CONFIG_IEEE802154_RECORD_EVENT
  ieee802154_event_info_t
      event[IEEE802154_ASSERT_RECORD_EVENT_SIZE]; /*!< record radio event */
  uint8_t event_index;                            /*!< the index of event */
#endif  // CONFIG_IEEE802154_RECORD_EVENT
#if CONFIG_IEEE802154_RECORD_STATE
  ieee802154_state_info_t
      state[IEEE802154_ASSERT_RECORD_STATE_SIZE]; /*!< record radio state */
  uint8_t state_index;                            /*!< the index of state */
#endif  // CONFIG_IEEE802154_RECORD_STATE
#if CONFIG_IEEE802154_RECORD_CMD
  ieee802154_cmd_info_t
      cmd[IEEE802154_ASSERT_RECORD_CMD_SIZE]; /*!< record radio command */
  uint8_t cmd_index;                          /*!< the index of command */
#endif                                        // CONFIG_IEEE802154_RECORD_CMD
#if CONFIG_IEEE802154_RECORD_ABORT
  ieee802154_abort_info_t
      abort[IEEE802154_ASSERT_RECORD_ABORT_SIZE]; /*!< record radio abort */
...
/**
 * @brief  Set the IEEE802154 radio coexistence scene during transmitting or
 * receiving.
 *
...
void ieee802154_set_txrx_pti(ieee802154_txrx_scene_t txrx_scene);
  ...
  #define IEEE802154_SET_TXRX_PTI(txrx_scene) \
    ieee802154_set_txrx_pti(txrx_scene)

#else

  #define IEEE802154_SET_TXRX_PTI(txrx_scene)

#endif  // !CONFIG_IEEE802154_TEST && CONFIG_ESP_COEX_SW_COEXIST_ENABLE ||
        // CONFIG_EXTERNAL_COEX_ENABLE
...
/**
 * @brief  Configure the ETM module, using [channel] for monitoring the event,
 * when event appears hardware will operate the [task].
 *
 ...
 *
 */
void ieee802154_etm_set_event_task(uint32_t channel,
                                   uint32_t event,
                                   uint32_t task);
