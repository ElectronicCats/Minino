...
#define ESP_ZB_CLI_CMD_OP_FEILDS(_op) .operation = _op,
...
#define ESP_ZB_CLI_CMD_WITH_SUB(_name, _sub, _help) \
  ESP_ZB_CLI_CMD(_name, , _subcmd_list_name(_sub), _help)
#define ESP_ZB_CLI_CMD_WITH_OP(_name, _op, _help) \
  ESP_ZB_CLI_CMD(_name, _op, , _help)
 ...
 * @param ...     Sub-command definations (should be valid initiallizer of
 * `esp_zb_cli_cmd_t`).
 *
 */
#define DECLARE_ESP_ZB_CLI_SUBCMD_LIST(_parent, ...) \
  static esp_zb_cli_cmd_t _subcmd_list_name(_parent)[] = {__VA_ARGS__}

/**
 * @brief Declare a cli command.
 *
 * @note The command structure will be placed in
 * `.esp_zb_cli_cmd_desc.<cmd_name>` section.
 *
...
#define DECLARE_ESP_ZB_CLI_CMD(_name, _op, _sub, _help) \
  static const esp_zb_cli_cmd_t _cmd_var_name(_name)    \
      ESP_ZB_CLI_CMD_DESC(_name) = ESP_ZB_CLI_CMD(_name, _op, _sub, _help)
