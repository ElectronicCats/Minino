...
static esp_err_t cli_role(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  ...
  EXIT_ON_FALSE(argtable.help->count == 0, ESP_OK,
                arg_print_help((void**) &argtable, argv[0]));
  EXIT_ON_FALSE(nerrors == 0, ESP_ERR_INVALID_ARG,
                arg_print_errors(stdout, argtable.end, argv[0]));

  if (argtable.role->count > 0) {
    for (esp_zb_nwk_device_type_t role = ESP_ZB_DEVICE_TYPE_COORDINATOR;
         role < ESP_ZB_DEVICE_TYPE_NONE; role++) {
...
static esp_err_t cli_panid(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .address =
          arg_addrn(NULL, NULL, "<addr:ADDR>", 0, 1, "(extended) PAN ID"),
      .help = arg_lit0(NULL, "help", "Print this help message"),
      .end = arg_end(2),
  };
  esp_err_t ret = ESP_OK;
  ...
  if (argtable.extended->count > 0) {
    if (argtable.address->count > 0) {
      EXIT_ON_FALSE(
          argtable.address->addr[0].addr_type == ESP_ZB_ZCL_ADDR_TYPE_IEEE,
          ESP_ERR_INVALID_ARG);
    ...
    if (argtable.address->count > 0) {
      EXIT_ON_FALSE(
          argtable.address->addr[0].addr_type == ESP_ZB_ZCL_ADDR_TYPE_SHORT,
          ESP_ERR_INVALID_ARG);
...
static esp_err_t cli_address(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .address =
          arg_addrn(NULL, NULL, "<addr:ADDR>", 0, 1, "(extended) address"),
      .help = arg_lit0(NULL, "help", "Print this help message"),
      .end = arg_end(2),
  };
  esp_err_t ret = ESP_OK;
  ...
  if (argtable.extended->count > 0) {
    if (argtable.address->count > 0) {
      EXIT_ON_FALSE(
          ...
          ESP_ERR_INVALID_ARG);
      EXIT_ON_ERROR(
          esp_zb_set_long_address(argtable.address->addr[0].u.ieee_addr));
    } else {
      esp_zb_ieee_addr_t ext_pan_id;
    ...
    if (argtable.address->count > 0) {
      EXIT_ON_FALSE(0, ESP_ERR_NOT_SUPPORTED,
                    cli_output_line("set short address is not supported"));
    } else {
...
static esp_err_t cli_channel(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .channel =
          arg_u32n(NULL, NULL, "<u32:CHANNEL>", 0, 1, "allowed channel (mask)"),
      .help = arg_lit0(NULL, "help", "Print this help message"),
      .end = arg_end(2),
  };
  esp_err_t ret = ESP_OK;
...
static esp_err_t cli_nwk_type(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .role =
          arg_str0(NULL, NULL, "<sc:d|c>", "set the network type. Default: c"),
      .help = arg_lit0(NULL, "help", "Print this help message"),
      .end = arg_end(2),
  };
  esp_err_t ret = ESP_OK;
  ...
  if (argtable.role->count > 0) {
    switch (argtable.role->sval[0][0]) {
      ...
      default:
        EXIT_ON_ERROR(
            ESP_ERR_INVALID_ARG,
            cli_output("unknown network type: %s\n", argtable.role->sval[0]));
        break;
    }
  } else {
...
static esp_err_t cli_nwk_key(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .key = arg_hexn(NULL, NULL, "<key128:KEY>", 0, 1,
                      "network key, in HEX format"),
      .help = arg_lit0(NULL, "help", "Print this help message"),
      .end = arg_end(2),
  };
  esp_err_t ret = ESP_OK;
  ...
  if (argtable.key->count > 0) {
    ...
    EXIT_ON_ERROR(esp_zb_secur_primary_network_key_get(key),
                  cli_output_line("Fail to get network key."));
...
static esp_err_t cli_nwk_childmax(esp_zb_cli_cmd_t* self,
                                  int argc,
                                  char** argv) {
  struct {
    arg_u8_t* childmax;
    ...
    arg_end_t* end;
      ...
      .childmax =
          arg_u8n(NULL, NULL, "<u8:CHILD_NUM>", 0, 1, "max children number"),
      .help = arg_lit0(NULL, "help", "Print this help message"),
      .end = arg_end(2),
  };
  esp_err_t ret = ESP_OK;
...
static esp_err_t cli_nwk_open(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .timeout = arg_u8n("t", NULL, "<u8:TIMEOUT>", 1, 1,
                         "timeouts in second for opening"),
      .end = arg_end(2),
  };
  esp_err_t ret = ESP_OK;
  ...
  EXIT_ON_ERROR(esp_zb_bdb_open_network(argtable.timeout->val[0]),
                cli_output_line("Fail to open network."));
...
static esp_err_t cli_nwk_close(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  ...
  EXIT_ON_ERROR(esp_zb_bdb_close_network(),
                cli_output_line("Fail to close network."));

exit:
  return ret;
}

static void cli_output_active_scan_results(
  ...
  for (int i = 0; i < count; i++) {
    cli_output("| %2d | 0x%04hx | 0x%016" PRIx64 " | %5s | %c%c|\n",
               nwk_descriptor[i].logic_channel, nwk_descriptor[i].short_pan_id,
...
static void cli_nwk_scan_cb(esp_zb_zdp_status_t status,
                            uint8_t count,
                            esp_zb_network_descriptor_t* nwk_descriptor) {
  esp_err_t ret = ESP_OK;
...
static esp_err_t cli_nwk_scan(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .mask = arg_lit0("m", NULL, "use channel mask"),
      .channel = arg_u32n(NULL, NULL, "<u32:CHANNEL>", 1, 1,
                          "channel (mask) to perform scan on"),
      .duration = arg_u8n("t", NULL, "<u8:TIMEOUT>", 0, 1,
                          "timeout in sec of scan, default: 1s"),
      .end = arg_end(2),
  };
...
static void cli_output_ed_scan_results(
  ...
  cli_output_table_header(ARRAY_SIZE(widths), titles, widths);
  for (int i = 0; i < count; i++) {
    cli_output("| %2d | %4d |\n", channel_info[i].channel_number,
               channel_info[i].energy_detected);
  }
}

static void cli_nwk_ed_scan_cb(
...
static esp_err_t cli_nwk_ed_scan(esp_zb_cli_cmd_t* self,
                                 int argc,
                                 char** argv) {
  struct {
    arg_lit_t* mask;
...
static esp_err_t cli_ic_add(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .add_addr =
          arg_addrn(NULL, NULL, "<EUI64>", 1, 1, "IEEE address of the device"),
  ...
  EXIT_ON_FALSE(esp_zb_is_started(), ESP_ERR_INVALID_SIZE,
                cli_output_line("Please start Zigbee stack first"));
  EXIT_ON_FALSE(argtable.add_addr->addr->addr_type == ESP_ZB_ZCL_ADDR_TYPE_IEEE,
                ESP_ERR_INVALID_ARG,
                cli_output_line("IEEE address is required."));

  switch (argtable.ic_code->hsize[0]) {
    case 8:
      ic_type = ESP_ZB_IC_TYPE_48;
      break;
    case 10:
      ic_type = ESP_ZB_IC_TYPE_64;
      break;
    case 14:
      ic_type = ESP_ZB_IC_TYPE_96;
      break;
    case 18:
      ic_type = ESP_ZB_IC_TYPE_128;
      break;
    default:
      EXIT_NOW(ret = ESP_ERR_INVALID_ARG;
               cli_output("Install code incorrect length %d\n",
                          argtable.ic_code->hsize[0]));
      break;
  }

  EXIT_ON_ERROR(esp_zb_secur_ic_add(argtable.add_addr->addr[0].u.ieee_addr,
...
static esp_err_t cli_ic_remove(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .add_addr = arg_addrn(NULL, NULL, "<eui64:EUI64>", 1, 1,
  ...
  EXIT_ON_FALSE(argtable.add_addr->addr->addr_type == ESP_ZB_ZCL_ADDR_TYPE_IEEE,
                ...
                cli_output_line("IEEE address is required."));
  EXIT_ON_ERROR(
      esp_zb_secur_ic_remove_req(argtable.add_addr->addr->u.ieee_addr),
...
static esp_err_t cli_ic_set(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  ...
  EXIT_ON_ERROR(esp_zb_secur_ic_set(ic_type, argtable.ic_code->hval[0]),
                cli_output_line("Fail to set install code\n"));
...
static esp_err_t cli_bdb_channel(esp_zb_cli_cmd_t* self,
                                 int argc,
                                 char** argv) {
  struct {
    arg_lit_t* mask;
    ...
    arg_end_t* end;
      ...
      .mask = arg_lit0("m", NULL, "use channel mask"),
      .primary_chn =
          arg_u32n(NULL, NULL, "<u32:PRIMARY>", 0, 1, "primary channel (mask)"),
      .secondary_chn = arg_u32n(NULL, NULL, "<u32:SECONDARY>", 0, 1,
  ...
  if (argc > 1) {
    ...
    EXIT_ON_ERROR(
        esp_zb_set_secondary_network_channel_set(secondary_channel_mask));
  } else {
...
static esp_err_t cli_bdb_start(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  ...
  esp_zb_bdb_commissioning_mode_t bdb_comm_mode =
    ...
    for (int i = 0; i < argtable.mode->count; i++) {
      if (!strcmp(argtable.mode->sval[i], "steer")) {
        ...
        cli_output("Skip %s bdb commissioning mode: %s\n", "unknown",
                   argtable.mode->sval[i]);
      }
    }
  }

  if (!esp_zb_is_started()) {
    EXIT_ON_ERROR(esp_zb_start(false),
                  cli_output_line("Fail to start Zigbee stack."));
  }
...
static esp_err_t cli_bdb_cancel(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .mode = arg_strn(NULL, NULL, "<steer|form|target>", 1, 1,
                       "commissioning mode to cancel"),
      .end = arg_end(2),
  };
  esp_err_t ret = ESP_OK;
...
static esp_err_t cli_tl_timeout(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .timeout = arg_u32n(NULL, NULL, "<u32:TIMEOUT>", 0, 1,
                          "touchlink target timeout, in second"),
      .help = arg_lit0(NULL, "help", "Print this help message"),
      .end = arg_end(2),
  };
  esp_err_t ret = ESP_OK;
  ...
  if (argtable.timeout->count > 0) {
    ...
    EXIT_ON_ERROR(ESP_ERR_NOT_SUPPORTED,
                  cli_output_line("Get value is not supported."));
  }
...
static esp_err_t cli_tl_rssi(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .rssi = arg_intn(NULL, NULL, "<int:RSSI>", 0, 1,
                       "touchlink target rssi threshold"),
      .help = arg_lit0(NULL, "help", "Print this help message"),
      .end = arg_end(2),
  };
  esp_err_t ret = ESP_OK;
...
static esp_err_t cli_tl_key(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .key = arg_hexn(NULL, NULL, "<hex:KEY128>", 0, 1,
                      "network key, in HEX format"),
      .help = arg_lit0(NULL, "help", "Print this help message"),
      .end = arg_end(2),
  };
...
DECLARE_ESP_ZB_CLI_CMD(panid,
                       cli_panid,
                       ,
                       "Get/Set the (extended) PAN ID of the node");
DECLARE_ESP_ZB_CLI_CMD(address,
                       cli_address,
                       ,
                       "Get/Set the (extended) address of the node");
DECLARE_ESP_ZB_CLI_CMD(channel,
                       cli_channel,
                       ,
...
DECLARE_ESP_ZB_CLI_CMD_WITH_SUB(
    ...
    ESP_ZB_CLI_SUBCMD(legacy,
                      cli_nwk_legacy,
                      "Enable/Disable legacy device support"),
    ESP_ZB_CLI_SUBCMD(childmax,
                      cli_nwk_childmax,
    ...
    ESP_ZB_CLI_SUBCMD(ed_scan,
                      cli_nwk_ed_scan,
                      "Scan for energy detect on channels"), );
DECLARE_ESP_ZB_CLI_CMD_WITH_SUB(
    ic,
    "Install code configuration",
    ESP_ZB_CLI_SUBCMD(add, cli_ic_add, "Add install code for a device"),
    ESP_ZB_CLI_SUBCMD(remove,
                      cli_ic_remove,
                      "Remove install code for a device"),
...
DECLARE_ESP_ZB_CLI_CMD_WITH_SUB(
    ...
    ESP_ZB_CLI_SUBCMD(channel,
                      cli_bdb_channel,
                      "Get/Set channels to perform commissioning"),
    ...
    ESP_ZB_CLI_SUBCMD(cancel,
                      cli_bdb_cancel,
                      "Cancel commissioning process"), );
DECLARE_ESP_ZB_CLI_CMD_WITH_SUB(
    tl,
    "TouchLink configuration",
    ESP_ZB_CLI_SUBCMD(timeout,
                      cli_tl_timeout,
                      "Get/Set touchlink target timeout"),
    ESP_ZB_CLI_SUBCMD(rssi,
                      cli_tl_rssi,
