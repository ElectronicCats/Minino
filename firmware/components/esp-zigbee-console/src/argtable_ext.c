...
static esp_err_t arg_common_checkfn(struct arg_lit* parent) {
  esp_err_t errorcode =
      (parent->count < parent->hdr.mincount) ? ESP_ERR_NOT_FOUND : ESP_OK;
  return errorcode;
}

static void arg_common_errorfn(struct arg_lit* parent,
                               arg_dstr_t ds,
                               int errorcode,
                               const char* argval,
                               const char* progname) {
...
arg_u8_t* arg_u8n(const char* shortopts,
                  const char* longopts,
                  const char* datatype,
                  int mincount,
                  int maxcount,
...
arg_u16_t* arg_u16n(const char* shortopts,
                    const char* longopts,
                    const char* datatype,
                    int mincount,
                    int maxcount,
...
arg_u32_t* arg_u32n(const char* shortopts,
                    const char* longopts,
                    const char* datatype,
                    int mincount,
                    int maxcount,
...
arg_hex_t* arg_hexn(const char* shortopts,
                    const char* longopts,
                    const char* datatype,
                    int mincount,
                    int maxcount,
  ...
  nbytes = sizeof(arg_hex_t) +
           (size_t) maxcount * (sizeof(uint16_t) + sizeof(uint8_t*));
...
arg_devid_t* arg_devidn(const char* shortopts,
                        const char* longopts,
                        const char* datatype,
                        int mincount,
                        int maxcount,
                        const char* glossary) {
  arg_devid_t* result = (arg_devid_t*) arg_u16n(shortopts, longopts, datatype,
                                                mincount, maxcount, glossary);
...
arg_addr_t* arg_addrn(const char* shortopts,
                      const char* longopts,
                      const char* datatype,
                      int mincount,
                      int maxcount,
