...
static esp_err_t zha_add_device(esp_zb_ep_list_t* ep_list,
                                cli_zha_device_params_t* device_params) {
  esp_err_t ret = ESP_OK;
  EXIT_ON_FALSE(device_params->device_name != NULL, ESP_OK);
  EXIT_ON_FALSE(ep_list, ESP_ERR_NOT_SUPPORTED,
                cli_output_line("Data model has been registered"));
  esp_zb_cluster_list_t* cluster_list =
      esp_zb_ep_create_ha_default(device_params->device_id);

  EXIT_ON_FALSE(
      cluster_list, ESP_ERR_NOT_SUPPORTED,
  ...
  ESP_LOGI(TAG, "%s created with endpoint_id %d", device_params->device_name,
           device_params->ep_id);
...
static esp_err_t cli_zha_add_parse_params(int argc,
                                          char* argv[],
                                          cli_zha_device_params_t* dev_param) {
  struct {
    arg_int_t* ep_id;
    arg_str_t* device;
    arg_end_t* end;
  } argtable = {
      .ep_id = arg_int1(NULL, NULL, "<u8:EID>",
                        "set the endpoint id of the added device"),
      .device =
          arg_str1(NULL, NULL, "<DEVICE>", "HA standard device to be added"),
      .end = arg_end(2),
  };
  ...
  EXIT_ON_FALSE(
      (0 < ep_id && ep_id < 255), ESP_ERR_INVALID_ARG,
      cli_output("<endpoint_id> should in range 1 - 254, got %d\n", ep_id));
  EXIT_ON_FALSE(
      (device_id != 0xFFFF), ESP_ERR_INVALID_ARG,
