...
esp_err_t esp_openthread_get_dnsserver_addr(ip6_addr_t* dnsserver_addr) {
  ...
  ESP_RETURN_ON_FALSE(dnsserver_addr, ESP_ERR_INVALID_ARG, TAG,
                      "dnsserver_addr cannot be NULL");
...
esp_err_t esp_openthread_get_nat64_prefix(ip6_addr_t* nat64_prefix) {
  ...
  if (route.mNat64) {
    memcpy(nat64_prefix->addr, route.mPrefix.mPrefix.mFields.m8,
           sizeof(nat64_prefix->addr));
    return ESP_OK;
  } else {
...
int lwip_hook_netconn_external_resolve(const char* name,
                                       ip_addr_t* addr,
                                       u8_t addrtype,
                                       err_t* err) {
