...
static err_t openthread_output_ip6(struct netif* netif,
                                   struct pbuf* p,
                                   const struct ip6_addr* peer_addr) {
  struct pbuf* q = p;
  ...
  if (q->next == NULL) {
    ...
    ret = esp_netif_transmit(esp_netif, q->payload, q->len);
    /* content in payload has been copied to OpenThread queue, it's safe to free
     * pbuf now */
    pbuf_free(q);
  }
  /* Check error */
...
static void openthread_netif_input(void* h,
                                   void* buffer,
                                   size_t len,
                                   void* eb) {
  ...
  if (unlikely(otMessageRead(message, 0, p->payload, len) !=
               otMessageGetLength(message))) {
...
static err_t openthread_netif_multicast_handler(
    struct netif* netif,
  ...
  if (action == NETIF_ADD_MAC_FILTER) {
    error = otIp6SubscribeMulticastAddress(esp_openthread_get_instance(),
                                           &multicast_addr);
  } else {
...
const ip_addr_t* lwip_hook_ip6_select_source_address(struct netif* netif,
  ...
  // Currently, prefer the address with the same prefix of the destination
  // address. If no address found, return NULL for selection source address
