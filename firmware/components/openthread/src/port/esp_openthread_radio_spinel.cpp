...
static const esp_openthread_radio_config_t* s_esp_openthread_radio_config =
    NULL;

static void esp_openthread_radio_config_set(
    const esp_openthread_radio_config_t* config) {
  s_esp_openthread_radio_config = config;
}

static const esp_openthread_radio_config_t* esp_openthread_radio_config_get(
    void) {
...
esp_err_t esp_openthread_radio_init(
...
#if CONFIG_OPENTHREAD_RADIO_SPINEL_UART  // CONFIG_OPENTHREAD_RADIO_SPINEL_UART
  ESP_RETURN_ON_ERROR(s_spinel_interface.GetSpinelInterface().Enable(
                          config->radio_config.radio_uart_config),
                      OT_PLAT_LOG_TAG, "Spinel interface init falied");
#else  // CONFIG_OPENTHREAD_RADIO_SPINEL_SPI
  ESP_RETURN_ON_ERROR(s_spinel_interface.GetSpinelInterface().Enable(
                          config->radio_config.radio_spi_config),
                      OT_PLAT_LOG_TAG, "Spinel interface init failed");
#endif
  s_radio.Init(s_spinel_interface.GetSpinelInterface(), /*reset_radio=*/true,
  ...
  ESP_RETURN_ON_ERROR(s_spinel_interface.GetSpinelInterface().AfterRadioInit(),
                      OT_PLAT_LOG_TAG, "Spinel interface init falied");
#endif
...
void esp_openthread_register_rcp_failure_handler(
    esp_openthread_rcp_failure_handler handler) {
  s_spinel_interface.GetSpinelInterface().RegisterRcpFailureHandler(handler);
}

esp_err_t esp_openthread_rcp_deinit(void) {
  ESP_RETURN_ON_FALSE(otThreadGetDeviceRole(esp_openthread_get_instance()) ==
                          OT_DEVICE_ROLE_DISABLED,
                      ESP_ERR_INVALID_STATE, OT_PLAT_LOG_TAG,
                      "Thread is enabled, failed to deinitialize RCP");
  ESP_RETURN_ON_FALSE(!otIp6IsEnabled(esp_openthread_get_instance()),
                      ESP_ERR_INVALID_STATE, OT_PLAT_LOG_TAG,
                      "OT interface is up, failed to deinitialize RCP");
  if (s_radio.IsEnabled()) {
    ESP_RETURN_ON_FALSE(s_radio.Sleep() == OT_ERROR_NONE, ESP_ERR_INVALID_STATE,
                        OT_PLAT_LOG_TAG, "Radio fails to sleep");
    ESP_RETURN_ON_FALSE(s_radio.Disable() == OT_ERROR_NONE,
                        ESP_ERR_INVALID_STATE, OT_PLAT_LOG_TAG,
                        "Fail to disable radio");
  }
  ESP_RETURN_ON_FALSE(
      s_spinel_interface.GetSpinelInterface().Disable() == OT_ERROR_NONE,
      ESP_ERR_INVALID_STATE, OT_PLAT_LOG_TAG, "Fail to deinitialize UART");
  esp_openthread_platform_workflow_unregister(radiospinel_workflow);
  return ESP_OK;
}

esp_err_t esp_openthread_rcp_init(void) {
  const esp_openthread_radio_config_t* radio_config =
      esp_openthread_radio_config_get();
#if CONFIG_OPENTHREAD_RADIO_SPINEL_UART
  ESP_RETURN_ON_ERROR(s_spinel_interface.GetSpinelInterface().Enable(
                          radio_config->radio_uart_config),
                      OT_PLAT_LOG_TAG, "Spinel interface init falied");
#else   // CONFIG_OPENTHREAD_RADIO_SPINEL_SPI
  ESP_RETURN_ON_ERROR(s_spinel_interface.GetSpinelInterface().Enable(
                          radio_config->radio_spi_config),
                      OT_PLAT_LOG_TAG, "Spinel interface init failed");
#endif  // CONFIG_OPENTHREAD_RADIO_SPINEL_UART

  ESP_RETURN_ON_FALSE(
      s_radio.Enable(esp_openthread_get_instance()) == OT_ERROR_NONE, ESP_FAIL,
      OT_PLAT_LOG_TAG, "Fail to enable radio");
...
void otPlatRadioSetExtendedAddress(otInstance* instance,
...
otError otPlatRadioAddSrcMatchShortEntry(otInstance* instance,
                                         uint16_t short_address) {
  return s_radio.AddSrcMatchShortEntry(short_address);
}

otError otPlatRadioAddSrcMatchExtEntry(otInstance* instance,
...
otError otPlatRadioClearSrcMatchShortEntry(otInstance* instance,
                                           uint16_t short_address) {
  return s_radio.ClearSrcMatchShortEntry(short_address);
}

otError otPlatRadioClearSrcMatchExtEntry(otInstance* instance,
                                         const otExtAddress* ext_address) {
  otExtAddress addr;
...
otError otPlatRadioEnergyScan(otInstance* instance,
                              uint8_t channel,
                              uint16_t duration) {
...
otError otPlatRadioGetCcaEnergyDetectThreshold(otInstance* instance,
                                               int8_t* threshold) {
  otError error;
...
otError otPlatRadioSetCcaEnergyDetectThreshold(otInstance* instance,
                                               int8_t threshold) {
...
void otPlatRadioSetMacKey(otInstance* aInstance,
  ...
  SuccessOrDie(
      s_radio.SetMacKey(aKeyIdMode, aKeyId, aPrevKey, aCurrKey, aNextKey));
}
...
otError otPlatDiagProcess(otInstance* instance,
                          int argc,
                          char* argv[],
                          char* output,
...
void otPlatDiagRadioReceived(otInstance* instance,
                             otRadioFrame* frame,
                             otError error) {}
...
otError otPlatRadioConfigureEnhAckProbing(otInstance* aInstance,
  ...
  OT_UNUSED_VARIABLE(aInstance);
  return s_radio.ConfigureEnhAckProbing(aLinkMetrics, aShortAddress,
                                        *aExtAddress);
}
#endif
