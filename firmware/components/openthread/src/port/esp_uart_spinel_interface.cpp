...
otError UartSpinelInterface::Init(ReceiveFrameCallback aCallback,
...
esp_err_t UartSpinelInterface::Enable(
    const esp_openthread_uart_config_t& radio_uart_config) {
  esp_err_t error = ESP_OK;
  m_uart_rx_buffer =
      static_cast<uint8_t*>(heap_caps_malloc(kMaxFrameSize, MALLOC_CAP_8BIT));
...
otError UartSpinelInterface::SendFrame(const uint8_t* frame, uint16_t length) {
  ...
  SuccessOrExit(
      error = Write(encoder_buffer.GetFrame(), encoder_buffer.GetLength()));
...
otError UartSpinelInterface::WaitForWritable(void) {
  otError error = OT_ERROR_NONE;
  struct timeval timeout = {kMaxWaitTime / MS_PER_S,
                            (kMaxWaitTime % MS_PER_S) * US_PER_MS};
...
esp_err_t UartSpinelInterface::InitUart(
    const esp_openthread_uart_config_t& radio_uart_config) {
  char uart_path[16];
  ...
  // driver also.
  uart_vfs_dev_port_set_tx_line_endings(m_uart_config.port,
                                        ESP_LINE_ENDINGS_LF);
  uart_vfs_dev_port_set_rx_line_endings(m_uart_config.port,
                                        ESP_LINE_ENDINGS_LF);

  snprintf(uart_path, sizeof(uart_path), "/dev/uart/%d",
           radio_uart_config.port);
...
esp_err_t UartSpinelInterface::TryRecoverUart(void) {
  ...
  ESP_RETURN_ON_ERROR(InitUart(m_uart_config), OT_PLAT_LOG_TAG,
                      "InitUart failed");
  return ESP_OK;
}
