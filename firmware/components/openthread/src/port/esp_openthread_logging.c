 ...
 *
 */
#if (OPENTHREAD_CONFIG_LOG_OUTPUT ==                   \
     OPENTHREAD_CONFIG_LOG_OUTPUT_PLATFORM_DEFINED) || \
    (OPENTHREAD_CONFIG_LOG_OUTPUT == OPENTHREAD_CONFIG_LOG_OUTPUT_NCP_SPINEL)
OT_TOOL_WEAK void otPlatLog(otLogLevel log_level,
                            otLogRegion log_region,
                            const char* format,
                            ...) {
  va_list args;
  ...
  switch (log_level) {
    case OT_LOG_LEVEL_CRIT:
      if (LOG_LOCAL_LEVEL >= ESP_LOG_ERROR) {
        esp_log_write(ESP_LOG_ERROR, OT_PLAT_LOG_TAG,
    ...
    case OT_LOG_LEVEL_WARN:
      if (LOG_LOCAL_LEVEL >= ESP_LOG_WARN) {
        esp_log_write(ESP_LOG_WARN, OT_PLAT_LOG_TAG,
    ...
    case OT_LOG_LEVEL_INFO:
      if (LOG_LOCAL_LEVEL >= ESP_LOG_INFO) {
        esp_log_write(ESP_LOG_INFO, OT_PLAT_LOG_TAG,
    ...
    default:
      if (LOG_LOCAL_LEVEL >= ESP_LOG_DEBUG) {
        esp_log_write(ESP_LOG_DEBUG, OT_PLAT_LOG_TAG,
