...
esp_err_t UartSpinelInterface::Enable(
  ...
  if (error == ESP_OK) {
    ESP_LOGI(ESP_SPINEL_LOG_TAG,
             "spinel UART interface initialization completed");
  }
  return error;
}
...
void UartSpinelInterface::Process(const void* aMainloopContext) {
  if (FD_ISSET(m_uart_fd,
               &((esp_radio_spinel_mainloop_context_t*) aMainloopContext)
                    ->read_fds)) {
...
void UartSpinelInterface::HandleHdlcFrame(otError error) {
  if (error == OT_ERROR_NONE) {
    ...
    ESP_LOGE(ESP_SPINEL_LOG_TAG, "dropping radio frame: %s",
...
esp_err_t UartSpinelInterface::TryRecoverUart(void) {
  ...
  ESP_RETURN_ON_ERROR(InitUart(m_uart_config), ESP_SPINEL_LOG_TAG,
                      "InitUart failed");
  return ESP_OK;
}
...
void UartSpinelInterface::UpdateFdSet(void* aMainloopContext) {
  ...
  FD_SET(m_uart_fd,
         &((esp_radio_spinel_mainloop_context_t*) aMainloopContext)->read_fds);
  if (m_uart_fd >
      ((esp_radio_spinel_mainloop_context_t*) aMainloopContext)->max_fd) {
    ((esp_radio_spinel_mainloop_context_t*) aMainloopContext)->max_fd =
        m_uart_fd;
  }
}
