...
uint16_t otMessageRead(const otMessage *aMessage, uint16_t aOffset, void *aBuf, uint16_t aLength) { return aLength; }

int otMessageWrite(otMessage *aMessage, uint16_t aOffset, const void *aBuf, uint16_t aLength) { return aLength; }
...
void cbuf_test(const char *test_name, struct cbufhead *chdr, const char *contents)
{
    ...
    {
        printf("%s: FAIL: %s (%zu) vs. %s (%zu)\n", test_name, contents, strlen(contents), buf_string,
               strlen(buf_string));
        num_tests_failed++;
    }
}
