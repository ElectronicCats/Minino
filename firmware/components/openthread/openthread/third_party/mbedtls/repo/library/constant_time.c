...
size_t mbedtls_ct_size_mask_ge(size_t x, size_t y) { return (~mbedtls_ct_size_mask_lt(x, y)); }
...
#pragma warning(pop)
    ...
    for (i = 0; i < n; i++) dest[i] = (src[i] & mask) | (dest[i] & ~mask);
}
...
void mbedtls_ct_memcpy_if_eq(unsigned char *dest, const unsigned char *src, size_t len, size_t c1, size_t c2)
{
    ...
    for (size_t i = 0; i < len; i++) dest[i] = (src[i] & mask) | (dest[i] & ~mask);
}
...
int mbedtls_mpi_safe_cond_assign(mbedtls_mpi *X, const mbedtls_mpi *Y, unsigned char assign)
{
    ...
    for (i = Y->n; i < X->n; i++) X->p[i] &= ~limb_mask;
