...
void mbedtls_md2_init(mbedtls_md2_context *ctx) { memset(ctx, 0, sizeof(mbedtls_md2_context)); }
...
void mbedtls_md2_clone(mbedtls_md2_context *dst, const mbedtls_md2_context *src) { *dst = *src; }
...
void mbedtls_md2_starts(mbedtls_md2_context *ctx) { mbedtls_md2_starts_ret(ctx); }
#endif
...
#if !defined(MBEDTLS_DEPRECATED_REMOVED)
void mbedtls_md2_process(mbedtls_md2_context *ctx) { mbedtls_internal_md2_process(ctx); }
#endif
...
int mbedtls_md2_finish_ret(mbedtls_md2_context *ctx, unsigned char output[16])
{
    ...
    for (i = ctx->left; i < 16; i++) ctx->buffer[i] = x;
...
#if !defined(MBEDTLS_DEPRECATED_REMOVED)
void mbedtls_md2_finish(mbedtls_md2_context *ctx, unsigned char output[16]) { mbedtls_md2_finish_ret(ctx, output); }
#endif
