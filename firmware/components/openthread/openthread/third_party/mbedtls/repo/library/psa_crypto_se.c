...
const psa_drv_se_t *psa_get_se_driver_methods(const psa_se_drv_table_entry_t *driver) { return (driver->methods); }

psa_drv_se_context_t *psa_get_se_driver_context(psa_se_drv_table_entry_t *driver) { return (&driver->u.context); }
...
psa_status_t psa_load_se_persistent_data(const psa_se_drv_table_entry_t *driver)
{
    ...
    return (psa_its_get(uid, 0, (uint32_t)driver->u.internal.persistent_data_size, driver->u.internal.persistent_data,
                        &length));
}
...
psa_status_t psa_save_se_persistent_data(const psa_se_drv_table_entry_t *driver)
{
     ...
     * persistent_data_size is in range, but compilers don't know that,
     * so cast to reassure them. */
    return (psa_its_set(uid, (uint32_t)driver->u.internal.persistent_data_size, driver->u.internal.persistent_data, 0));
}
                                      ...
                                      psa_key_slot_number_t      *slot_number)
{
    ...
    if (psa_get_key_slot_number(attributes, slot_number) == PSA_SUCCESS)
    {
        ...
        if (p_validate_slot_number == NULL)
            return (PSA_ERROR_NOT_SUPPORTED);
        status = p_validate_slot_number(&driver->u.context, driver->u.internal.persistent_data, attributes, method,
                                        *slot_number);
    }
...
psa_status_t psa_destroy_se_key(psa_se_drv_table_entry_t *driver, psa_key_slot_number_t slot_number)
{
    ...
    status =
        driver->methods->key_management->p_destroy(&driver->u.context, driver->u.internal.persistent_data, slot_number);
...
psa_status_t psa_init_all_se_drivers(void)
{
    ...
    for (i = 0; i < PSA_MAX_SE_DRIVERS; i++)
    {
        ...
        {
            psa_status_t status =
                methods->p_init(&driver->u.context, driver->u.internal.persistent_data, driver->location);
            if (status != PSA_SUCCESS)
                return (status);
