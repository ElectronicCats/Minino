...
#define USAGE3                                                                                                       \
    ...
    "\n" USAGE_TICKETS USAGE_EAP_TLS USAGE_REPRODUCIBLE USAGE_NSS_KEYLOG USAGE_NSS_KEYLOG_FILE USAGE_CACHE           \
        USAGE_MAX_FRAG_LEN USAGE_TRUNC_HMAC USAGE_ALPN USAGE_EMS USAGE_ETM USAGE_CURVES "\n"
#define USAGE4                                                                          \
    USAGE_SSL_ASYNC                                                                     \
    USAGE_SNI                                                                           \
...
static int ssl_async_decrypt(mbedtls_ssl_context *ssl,
                             ...
                             size_t               input_len)
{
    return (ssl_async_start(ssl, cert, ASYNC_OP_DECRYPT, MBEDTLS_MD_NONE, input, input_len));
}
...
static int ssl_async_resume(mbedtls_ssl_context *ssl, unsigned char *output, size_t *output_len, size_t output_size)
{
    ...
    case ASYNC_OP_DECRYPT:
        ret = mbedtls_pk_decrypt(key_slot->pk, ctx->input, ctx->input_len, output, output_len, output_size,
                                 config_data->f_rng, config_data->p_rng);
        break;
    case ASYNC_OP_SIGN:
        ret = mbedtls_pk_sign(key_slot->pk, ctx->md_alg, ctx->input, ctx->input_len, output, output_len,
                              config_data->f_rng, config_data->p_rng);
        break;
    default:
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
static psa_status_t psa_setup_psk_key_slot(psa_key_id_t *slot, psa_algorithm_t alg, unsigned char *psk, size_t psk_len)
{
    psa_status_t         status;
...
#if defined(MBEDTLS_SSL_DTLS_CONNECTION_ID)
int report_cid_usage(mbedtls_ssl_context *ssl, const char *additional_description)
{
    int           ret;
...
int main(int argc, char *argv[])
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        *q++ = '\0';
        ...
        else if (strcmp(p, "min_version") == 0)
        {
            ...
            else if (strcmp(q, "tls12") == 0 || strcmp(q, "dtls12") == 0)
    ...
    if (opt.psk_opaque != 0)
    {
        ...
        if (opt.force_ciphersuite[0] <= 0)
        {
            mbedtls_printf("opaque PSKs are only supported in conjunction with forcing TLS 1.2 and a PSK-only "
                           "ciphersuite through the 'force_ciphersuite' option.\n");
            ret = 2;
            goto usage;
        }
    }
    ...
    if ((ret = mbedtls_net_bind(&listen_fd, opt.server_addr, opt.server_port,
                                opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM ? MBEDTLS_NET_PROTO_TCP
    ...
    if (opt.use_srtp == 1)
    {
        ...
        mbedtls_ssl_conf_srtp_mki_value_supported(&conf, opt.support_mki ? MBEDTLS_SSL_DTLS_SRTP_MKI_SUPPORTED
    ...
    if (opt.eap_tls != 0)
    {
        mbedtls_ssl_conf_export_keys_ext_cb(&conf, eap_tls_key_derivation, &eap_tls_keying);
    }
    else if (opt.nss_keylog != 0)
    {
        mbedtls_ssl_conf_export_keys_ext_cb(&conf, nss_keylog_export, NULL);
    }
#if defined(MBEDTLS_SSL_DTLS_SRTP)
    else if (opt.use_srtp != 0)
    {
        mbedtls_ssl_conf_export_keys_ext_cb(&conf, dtls_srtp_key_derivation, &dtls_srtp_keying);
    }
#endif /* MBEDTLS_SSL_DTLS_SRTP */
#endif /* MBEDTLS_SSL_EXPORT_KEYS */
    ...
    {
        if ((ret = mbedtls_ssl_ticket_setup(&ticket_ctx, rng_get, &rng, MBEDTLS_CIPHER_AES_256_GCM,
    ...
    if (opt.transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM)
    {
        ...
        if (opt.cookies > 0)
        {
            if ((ret = mbedtls_ssl_cookie_setup(&cookie_ctx, rng_get, &rng)) != 0)
            {
    ...
    if (opt.version_suites != NULL)
    {
        mbedtls_ssl_conf_ciphersuites_for_version(&conf, version_suites[0], MBEDTLS_SSL_MAJOR_VERSION_3,
                                                  MBEDTLS_SSL_MINOR_VERSION_0);
        mbedtls_ssl_conf_ciphersuites_for_version(&conf, version_suites[1], MBEDTLS_SSL_MAJOR_VERSION_3,
                                                  MBEDTLS_SSL_MINOR_VERSION_1);
        mbedtls_ssl_conf_ciphersuites_for_version(&conf, version_suites[2], MBEDTLS_SSL_MAJOR_VERSION_3,
                                                  MBEDTLS_SSL_MINOR_VERSION_2);
        mbedtls_ssl_conf_ciphersuites_for_version(&conf, version_suites[3], MBEDTLS_SSL_MAJOR_VERSION_3,
    ...
    if (opt.async_operations[0] != '-')
    {
        ...
        mbedtls_ssl_conf_async_private_cb(&conf, sign, decrypt, ssl_async_resume, ssl_async_cancel, &ssl_async_keys);
    }
#endif /* MBEDTLS_SSL_ASYNC_PRIVATE */
#endif /* MBEDTLS_X509_CRT_PARSE_C */
