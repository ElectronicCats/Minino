...
#define USAGE3                                                                                                         \
    ...
    "                        default: 1\n"                                                                             \
    "    reconnect_hard=%%d   default: 0 (disabled)\n" USAGE_TICKETS USAGE_EAP_TLS USAGE_MAX_FRAG_LEN USAGE_TRUNC_HMAC \
        USAGE_CONTEXT_CRT_CB USAGE_ALPN USAGE_FALLBACK USAGE_EMS USAGE_ETM USAGE_REPRODUCIBLE USAGE_CURVES             \
            USAGE_RECSPLIT USAGE_DHMLEN "\n"
#define USAGE4                                                                          \
...
int main(int argc, char *argv[])
{
    ...
    if (opt.key_opaque != 0)
    {
        if ((ret = mbedtls_pk_wrap_as_opaque(&pkey, &key_slot, PSA_ALG_SHA_256)) != 0)
        {
            mbedtls_printf(" failed\n  !  "
    ...
    if (opt.reconnect != 0)
    {
        ...
        if (opt.reco_mode == 1)
        {
            ...
            mbedtls_ssl_session_save(mbedtls_ssl_get_session_pointer(&ssl), NULL, 0, &session_data_len);
            ...
            if ((ret = mbedtls_ssl_session_save(mbedtls_ssl_get_session_pointer(&ssl), session_data, session_data_len,
                                                &session_data_len)) != 0)
            {
...
exit:
    ...
    if (opt.psk_opaque != 0)
    {
        ...
        {
            mbedtls_printf("Failed to destroy key slot %u - error was %d", (unsigned)slot, (int)status);
            if (ret == 0)
                ret = MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
        }
    }
