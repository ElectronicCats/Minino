...
#if !defined(MBEDTLS_BIGNUM_C) || !defined(MBEDTLS_CERTS_C) || !defined(MBEDTLS_ENTROPY_C) ||        \
    !defined(MBEDTLS_SSL_TLS_C) || !defined(MBEDTLS_SSL_SRV_C) || !defined(MBEDTLS_NET_C) ||         \
    !defined(MBEDTLS_RSA_C) || !defined(MBEDTLS_CTR_DRBG_C) || !defined(MBEDTLS_X509_CRT_PARSE_C) || \
    !defined(MBEDTLS_TIMING_C) || !defined(MBEDTLS_FS_IO) || !defined(MBEDTLS_PEM_PARSE_C)
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (pid != 0)
        {
            ...
            if ((ret = mbedtls_ctr_drbg_reseed(&ctr_drbg, (const unsigned char *)"parent", 6)) != 0)
            {
        ...
        {
            mbedtls_printf("pid %d: SSL setup failed!  mbedtls_ctr_drbg_reseed returned %d\n\n", pid, ret);
            goto exit;
        }
