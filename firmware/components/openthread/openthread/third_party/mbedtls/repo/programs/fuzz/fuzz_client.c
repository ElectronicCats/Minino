...
int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size)
{
    ...
    mbedtls_ssl_conf_truncated_hmac(&conf,
                                    (options & 8) ? MBEDTLS_SSL_TRUNC_HMAC_ENABLED : MBEDTLS_SSL_TRUNC_HMAC_DISABLED);
#endif
    ...
    mbedtls_ssl_conf_session_tickets(&conf, (options & 0x100) ? MBEDTLS_SSL_SESSION_TICKETS_DISABLED
