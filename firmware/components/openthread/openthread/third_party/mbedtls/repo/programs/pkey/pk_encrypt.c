...
#if defined(MBEDTLS_BIGNUM_C) && defined(MBEDTLS_PK_PARSE_C) && defined(MBEDTLS_ENTROPY_C) && \
...
int main(int argc, char *argv[])
{
    ...
    if ((ret = mbedtls_pk_encrypt(&pk, input, strlen(argv[2]), buf, &olen, sizeof(buf), mbedtls_ctr_drbg_random,
                                  &ctr_drbg)) != 0)
    {
