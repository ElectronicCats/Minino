...
#define MBEDTLS_EXIT_FAILURE EXIT_FAILURE
    ...
    !defined(MBEDTLS_ECP_DP_CURVE25519_ENABLED) || !defined(MBEDTLS_ENTROPY_C) || !defined(MBEDTLS_CTR_DRBG_C)
int main(void)
{
...
int main(int argc, char *argv[])
{
    ...
    ret = mbedtls_ecdh_compute_shared(&ctx_srv.grp, &ctx_srv.z, &ctx_srv.Qp, &ctx_srv.d, mbedtls_ctr_drbg_random,
                                      &ctr_drbg);
    if (ret != 0)
    {
    ...
    ret = mbedtls_ecdh_compute_shared(&ctx_cli.grp, &ctx_cli.z, &ctx_cli.Qp, &ctx_cli.d, mbedtls_ctr_drbg_random,
                                      &ctr_drbg);
    if (ret != 0)
    {
        mbedtls_printf(" failed\n  ! mbedtls_ecdh_compute_shared returned %d\n", ret);
