...
typedef enum {
  ...
  OPENTHREAD_EVENT_MULTICAST_GROUP_JOIN,  /*!< OpenThread stack joined IPv6
                                             multicast group */
  OPENTHREAD_EVENT_MULTICAST_GROUP_LEAVE, /*!< OpenThread stack left IPv6
                                             multicast group */
  OPENTHREAD_EVENT_TREL_ADD_IP6, /*!< OpenThread stack added TREL IPv6 address
                                  */
  OPENTHREAD_EVENT_TREL_REMOVE_IP6, /*!< OpenThread stack removed TREL IPv6
                                       address */
  OPENTHREAD_EVENT_TREL_MULTICAST_GROUP_JOIN, /*!< OpenThread stack joined TREL
...
typedef enum {
  ...
  RADIO_MODE_UART_RCP, /*!< UART connection to a 15.4 capable radio co-processor
                          (RCP) */
  RADIO_MODE_SPI_RCP,  /*!< SPI connection to a 15.4 capable radio co-processor
                          (RCP) */
...
typedef struct {
  ...
  union {
    ...
    esp_openthread_spi_host_config_t
        radio_spi_config; /*!< The spi configuration to RCP */
  };
...
typedef struct {
  ...
  union {
    ...
    usb_serial_jtag_driver_config_t
        host_usb_config; /*!< The usb configuration to host */
    esp_openthread_spi_slave_config_t
        spi_slave_config; /*!< The spi configuration to host */
  };
 ...
 */
typedef struct {
  const char* storage_partition_name; /*!< The partition for storing OpenThread
                                         dataset */
  uint8_t
      netif_queue_size; /*!< The packet queue size for the network interface */
