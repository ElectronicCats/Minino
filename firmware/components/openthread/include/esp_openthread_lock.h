 ...
 *
 * @note Every OT APIs that takes an otInstance argument MUST be protected with
 * this API lock except that the call site is in OT callbacks.
 *
 * @param[in] block_ticks   The maxinum number of RTOS ticks to wait for the
 * lock.
 *
 * @return
 ...
 *
 * @note In OpenThread API context, it waits for some actions to be done in
 * other tasks (like lwip), after task switching, it needs to call OpenThread
 * API again. Normally it's not allowed, since the previous OpenThread API lock
 * is not released yet. This task_switching lock allows the OpenThread API can
