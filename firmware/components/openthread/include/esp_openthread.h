 ...
 *
 */
esp_err_t esp_openthread_init(
    const esp_openthread_platform_config_t* init_config);

/**
 * @brief   Starts the Thread protocol operation and attaches to a Thread
 * network.
 *
 * @param[in]  datasetTlvs    The operational dataset (TLV encoded), if it's
 * NULL, the function will generate the dataset based on the configurations from
 * kconfig.
 *
 * @return
 *      - ESP_OK on success
 ...
 *
 * @note This function will not return unless error happens when running the
 * OpenThread stack.
 *
 * @return
 *      - ESP_OK on success
...
/**
 * @brief This function performs OpenThread stack and platform driver
 * deinitialization.
 *
 * @return
 *      - ESP_OK on success
