...
#define SPINEL_PROP_VENDOR_ESP_SET_COORDINATOR \
  (SPINEL_PROP_VENDOR_ESP__BEGIN + 1) /* Vendor command for coordinator.*/

#define SPINEL_PROP_VENDOR_ESP_SET_PENDINGMODE \
  (SPINEL_PROP_VENDOR_ESP__BEGIN + 2) /* Vendor command for pending mode.*/
...
} esp_radio_spinel_idx_t; /* The index of 802.15.4 related protocol stack for
                             ESP radio spinel.*/
...
} esp_radio_spinel_uart_config_t; /*This structure represents a context for ESP
                                     radio spinel. */

typedef void (*esp_radio_spinel_rcp_failure_handler)(
    void); /* The handler for rcp failure.*/
typedef esp_err_t (*esp_radio_spinel_uart_init_handler)(
    const esp_radio_spinel_uart_config_t* uart_config_t,
    int* uart_fd); /* The handler for UART initialization.*/
typedef esp_err_t (*esp_radio_spinel_uart_deinit_handler)(
    const esp_radio_spinel_uart_config_t* uart_config_t,
    int* uart_fd); /* The handler for UART deinitialization.*/

typedef struct {
  void (*receive_done)(
      const uint8_t* frame,
      esp_ieee802154_frame_info_t* frame_info); /* Callback for Receive Done.*/
  void (*transmit_done)(const uint8_t* frame,
                        const uint8_t* ack,
                        esp_ieee802154_frame_info_t*
                            ack_frame_info); /* Callback for Transmit Done.*/
  void (*transmit_failed)(
      esp_ieee802154_tx_error_t error);      /* Callback for Transmit Failed.*/
  void (*energy_scan_done)(int8_t max_rssi); /* Callback for Energy Scan Done.*/
  void (*transmit_started)(
      const uint8_t* frame);             /* Callback for Transmit Started.*/
  void (*switchover_done)(bool success); /* Callback for Switchover Done.*/

#if CONFIG_OPENTHREAD_DIAG
  void (*diag_receive_done)(
      const uint8_t* frame,
      esp_ieee802154_frame_info_t*
          frame_info); /* Callback for Receive Done (diag).*/
  void (*diag_transmit_done)(
      const uint8_t* frame,
      esp_ieee802154_frame_info_t*
          frame_info); /* Callback for Transmit Done (diag).*/
  void (*diag_transmit_failed)(
      esp_ieee802154_tx_error_t
          error);               /* Callback for Transmit Failed (diag).*/
 ...
 * @param[in]  idx                  The index of 802.15.4 related protocol
 * stack.
 *
 * @return
 *      - ESP_OK on success
 ...
 *
 * @note This function should be called after `esp_radio_spinel_set_callbacks`
 ...
 *
 * @param[in]  scan_channel     The channel of energy scan (usually the range is
 * 11~26, scan all channels if it's set to 0).
 ...
 * @param[in]  cca      Perform clear channel assessment(if it's true) or not(if
 * it's false)
 * @param[in]  idx      The index of 802.15.4 related protocol stack.
 *
 * @return
