 ...
 *
 * When defined to 1, the platform MUST implement the otPlatFlash* APIs instead
 * of the otPlatSettings* APIs.
 *
 */
 ...
 *
 * The log level (used at compile time). If
 * `OPENTHREAD_CONFIG_LOG_LEVEL_DYNAMIC_ENABLE` is set, this defines the most
 * verbose log level possible. See `OPENTHREAD_CONFIG_LOG_LEVEL_INIT` to set the
 * initial log level.
 *
 */
 ...
 */
   ...
   *
   * This setting configures the interval (in units of microseconds) for
   * host-rcp time sync. The host will recalculate the time offset between host
   * and RCP every interval.
   *
   */
 ...
 *
 * Define to 1 to support injecting Service entries into the Thread Network
 * Data.
 *
 */
 ...
 *
 * Note that the OPENTHREAD_CONFIG_ENABLE_BUILTIN_MBEDTLS determines whether to
 * use bultin-mbedtls as well as whether to manage mbedTLS internally, such as
 * memory allocation and debug.
 *
 */
 ...
 *
 * Define as 1 to enable support for adding of auto-configured SLAAC addresses
 * by OpenThread.
 *
 */
 ...
 *
 * The maximum number of state-changed callback handlers (set using
 ...
 *
 * Specifies the rx frame buffer size used by `SpinelInterface` in RCP host
 * code. This is applicable/used when `RadioSpinel` platform is used.
 *
 */
 ...
 *
 * Ping sender module implements sending ICMPv6 Echo Request messages and
 * processing ICMPv6 Echo Reply messages.
 *
 */
 ...
 */
   ...
   *
   * This config is applicable only when
   * `OPENTHREAD_CONFIG_SRP_CLIENT_BUFFERS_ENABLE` is enabled.
   *
   */
 ...
 */
   ...
   *
   * The message pool is managed by platform defined logic when this flag is
   * set. This feature would typically be used when operating in a
   * multi-threaded system and multiple threads need to access the message pool.
   *
   */
 ...
 *
 * Set to 1 to enable support for Radio Statistics. Note that this option only
 * works for OPENTHREAD_FTD and OPENTHREAD_MTD.
 *
 */
 ...
 *
 * The timeout value (in seconds) waiting for a address notification response
 * after sending an address query.
 *
 */
