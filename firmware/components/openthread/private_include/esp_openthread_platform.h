 ...
 *
 */
typedef void (*esp_openthread_update_func)(
 ...
 *
 */
typedef esp_err_t (*esp_openthread_process_func)(
    otInstance* instance,
    const esp_openthread_mainloop_context_t* mainloop);

/**
 * @brief This struct contains a platform update function, a platform process
 * function and the workflow name
 *
 * @note The structs will form a list, and the update functions and process
 * functions in the list will be called in esp_openthread_platform_update and
...
/**
 * @brief This function adds a esp_openthread_platform_workflow to the workflow
 * list
 *
 *
 * @param[in]  updatefcn   The update function of the workflow added to the
 * list.
 * @param[in]  processfcn  The process function of the workflow added to the
 * list.
...
/**
 * @brief This function removes a esp_openthread_platform_workflow according the
 * input name from the workflow list
 *
 ...
 *
 * @note This function is not called by and will not call the OpenThread
 * library. The user needs to call otInstanceInitSingle to intialize the
 * OpenThread stack after calling this function.
 *
...
esp_err_t esp_openthread_platform_init(
    const esp_openthread_platform_config_t* init_config);

/**
 * This function performs all platform-specific deinitialization for
 * OpenThread's drivers.
 *
 * @note This function is not called by the OpenThread library. Instead, the
 * user should call this function when deinitialization of OpenThread's drivers
 * is most appropriate.
 *
 * @return
 *      - ESP_OK on success
 ...
 *       The users need to call `otTaskletsArePending` to check whether there
 * being pending OpenThread tasks.
 *
 * @param[inout]    mainloop    The main loop context.
 *
 */
...
/**
 * @brief This function performs the OpenThread related platform process (radio,
 * uart, alarm etc.)
 *
