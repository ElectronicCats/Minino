...
static int buf_fill_ieee80211_beacon(uint8_t* buf,
                                     size_t* len,
                                     size_t buf_size,
  ...
  beacon->capability = cpu_to_le16(IEEE80211_CAPINFO_SHORT_SLOTTIME |
...
void drone_export_gps_data(ODID_UAS_Data* UAS_Data,
                           char* buf,
                           size_t buf_size) {
  ptrdiff_t len = 0;
...
int odid_message_build_pack(ODID_UAS_Data* UAS_Data,
                            void* pack,
                            size_t buflen) {
  ...
  if (UAS_Data->OperatorIDValid) {
    ...
    if (encodeOperatorIDMessage(
            (void*) &msg_pack.Messages[msg_pack.MsgPackSize],
  ...
  len = sizeof(*msg_pack_enc) -
        (ODID_PACK_MAX_MESSAGES - msg_pack.MsgPackSize) * ODID_MESSAGE_SIZE;
...
int odid_wifi_build_nan_sync_beacon_frame(char* mac,
  ...
  ret = buf_fill_ieee80211_mgmt(buf, &len, buf_size, IEEE80211_STYPE_BEACON,
                                target_addr, (uint8_t*) mac, cluster_id);
  if (ret < 0)
    return ret;
  ...
  master_indication_attr =
      (struct nan_master_indication_attribute*) (buf + len);
...
int odid_wifi_build_message_pack_nan_action_frame(ODID_UAS_Data* UAS_Data,
  ...
  nsda->header.length =
      cpu_to_le16(sizeof(*nsda) - sizeof(struct nan_attribute_header) +
...
int odid_wifi_build_message_pack_beacon_frame(ODID_UAS_Data* UAS_Data,
  ...
  /* IEEE 802.11 Management Header */
  ret = buf_fill_ieee80211_mgmt(buf, &len, buf_size, IEEE80211_STYPE_BEACON,
                                target_addr, (uint8_t*) mac, (uint8_t*) mac);
  if (ret < 0)
    return ret;
  ...
  /* set the lengths according to the message pack lengths */
  vendor->length =
      sizeof(vendor->oui) + sizeof(vendor->oui_type) + sizeof(*si) + ret;
