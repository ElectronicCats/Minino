 ...
 * http://asd-stan.org/downloads/asd-stan-pren-4709-002-p1/ ASTM F3411 v1.1
 * draft sent for first ballot round autumn 2021 2: ASTM F3411-v1.1 draft sent
 * for second ballot round Q1 2022. (ASTM F3411-22 ?) The delta to protocol
 * version 1 is small:
 *    - New enum values ODID_STATUS_REMOTE_ID_SYSTEM_FAILURE,
 * ODID_DESC_TYPE_EMERGENCY and ODID_DESC_TYPE_EXTENDED_STATUS
...
typedef enum ODID_idtype {
  ...
  ODID_IDTYPE_UTM_ASSIGNED_UUID =
      3,  // UAS (Unmanned Aircraft System) Traffic Management
  ODID_IDTYPE_SPECIFIC_SESSION_ID =
      4,  // The exact id type is specified by the first byte of UASID and these
          // type values are managed by ICAO. 0 is reserved. 1 - 224 are managed
          // by ICAO. 225 - 255 are available for private experimental usage
          // only
  // 5 to 15 reserved
} ODID_idtype_t;
...
typedef enum ODID_uatype {
  ...
  ODID_UATYPE_HYBRID_LIFT =
      4,  // Fixed wing aircraft that can take off vertically
...
typedef enum ODID_authtype {
  ...
  ODID_AUTH_SPECIFIC_AUTHENTICATION =
      5,  // Specific auth method. The exact authentication type is indicated by
          // the first byte of AuthData and these type values are managed by
          // ICAO. 0 is reserved. 1 - 224 are managed by ICAO. 225 - 255 are
          // available for private experimental usage only
...
typedef enum ODID_desctype {
  ...
  ODID_DESC_TYPE_EMERGENCY =
      1,  // Additional clarification when ODID_status == EMERGENCY
...
typedef enum ODID_operator_location_type {
  ...
  ODID_OPERATOR_LOCATION_TYPE_LIVE_GNSS =
      1,                                  // Dynamic/Live location and altitude
...
typedef struct ODID_BasicID_data {
  ...
  char UASID[ODID_ID_SIZE + 1];  // Additional byte to allow for null term in
                                 // normative form. Fill unused space with NULL
} ODID_BasicID_data;
...
typedef struct ODID_Location_data {
  ...
  float SpeedHorizontal;  // m/s. Positive only. Invalid, No Value, or Unknown:
                          // 255m/s. If speed is >= 254.25 m/s: 254.25m/s
  float SpeedVertical;    // m/s. Invalid, No Value, or Unknown: 63m/s. If speed
                          // is >= 62m/s: 62m/s
  ...
  float AltitudeBaro;  // meter (Ref 29.92 inHg, 1013.24 mb). Invalid, No Value,
                       // or Unknown: -1000m
  float
      AltitudeGeo;  // meter (WGS84-HAE). Invalid, No Value, or Unknown: -1000m
  ...
  float TimeStamp;  // seconds after the full hour relative to UTC. Invalid, No
                    // Value, or Unknown: 0xFFFF
} ODID_Location_data;

/*
 * The Authentication message can have two different formats:
 *  - For data page 0, the fields LastPageIndex, Length and TimeStamp are
 * present. The size of AuthData is maximum 17 bytes
 * (ODID_AUTH_PAGE_ZERO_DATA_SIZE).
 *  - For data page 1 through (ODID_AUTH_MAX_PAGES - 1), LastPageIndex, Length
 * and TimeStamp are not present. For pages 1 to LastPageIndex, the size of
 * AuthData is maximum 23 bytes (ODID_AUTH_PAGE_NONZERO_DATA_SIZE).
 *
...
typedef struct ODID_Auth_data {
  ...
  uint32_t
      Timestamp;  // Page 0 only. Relative to 00:00:00 01/01/2019 UTC/Unix Time
  uint8_t
      AuthData[ODID_AUTH_PAGE_NONZERO_DATA_SIZE +
               1];  // Additional byte to allow for null term in normative form
} ODID_Auth_data;

typedef struct ODID_SelfID_data {
  ODID_desctype_t DescType;
  char Desc[ODID_STR_SIZE + 1];  // Additional byte to allow for null term in
                                 // normative form. Fill unused space with NULL
} ODID_SelfID_data;
...
typedef struct ODID_System_data {
  ...
  double
      OperatorLatitude;  // Invalid, No Value, or Unknown: 0 deg (both Lat/Lon)
  double
      OperatorLongitude;  // Invalid, No Value, or Unknown: 0 deg (both Lat/Lon)
  ...
  ODID_category_EU_t CategoryEU;  // Only filled if ClassificationType =
                                  // ODID_CLASSIFICATION_TYPE_EU
  ODID_class_EU_t ClassEU;        // Only filled if ClassificationType =
                                  // ODID_CLASSIFICATION_TYPE_EU
  float OperatorAltitudeGeo;      // meter (WGS84-HAE). Invalid, No Value, or
                                  // Unknown: -1000m
...
typedef struct ODID_OperatorID_data {
  ...
  char OperatorId[ODID_ID_SIZE +
                  1];  // Additional byte to allow for null term in normative
                       // form. Fill unused space with NULL
 ...
 * DataPage and AuthType) from either of the union members, since the
 * declarations for these fields are identical and the first parts of the
 * structures. The ISO/IEC 9899:1999 Chapter 6.5.2.3 part 5 and related Example
 * 3 documents this. https://www.iso.org/standard/29237.html
 */
...
typedef struct __attribute__((__packed__)) ODID_System_encoded {
  ...
  // Byte 1 [Reserved][ClassificationType][OperatorLocationType]  -- must define
  // LSb first
 ...
 * The ISO/IEC 9899:1999 Chapter 6.5.2.3 part 5 and related Example 3 documents
 * this. https://www.iso.org/standard/29237.html
 */
...
int encodeBasicIDMessage(ODID_BasicID_encoded* outEncoded,
                         ODID_BasicID_data* inData);
int encodeLocationMessage(ODID_Location_encoded* outEncoded,
                          ODID_Location_data* inData);
int encodeAuthMessage(ODID_Auth_encoded* outEncoded, ODID_Auth_data* inData);
int encodeSelfIDMessage(ODID_SelfID_encoded* outEncoded,
                        ODID_SelfID_data* inData);
int encodeSystemMessage(ODID_System_encoded* outEncoded,
                        ODID_System_data* inData);
int encodeOperatorIDMessage(ODID_OperatorID_encoded* outEncoded,
                            ODID_OperatorID_data* inData);
int encodeMessagePack(ODID_MessagePack_encoded* outEncoded,
                      ODID_MessagePack_data* inData);

int decodeBasicIDMessage(ODID_BasicID_data* outData,
                         ODID_BasicID_encoded* inEncoded);
int decodeLocationMessage(ODID_Location_data* outData,
                          ODID_Location_encoded* inEncoded);
int decodeAuthMessage(ODID_Auth_data* outData, ODID_Auth_encoded* inEncoded);
int decodeSelfIDMessage(ODID_SelfID_data* outData,
                        ODID_SelfID_encoded* inEncoded);
int decodeSystemMessage(ODID_System_data* outData,
                        ODID_System_encoded* inEncoded);
int decodeOperatorIDMessage(ODID_OperatorID_data* outData,
...
ODID_messagetype_t decodeOpenDroneID(ODID_UAS_Data* uas_data,
                                     uint8_t* msg_data);
 ...
 * with each type of message from the drone information into an NAN action
 * frame.
 * @UAS_Data: general drone status information
 * @mac: mac address of the wifi adapter where the NAN frame will be sent
 * @send_counter: sequence number, to be increased for each call of this
 * function
 * @buf: pointer to buffer space where the NAN will be written to
 * @buf_size: maximum size of the buffer
 *
 ...
 */
int odid_message_process_pack(ODID_UAS_Data* UAS_Data,
                              uint8_t* pack,
                              size_t buflen);

/* odid_wifi_receive_message_pack_nan_action_frame - processes a received
 * message pack with each type of message from the drone information into an NAN
 * action frame
 * @UAS_Data: general drone status information
 * @mac: mac address of the wifi adapter where the NAN frame will be sent
