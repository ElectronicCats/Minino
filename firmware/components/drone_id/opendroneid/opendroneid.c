 ...
 * @return Encoded Speed in a single byte or max speed if over max encoded
 * speed.
 */
static uint8_t encodeSpeedHorizontal(float Speed_data, uint8_t* mult) {
  if (Speed_data <= UINT8_MAX * SPEED_DIV[0]) {
    ...
    *mult = 1;
    int big_value =
        (int) ((Speed_data - (UINT8_MAX * SPEED_DIV[0])) / SPEED_DIV[1]);
...
static int32_t encodeLatLon(double LatLon_data) {
  return (int32_t) intRangeMax((int64_t) (LatLon_data * LATLON_MULT),
                               -180 * LATLON_MULT, 180 * LATLON_MULT);
}
...
static uint16_t encodeAltitude(float Alt_data) {
  return (uint16_t) intRangeMax(
      (int) ((Alt_data + (float) ALT_ADDER) / ALT_DIV), 0, UINT16_MAX);
}
...
int encodeLocationMessage(ODID_Location_encoded* outEncoded,
  ...
  if (inData->SpeedHorizontal < MIN_SPEED_H ||
      inData->SpeedHorizontal > INV_SPEED_H ||
      (inData->SpeedHorizontal > MAX_SPEED_H &&
       inData->SpeedHorizontal < INV_SPEED_H))
    return ODID_FAIL;

  if (inData->SpeedVertical < MIN_SPEED_V ||
      inData->SpeedVertical > INV_SPEED_V ||
      (inData->SpeedVertical > MAX_SPEED_V &&
       inData->SpeedVertical < INV_SPEED_V))
    return ODID_FAIL;
  ...
  outEncoded->SpeedHorizontal =
      encodeSpeedHorizontal(inData->SpeedHorizontal, &bitflag);
...
int encodeSystemMessage(ODID_System_encoded* outEncoded,
  ...
  if (inData->OperatorLatitude < MIN_LAT ||
      inData->OperatorLatitude > MAX_LAT ||
      inData->OperatorLongitude < MIN_LON ||
      inData->OperatorLongitude > MAX_LON)
    return ODID_FAIL;
  ...
  if (inData->AreaCeiling < MIN_ALT || inData->AreaCeiling > MAX_ALT ||
      ...
      inData->OperatorAltitudeGeo < MIN_ALT ||
      inData->OperatorAltitudeGeo > MAX_ALT)
    return ODID_FAIL;
...
int encodeOperatorIDMessage(ODID_OperatorID_encoded* outEncoded,
  ...
  strncpy(outEncoded->OperatorId, inData->OperatorId,
          sizeof(outEncoded->OperatorId));
  memset(outEncoded->Reserved, 0, sizeof(outEncoded->Reserved));
  return ODID_SUCCESS;
}
...
static int checkPackContent(ODID_Message_encoded* msgs, int amount) {
  ...
  int numMessages[6] = {
      0};  // Counters for relevant parts of ODID_messagetype_t
...
int decodeLocationMessage(ODID_Location_data* outData,
  ...
  outData->HorizAccuracy =
      (ODID_Horizontal_accuracy_t) inEncoded->HorizAccuracy;
...
int decodeOperatorIDMessage(ODID_OperatorID_data* outData,
  ...
  safe_dec_copyfill(outData->OperatorId, inEncoded->OperatorId,
                    sizeof(outData->OperatorId));
  return ODID_SUCCESS;
}
  ...
  switch (decodeMessageType(msgData[0])) {
    case ODID_MESSAGETYPE_BASIC_ID: {
      ...
      if (getBasicIDType(basicId, &idType) == ODID_SUCCESS) {
        // Find a free slot to store the current message in or overwrite old
    ...
    case ODID_MESSAGETYPE_OPERATOR_ID: {
      ...
      if (decodeOperatorIDMessage(&uasData->OperatorID, operatorId) ==
          ODID_SUCCESS) {
 ...
 * Therefore, the destination must use the last byte for a null (and is +1 in
 * size)
 *
...
static char* safe_dec_copyfill(char* dstStr, const char* srcStr, int dstSize) {
  ...
  strncpy(dstStr, srcStr,
          dstSize - 1);  // copy only up to dst size-1 (no overruns)
  return dstStr;
}

/**
 * Safely range check a value and return the minimum or max within the range if
 * exceeded
 *
...
void printAuth_data(ODID_Auth_data* Auth) {
  if (Auth->DataPage == 0) {
    ...
    printf(ODID_Auth_data_format, Auth->AuthType, Auth->DataPage,
           Auth->LastPageIndex, Auth->Length, (uint32_t) Auth->Timestamp);
...
void printSystem_data(ODID_System_data* System_data) {
  ...
  printf(ODID_System_data_format, System_data->OperatorLocationType,
         ...
         System_data->OperatorLongitude, System_data->AreaCount,
         System_data->AreaRadius, (double) System_data->AreaCeiling,
         (double) System_data->AreaFloor, System_data->CategoryEU,
         System_data->ClassEU, (double) System_data->OperatorAltitudeGeo,
         (uint16_t) System_data->Timestamp);
}
