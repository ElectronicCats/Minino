 ...
 */
  ...
  // The Pico doesn't have BT and the NAN/OD beacon code needs work to get it to
...
// #define ID_JAPAN 0  // Experimental
  ...
  #define WIFI_CHANNEL      13  // Be careful changing this.
  #define BEACON_FRAME_SIZE 512
  #define BEACON_INTERVAL \
    0  // ms, defaults to 500. Android apps would prefer 100ms.
...
class ID_OpenDrone {
 ...
 private:
  ...
  uint8_t WiFi_mac_addr[6], wifi_channel = WIFI_CHANNEL, *auth_key = NULL,
                            *auth_iv = NULL;
    #if ID_OD_WIFI
  uint32_t sequence = 1;
  uint16_t beacon_interval = 0x200;
