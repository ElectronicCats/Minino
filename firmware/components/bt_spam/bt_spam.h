...
/**
 * @brief Fully deinitialize the bluetooth spam module (BT controller + Bluedroid)
 * Call this when completely shutting down BLE functionality
 */
void bt_spam_app_deinit(void);

/**
 * @brief Set BLE advertising TX power level
 * @param power_level ESP_PWR_LVL_P9, ESP_PWR_LVL_P12, ESP_PWR_LVL_P15, ESP_PWR_LVL_P18, ESP_PWR_LVL_P20
 ...
 * @param min_interval_min Minimum advertising interval in 0.625ms units (0x20 = 20ms)
 * @param max_interval_max Maximum advertising interval in 0.625ms units (0x30 = 30ms)
 * @return ESP_OK on success
 */
esp_err_t bt_spam_set_adv_interval(uint16_t min_interval_min, uint16_t max_interval_max);
 ...
 * This allows multiple modules (e.g., spam + scanner) to share the single GAP callback
