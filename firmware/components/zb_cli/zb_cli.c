...
static void log_nwk_info(const char* status_string) {
  ...
  ESP_LOGI(TAG,
           ...
           status_string, extended_pan_id[7], extended_pan_id[6],
           extended_pan_id[5], extended_pan_id[4], extended_pan_id[3],
  ...
  switch (sig_type) {
    ...
    case ESP_ZB_BDB_SIGNAL_DEVICE_REBOOT:
      if (err_status == ESP_OK) {
        ...
        ESP_LOGE(TAG, "%s failed with status: %s, please retry",
    ...
    case ESP_ZB_ZDO_SIGNAL_LEAVE_INDICATION:
      ...
      ESP_LOGI(TAG, "Zigbee Node (0x%04hx) is leaving network",
