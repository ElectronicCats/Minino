...
typedef esp_err_t (*register_fn_t)(esp_zb_cluster_list_t* cluster_list,
                                   esp_zb_attribute_list_t* attr_list,
                                   uint8_t role_mask);
typedef esp_zb_attribute_list_t* (*create_fn_t)(void*);
typedef esp_err_t (*add_attr_fn_t)(esp_zb_attribute_list_t* attr_list,
                                   uint16_t attr_id,
                                   void* value_p);
...
typedef struct esp_zcl_cluster_fn_s {
  ...
  uint16_t cluster_id;
  register_fn_t register_fn; /*!< function to add cluster (attribute list) to
                                cluster list */
  create_fn_t create_fn;     /*!< function to create default cluster (attribute
                                list) with mandatory attributes */
  add_attr_fn_t
      add_attr_fn; /*!< function to add attibute to cluster (attribute list)  */
 ...
 *
 * Note: The entries in the table MUST be arranged in ascending order of cluster
 * ID.
 */
const static esp_zcl_cluster_fn_t s_cluster_fn_table[] = {
    ...
    CLUSTER_NON_SUPPORT_FN_ENTRY(device_temp_config,
    ...
    CLUSTER_FN_ENTRY(on_off_switch_config,
    ...
    CLUSTER_NON_SUPPORT_FN_ENTRY(rssi_location,
    ...
    CLUSTER_NON_SUPPORT_FN_ENTRY(binary_output,
                                 ESP_ZB_ZCL_CLUSTER_ID_BINARY_OUTPUT),
    CLUSTER_NON_SUPPORT_FN_ENTRY(binary_value,
                                 ESP_ZB_ZCL_CLUSTER_ID_BINARY_VALUE),
    CLUSTER_NON_SUPPORT_FN_ENTRY(multistate_input,
                                 ESP_ZB_ZCL_CLUSTER_ID_MULTI_INPUT),
    CLUSTER_NON_SUPPORT_FN_ENTRY(multistate_output,
    ...
    CLUSTER_NON_SUPPORT_FN_ENTRY(poll_control,
                                 ESP_ZB_ZCL_CLUSTER_ID_POLL_CONTROL),
    CLUSTER_NON_SUPPORT_FN_ENTRY(
        green_power,
        ESP_ZB_ZCL_CLUSTER_ID_GREEN_POWER), /* No API provided */
    ...
    CLUSTER_NON_SUPPORT_FN_ENTRY(pump_config_control,
    ...
    CLUSTER_FN_ENTRY(dehumidification_control,
                     ESP_ZB_ZCL_CLUSTER_ID_DEHUMIDIFICATION_CONTROL),
    CLUSTER_FN_ENTRY(thermostat_ui_config,
                     ESP_ZB_ZCL_CLUSTER_ID_THERMOSTAT_UI_CONFIG),
    CLUSTER_FN_ENTRY(color_control, ESP_ZB_ZCL_CLUSTER_ID_COLOR_CONTROL),
    CLUSTER_NON_SUPPORT_FN_ENTRY(ballast_config,
                                 ESP_ZB_ZCL_CLUSTER_ID_BALLAST_CONFIG),
    CLUSTER_FN_ENTRY(illuminance_meas,
    ...
    CLUSTER_FN_ENTRY(humidity_meas,
                     ESP_ZB_ZCL_CLUSTER_ID_REL_HUMIDITY_MEASUREMENT),
    CLUSTER_FN_ENTRY(occupancy_sensing,
    ...
    CLUSTER_FN_ENTRY(wind_speed_measurement,
                     ESP_ZB_ZCL_CLUSTER_ID_WIND_SPEED_MEASUREMENT),
    CLUSTER_FN_ENTRY(carbon_dioxide_measurement,
                     ESP_ZB_ZCL_CLUSTER_ID_CARBON_DIOXIDE_MEASUREMENT),
    CLUSTER_FN_ENTRY(pm2_5_measurement,
                     ESP_ZB_ZCL_CLUSTER_ID_PM2_5_MEASUREMENT),
    CLUSTER_FN_ENTRY(ias_zone, ESP_ZB_ZCL_CLUSTER_ID_IAS_ZONE),
    CLUSTER_NO_ATTR_FN_ENTRY(
        ias_ace,
        ESP_ZB_ZCL_CLUSTER_ID_IAS_ACE), /* Only have zone table internal */
    CLUSTER_FN_ENTRY(ias_wd, ESP_ZB_ZCL_CLUSTER_ID_IAS_WD),
    CLUSTER_NO_ATTR_FN_ENTRY(
        price,
        ESP_ZB_ZCL_CLUSTER_ID_PRICE), /* Too many attributes */
    CLUSTER_NO_ATTR_FN_ENTRY(
        metering,
        ESP_ZB_ZCL_CLUSTER_ID_METERING), /* Too many attributes */
    CLUSTER_FN_ENTRY(meter_identification,
                     ESP_ZB_ZCL_CLUSTER_ID_METER_IDENTIFICATION),
    CLUSTER_FN_ENTRY(electrical_meas,
...
static const esp_zcl_cluster_fn_t* cluster_fn_table_get_by_id(
    uint16_t cluster_id) {
...
static const esp_zcl_cluster_fn_t* cluster_fn_table_get_by_name(
    const char* name) {
...
esp_err_t esp_zb_cluster_add_std_attr(esp_zb_attribute_list_t* attr_list,
                                      uint16_t attr_id,
                                      void* value_p) {
  esp_err_t ret = ESP_OK;
  const esp_zcl_cluster_fn_t* ent =
      cluster_fn_table_get_by_id(attr_list->next->cluster_id);
...
esp_err_t esp_zb_cluster_register(esp_zb_cluster_list_t* cluster_list,
                                  esp_zb_attribute_list_t* attr_list,
                                  uint8_t role_mask) {
  esp_err_t ret = ESP_OK;
  const esp_zcl_cluster_fn_t* ent =
