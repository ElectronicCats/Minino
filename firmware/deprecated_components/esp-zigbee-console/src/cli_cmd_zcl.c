...
#define cli_output_callback_info(name, info)                \
  ESP_LOGI(TAG, "%s: endpoint(%d) cluster(0x%04x)", (name), \
           (info)->dst_endpoint, (info)->cluster);
...
static esp_err_t zcl_set_attr_value_handler(
...
static esp_err_t zcl_read_attr_resp_handler(
    const esp_zb_zcl_cmd_read_attr_resp_message_t* message) {
  cli_output_callback_info("Read attribute response", &message->info);

  for (esp_zb_zcl_read_attr_resp_variable_t* variables = message->variables;
       variables != NULL; variables = variables->next) {
    if (variables->status == ESP_ZB_ZCL_STATUS_SUCCESS) {
      cli_output_attribute(&variables->attribute);
    } else {
      ESP_LOGI(TAG, "- attribute(0x%04x), status(0x%x)",
               variables->attribute.id, variables->status);
    }
  }
...
static esp_err_t zcl_report_attr_handler(
...
static esp_err_t zcl_write_attr_resp_handler(
    const esp_zb_zcl_cmd_write_attr_resp_message_t* message) {
  cli_output_callback_info("Write attribute response", &message->info);

  for (esp_zb_zcl_write_attr_resp_variable_t* variables = message->variables;
       variables != NULL; variables = variables->next) {
    ESP_LOGI(TAG, "- attribute(0x%04x), status(0x%x)", variables->attribute_id,
             variables->status);
  }
...
static esp_err_t zcl_read_report_cfg_resp_handler(
  ...
  for (esp_zb_zcl_read_report_config_resp_variable_t* variables =
           message->variables;
       variables != NULL; variables = variables->next) {
    ESP_LOGI(TAG, "- attribute(0x%04x), status(0x%x)", variables->attribute_id,
             variables->status);
    if (variables->status == ESP_ZB_ZCL_STATUS_SUCCESS) {
      if (variables->report_direction == ESP_ZB_ZCL_REPORT_DIRECTION_SEND) {
        ...
        ESP_LOGI(TAG, "  min(%d), max(%d), delta(%d)",
                 variables->client.min_interval, variables->client.max_interval,
                 variables->client.delta[0]);
      } else {
...
static esp_err_t zcl_disc_attr_resp_handler(
  ...
  for (esp_zb_zcl_disc_attr_variable_t* variables = message->variables;
       variables != NULL; variables = variables->next) {
    ESP_LOGI(TAG, "- attribute(0x%04x), type(0x%02x)", variables->attr_id,
             variables->data_type);
  }
...
static esp_err_t zcl_add_attribute(esp_zb_attribute_list_t* attr_list,
                                   attribute_config_t* attr_cfg,
                                   bool force) {
  esp_err_t ret = ESP_OK;
  ...
  EXIT_ON_FALSE(attr_cfg->attr_value_p, ESP_ERR_INVALID_ARG,
                cli_output_line("Invalid attribute value"));

  EXIT_ON_ERROR(
      force ? esp_zb_cluster_add_manufacturer_attr(
                  attr_list, attr_list->next->cluster_id, attr_cfg->attr_id,
                  attr_cfg->manuf_code, attr_cfg->attr_type,
                  attr_cfg->attr_access, attr_cfg->attr_value_p)
            : esp_zb_cluster_add_std_attr(attr_list, attr_cfg->attr_id,
...
static esp_err_t zcl_add_cluster(esp_zb_cluster_list_t* cluster_list,
                                 cluster_config_t* cluster_cfg,
                                 bool force) {
  esp_err_t ret = ESP_OK;
  ...
  attr_list = esp_zb_cluster_list_get_cluster(
      cluster_list, cluster_cfg->cluster_id, cluster_cfg->role_mask);
  ...
  if (new_created) {
    EXIT_ON_ERROR(esp_zb_cluster_register(cluster_list, attr_list,
                                          cluster_cfg->role_mask),
...
static esp_err_t zcl_add_endpoint(esp_zb_ep_list_t* ep_list,
                                  endpoint_config_t* endpoint_cfg,
                                  bool force) {
  esp_err_t ret = ESP_OK;
  ...
  EXIT_ON_FALSE(endpoint_cfg->ep_id != 0 && endpoint_cfg->ep_id != 0xFF,
                ESP_ERR_INVALID_ARG);
  ...
  EXIT_ON_ERROR(
      zcl_add_cluster(cluster_list, endpoint_cfg->cluster_cfg, force));
...
static esp_err_t cli_dm_add_parse_params(int argc,
                                         char* argv[],
                                         cli_zcl_add_params_t* params) {
  struct {
    arg_lit_t* force;
    arg_u8_t* ep_id;
    ...
    arg_end_t* end;
      ...
      .ep_id = arg_u8n("e", "endpoint", "<u8:EID>", 0, 1,
                       "EID, id of the operating endpoint"),
      .cluster_id = arg_u16n("c", "cluster", "<u16:CID>", 0, 1,
                             "CID, id of the operating cluster"),
      .attr_id = arg_u16n("a", "attr", "<u16:AID>", 0, 1,
                          "AID, id of the operating attribute"),
      .device_id = arg_devidn(NULL, "device", "<u16:DID>", 0, 1,
                              "set the device id (DID) of the endpoint"),
      .device_ver = arg_u8n(NULL, "version", "<u8:VER>", 0, 1,
                            "set the device version of the endpoint"),
      .profile_id = arg_u16n(NULL, "profile", "<u16:PID>", 0, 1,
                             "set the profile id (PID) of the endpoint"),
      .manuf_code = arg_u16n(NULL, "manuf", "<u16:CODE>", 0, 1,
                             "set CODE of the manufacture"),
      .role_mask = arg_str0("r", "role", "<sc:C|S>",
                            "set the role the cluster, default: S"),
      .attr_access = arg_str0(NULL, "access", "<mc:R|W|P|S>",
                              "set access of the attribute, default: RW"),
      .attr_type =
          arg_u8n(NULL, "type", "<u8:TID>", 0, 1, "TID, ZCL attribute type id"),
      .attr_value = arg_hexn("v", "value", "<hex:value>", 0, 1,
                             "set value of the attribute, raw data in HEX"),
      .end = arg_end(2),
  };
    ...
    if (cluster_cfg) {
      switch (argtable.role_mask->sval[0][0]) {
        ...
        default:
          EXIT_ON_ERROR(
              ESP_ERR_INVALID_ARG,
              cli_output_line("zcl add: invalid argument to option --role"));
          break;
      }
    }
  }
  ...
  if (argtable.attr_access->count > 0) {
    if (attribute_cfg) {
      EXIT_ON_ERROR(
          parse_attr_access(argtable.attr_access->sval[0],
                            &attribute_cfg->attr_access),
...
static esp_err_t cli_dm_add(esp_zb_cli_cmd_t* self, int argc, char* argv[]) {
  ...
  EXIT_ON_FALSE(CLI_CTX().ep_list, ESP_ERR_NOT_SUPPORTED,
                cli_output_line("Data model has been registered"));
...
static inline void zcl_cluster_println(const esp_zb_zcl_cluster_t* cluster) {
  cli_output("cluster:0x%04x, %c, manuf:0x%04x\n", cluster->cluster_id,
             cluster->role_mask == 1 ? 'S' : 'C', cluster->manuf_code);
}

static inline void zcl_ep_println(const esp_zb_endpoint_t* ep) {
  cli_output(
      "ep:%d, prfl:0x%04x, dev_id:0x%04x, dev_ver:%d\n", ep->ep_id,
      ep->profile_id,
      ((esp_zb_endpoint_config_t*) (ep->reserved_ptr))->app_device_id,
      ((esp_zb_endpoint_config_t*) (ep->reserved_ptr))->app_device_version);
}

static void dm_show_attr_list(const esp_zb_attribute_list_t* attr_list,
                              const char* prefix) {
...
static void dm_show_cluster_list(const esp_zb_cluster_list_t* cluster_list,
                                 char* prefix) {
  assert(prefix != NULL);
 ...
 *
 */
static esp_err_t cli_dm_register(esp_zb_cli_cmd_t* self,
                                 int argc,
                                 char** argv) {
  (void) argv;
  esp_err_t ret = ESP_OK;
  ...
  EXIT_ON_FALSE(!esp_zb_is_started(), ESP_ERR_NOT_SUPPORTED,
                cli_output_line("Can not register after stack started"));
  EXIT_ON_ERROR(esp_zb_device_register(CLI_CTX().ep_list),
                cli_output_line("Fail to register the ZCL data model"));
...
static esp_err_t cli_dm_read(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .ep_id = arg_u8n("e", "endpoint", "<u8:EID>", 1, 1,
                       "EID, id of the operating endpoint"),
      .cluster_id = arg_u16n("c", "cluster", "<u16:CID>", 1, 1,
                             "CID, id of the operating cluster"),
      .attr_id = arg_u16n("a", "attr", "<u16:AID>", 1, 1,
                          "AID, id of the operating attribute"),
      .manuf_code =
          arg_u16n(NULL, "manuf", "<u16:CODE>", 0, 1, "manufacture code"),
      .role_mask =
          arg_str0("r", "role", "<sc:C|S>", "role of the cluster, default: S"),
      .end = arg_end(2),
  };
  esp_err_t ret = ESP_OK;
  ...
  if (argtable.role_mask->count > 0) {
    switch (argtable.role_mask->sval[0][0]) {
      ...
      default:
        EXIT_ON_ERROR(
            ESP_ERR_INVALID_ARG,
            cli_output("%s: invalid argument to option --role\n", argv[0]));
        break;
    }
  }

  EXIT_ON_FALSE(CLI_CTX().ep_list == NULL, ESP_ERR_INVALID_STATE,
                cli_output_line("Please register the data model first"));
  esp_zb_zcl_attr_t* attr = esp_zb_zcl_get_manufacturer_attribute(
      argtable.ep_id->val[0], argtable.cluster_id->val[0], role_mask,
      argtable.attr_id->val[0], manuf_code);
  EXIT_ON_FALSE(attr, ESP_ERR_NOT_FOUND,
                cli_output_line("Attribute not found"));
  cli_output_buffer(attr->data_p,
                    esp_zb_zcl_get_attribute_size(attr->type, attr->data_p));
...
static esp_err_t cli_dm_write(esp_zb_cli_cmd_t* self, int argc, char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .role_mask =
          arg_str0("r", "role", "<sc:C|S>", "role of the cluster, default: S"),
      .attr_value = arg_hexn("v", "value", "<hex:VALUE>", 1, 1,
                             "value of the attribute, raw data in HEX"),
      .end = arg_end(2),
  };
  ...
  EXIT_ON_FALSE(CLI_CTX().ep_list == NULL, ESP_ERR_INVALID_STATE,
                cli_output_line("Please register the data model first"));
  esp_zb_zcl_status_t result = esp_zb_zcl_set_manufacturer_attribute_val(
      argtable.ep_id->val[0], argtable.cluster_id->val[0], role_mask,
      manuf_code, argtable.attr_id->val[0], argtable.attr_value->hval[0],
      !(argtable.force->count > 0));

  EXIT_ON_FALSE(result == ESP_ZB_ZCL_STATUS_SUCCESS, ESP_FAIL,
                cli_output("Fail to write attribute: %d\n", result));
...
static void esp_zb_cli_fill_aps_argtable(esp_zb_cli_aps_argtable_t* aps) {
  aps->dst_addr =
      arg_addrn("d", "dst-addr", "<addr:ADDR>", 0, 1, "destination address");
  aps->dst_ep =
      arg_u8n(NULL, "dst-ep", "<u8:EID>", 0, 1, "destination endpoint id");
  aps->src_ep = arg_u8n("e", "src-ep", "<u8:EID>", 1, 1, "source endpoint id");
  aps->profile =
      arg_u16n(NULL, "profile", "<u16:PID>", 0, 1, "profile id of the command");
  aps->cluster =
      arg_u16n("c", "cluster", "<u16:CID>", 1, 1, "cluster id of the command");
}

static esp_err_t esp_zb_cli_parse_aps_dst(
    esp_zb_cli_aps_argtable_t* parsed_argtable,
  ...
  if (parsed_argtable->dst_addr->count > 0) {
    ...
    esp_zb_aps_address_mode_t address_mode_select[][2] = {
        [ESP_ZB_ZCL_ADDR_TYPE_SHORT][0] =
            ESP_ZB_APS_ADDR_MODE_16_GROUP_ENDP_NOT_PRESENT,
        [ESP_ZB_ZCL_ADDR_TYPE_IEEE][0] =
    ...
    *address_mode =
        address_mode_select[dst_addr->addr_type]
                           [parsed_argtable->dst_ep->count > 0 ? 1 : 0];
    EXIT_ON_FALSE(
        *address_mode != ESP_ZB_APS_ADDR_MODE_64_PRESENT_ENDP_NOT_PRESENT,
...
static esp_err_t cli_zcl_attr_cmd(esp_zb_cli_cmd_t* self,
                                  int argc,
                                  char** argv) {
  struct {
    ...
    arg_end_t* end;
      ...
      .peer_role = arg_strn("r", "role", "<sc:C|S>", 0, 1,
                            "role of the peer cluster, default: S"),
      .attr_id = arg_u16n("a", "attr", "<u16:AID>", 0, 1,
                          "id of the operating attribute"),
      .manuf_code = arg_u16n(NULL, "manuf", "<u16:CODE>", 0, 1,
                             "set CODE of the manufacture"),
      .attr_type =
          arg_u8n("t", "type", "<u8:TID>", 0, 1, "ZCL attribute type id"),
      .attr_value = arg_hexn("v", "value", "<hex:DATA>", 0, 1,
                             "value of the attribute, raw data in HEX"),
      .end = arg_end(2),
  };
  ...
  EXIT_ON_ERROR(esp_zb_cli_parse_aps_dst(
      &argtable.aps, &req_params.zcl_basic_cmd.dst_addr_u,
      &req_params.zcl_basic_cmd.dst_endpoint, &req_params.address_mode,
      &req_params.zcl_basic_cmd.src_endpoint, &req_params.cluster_id, NULL));
    ...
    switch (argtable.peer_role->sval[0][0]) {
      ...
      default:
        EXIT_ON_ERROR(ESP_ERR_INVALID_ARG,
                      cli_output_line("invalid argument to option -r"));
        break;
    }
  }

  if (argtable.manuf_code->count > 0) {
    if (argtable.manuf_code->val[0] !=
  ...
  if (!strcmp(cmd, "read")) {
    ...
    EXIT_ON_FALSE(
        n == argtable.attr_type->count && n == argtable.attr_value->count,
        ESP_ERR_INVALID_ARG,
    ...
    EXIT_ON_FALSE(argtable.attr_id->count > 0, ESP_ERR_INVALID_ARG,
                  cli_output("%s: -a <u16:AID> is required\n", cmd));
    ...
    EXIT_ON_FALSE(
        n == argtable.attr_type->count, ESP_ERR_INVALID_ARG,
...
static esp_err_t cli_zcl_send_raw(esp_zb_cli_cmd_t* self,
                                  int argc,
                                  char** argv) {
  struct {
    esp_zb_cli_aps_argtable_t aps;
    ...
    arg_end_t* end;
      ...
      .peer_role = arg_strn("r", "role", "<sc:C|S>", 0, 1,
                            "role of the peer cluster, default: S"),
      .command = arg_u8n(NULL, "cmd", "<u8:CMD_ID>", 1, 1,
                         "identifier of the command"),
      .payload = arg_hexn("p", "payload", "<hex:DATA>", 0, 1,
                          "ZCL payload of the command, raw HEX data"),
  ...
  EXIT_ON_ERROR(esp_zb_cli_parse_aps_dst(
      &argtable.aps, &req_params.zcl_basic_cmd.dst_addr_u,
      &req_params.zcl_basic_cmd.dst_endpoint, &req_params.address_mode,
      &req_params.zcl_basic_cmd.src_endpoint, &req_params.cluster_id,
  ...
  if (argtable.payload->count > 0) {
    ...
    req_params.data.type =
        ESP_ZB_ZCL_ATTR_TYPE_SET; /* A workarround to send arbitrary data */
  }
  ...
  if (argtable.dry_run->count == 0) {
    ...
    cli_output("Mode[%d]: e:%d -> e:%d, ", req_params.address_mode,
    ...
    if (req_params.address_mode == ESP_ZB_APS_ADDR_MODE_64_ENDP_PRESENT) {
      cli_output("addr:0x%016" PRIx64,
                 *(uint64_t*) (&req_params.zcl_basic_cmd.dst_addr_u.addr_long));
    } else {
      cli_output("addr:0x%04" PRIx16,
                 req_params.zcl_basic_cmd.dst_addr_u.addr_short);
    }
    cli_output_line("");
    cli_output(
        "prfl:0x%04x, c:0x%04x, dir:%c, cmd:0x%02x\n", req_params.profile_id,
...
DECLARE_ESP_ZB_CLI_CMD_WITH_SUB(
    ...
    ESP_ZB_CLI_SUBCMD(write,
                      cli_dm_write,
                      "Write value to attribute in data model"), );

DECLARE_ESP_ZB_CLI_SUBCMD_LIST(
    zcl_send_gen,
