    ...
    *olen      = 0;
    ...
    if (ctx->cipher_info->mode == MBEDTLS_MODE_CBC)
    {
        ...
        if (0 != ctx->unprocessed_len)
        {
            ...
            if (0 != (ret = ctx->cipher_info->base->cbc_func(ctx->cipher_ctx, ctx->operation, block_size, ctx->iv,
    ...
    {
        if (0 != (ret = ctx->cipher_info->base->cfb_func(ctx->cipher_ctx, ctx->operation, ilen, &ctx->unprocessed_len,
                                                         ctx->iv, input, output)))
        {
            return (ret);
        }
    ...
    {
        if (0 != (ret = ctx->cipher_info->base->ofb_func(ctx->cipher_ctx, ilen, &ctx->unprocessed_len, ctx->iv, input,
                                                         output)))
        {
            return (ret);
        }
...
static void add_pkcs_padding(unsigned char *output, size_t output_len, size_t data_len)
{
    ...
    for (i = 0; i < padding_len; i++) output[data_len + i] = (unsigned char)padding_len;
}
...
static int get_pkcs_padding(unsigned char *input, size_t input_len, size_t *data_len)
{
    ...
    *data_len   = input_len - padding_len;
    ...
    for (i = 0; i < input_len; i++) bad |= (input[i] ^ padding_len) * (i >= pad_idx);
...
int mbedtls_cipher_finish(mbedtls_cipher_context_t *ctx, unsigned char *output, size_t *olen)
{
    ...
    *olen = 0;
    ...
    if (MBEDTLS_MODE_CBC == ctx->cipher_info->mode)
    {
        ...
        /* cipher block */
        if (0 !=
            (ret = ctx->cipher_info->base->cbc_func(ctx->cipher_ctx, ctx->operation, mbedtls_cipher_get_block_size(ctx),
                                       ...
                                       size_t                    tag_len)
{
    ...
    if (ctx->psa_enabled == 1)
    {
        ...
        status = psa_aead_encrypt(cipher_psa->slot, cipher_psa->alg, iv, iv_len, ad, ad_len, input, ilen, output,
                                  ilen + tag_len, olen);
        if (status != PSA_SUCCESS)
            return (MBEDTLS_ERR_CIPHER_HW_ACCEL_FAILED);
                                       ...
                                       size_t                    tag_len)
{
    ...
    if (ctx->psa_enabled == 1)
    {
        ...
        status = psa_aead_decrypt(cipher_psa->slot, cipher_psa->alg, iv, iv_len, ad, ad_len, input, ilen + tag_len,
                                  output, ilen, olen);
                                ...
                                size_t                    tag_len)
{
    ...
    return (mbedtls_cipher_aead_encrypt(ctx, iv, iv_len, ad, ad_len, input, ilen, output, olen, tag, tag_len));
}
                                ...
                                size_t                    tag_len)
{
    ...
    return (mbedtls_cipher_aead_decrypt(ctx, iv, iv_len, ad, ad_len, input, ilen, output, olen, tag, tag_len));
}
