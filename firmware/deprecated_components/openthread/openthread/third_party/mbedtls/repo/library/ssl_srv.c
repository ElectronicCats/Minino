...
static int ssl_parse_client_hello_v2(mbedtls_ssl_context *ssl)
{
    ...
    for (i = 0, p = buf + 6; i < ciph_len; i += 3, p += 3)
    {
        if (p[0] == 0 && MBEDTLS_GET_UINT16_BE(p, 1) != MBEDTLS_SSL_FALLBACK_SCSV_VALUE)
        {
...
#endif
        {
            if (p[0] != 0 || MBEDTLS_GET_UINT16_BE(p, 1) != ciphersuites[i])
                continue;
...
static int ssl_write_server_hello(mbedtls_ssl_context *ssl)
{
    ...
    *p++ = MBEDTLS_BYTE_0(ssl->session_negotiate->compression);
    ...
    MBEDTLS_SSL_DEBUG_MSG(3,
                          ("server hello, compress alg.: 0x%02X", (unsigned int)ssl->session_negotiate->compression));
                                   ...
                                   size_t               pms_offset)
{
    ...
    for (i = 0; i < ssl->handshake->pmslen; i++) pms[i] = (mask & fake_pms[i]) | ((~mask) & peer_pms[i]);
