...
void mbedtls_ecp_group_free(mbedtls_ecp_group *grp)
{
    ...
    if (grp->T != NULL)
    {
        for (i = 0; i < grp->T_size; i++) mbedtls_ecp_point_free(&grp->T[i]);
        mbedtls_free(grp->T);
    }
...
static int ecp_modp(mbedtls_mpi *N, const mbedtls_ecp_group *grp)
{
    ...
    while (mbedtls_mpi_cmp_mpi(N, &grp->P) >= 0) /* we known P, N and the result are positive */
...
static int ecp_normalize_jac_many(const mbedtls_ecp_group *grp, mbedtls_ecp_point *T[], size_t T_size)
{
    ...
    for (i = 0; i < T_size; i++) mbedtls_mpi_init(&c[i]);
...
cleanup:
    ...
    mbedtls_mpi_free(&Zi);
    mbedtls_mpi_free(&ZZi);
    for (i = 0; i < T_size; i++) mbedtls_mpi_free(&c[i]);
    mbedtls_free(c);
...
static void ecp_comb_recode_core(unsigned char x[], size_t d, unsigned char w, const mbedtls_mpi *m)
{
    ...
    for (i = 0; i < d; i++)
        for (j = 0; j < w; j++) x[i] |= mbedtls_mpi_get_bit(m, i + d * j) << j;
...
add:
    ...
    {
        j = i;
        while (j--) MBEDTLS_MPI_CHK(ecp_add_mixed(grp, &T[i + j], &T[j], &T[i]));
    }
...
norm_add:
     ...
     */
    for (j = 0; j + 1 < T_size; j++) TT[j] = T + j + 1;
                        ...
                        mbedtls_ecp_restart_ctx *rs_ctx)
{
    ...
    if (p_eq_g && grp->T != NULL)
    {
        ...
        /* Allocate table if we didn't have any */
        {
            ...
            for (i = 0; i < T_size; i++) mbedtls_ecp_point_init(&T[i]);
...
cleanup:
    ...
    if (T != NULL)
    {
        for (i = 0; i < T_size; i++) mbedtls_ecp_point_free(&T[i]);
        mbedtls_free(T);
    }
