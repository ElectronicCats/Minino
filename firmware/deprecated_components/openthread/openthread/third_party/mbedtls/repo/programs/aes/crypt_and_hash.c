...
int main(int argc, char *argv[])
{
    ...
    if (mode == MODE_ENCRYPT)
    {
         ...
         */
        for (i = 0; i < 8; i++) buffer[i] = (unsigned char)(filesize >> (i << 3));
    ...
    if (mode == MODE_DECRYPT)
    {
        ...
        if (cipher_info->mode != MBEDTLS_MODE_GCM &&
            ((filesize - mbedtls_md_get_size(md_info)) % mbedtls_cipher_get_block_size(&cipher_ctx)) != 0)
        {
        ...
        if (mbedtls_cipher_setkey(&cipher_ctx, digest, cipher_info->key_bitlen, MBEDTLS_DECRYPT) != 0)
        {
            mbedtls_fprintf(stderr, "mbedtls_cipher_setkey() returned error\n");
            goto exit;
        }
        ...
        diff = 0;
        for (i = 0; i < mbedtls_md_get_size(md_info); i++) diff |= digest[i] ^ buffer[i];
