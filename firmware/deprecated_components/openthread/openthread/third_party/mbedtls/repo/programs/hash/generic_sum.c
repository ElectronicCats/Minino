...
static int generic_print(const mbedtls_md_info_t *md_info, char *filename)
{
    ...
    for (i = 0; i < mbedtls_md_get_size(md_info); i++) mbedtls_printf("%02x", sum[i]);
...
static int generic_check(const mbedtls_md_info_t *md_info, char *filename)
{
    ...
    while (fgets(line, (int)n - 1, f) != NULL)
    {
        ...
        for (i = 0; i < mbedtls_md_get_size(md_info); i++) sprintf(buf + i * 2, "%02x", sum[i]);

        /* Use constant-time buffer comparison */
        diff = 0;
        for (i = 0; i < 2 * mbedtls_md_get_size(md_info); i++) diff |= line[i] ^ buf[i];
...
int main(int argc, char *argv[])
{
    ...
    for (i = 2; i < argc; i++) ret |= generic_print(md_info, argv[i]);
