...
const esp_netif_inherent_config_t g_esp_netif_inherent_openthread_config =
...
static bool is_mesh_local_addr(const otIp6Address* address) {
  return memcmp(address->mFields.m8,
                otThreadGetMeshLocalPrefix(esp_openthread_get_instance())->m8,
                sizeof(otThreadGetMeshLocalPrefix(esp_openthread_get_instance())
                           ->m8)) == 0;
}

static bool is_addr_locator(const otIp6Address* address) {
  return address->mFields.m32[2] == 0xff000000 &&
         address->mFields.m16[6] == 0xfe;
}
...
static esp_err_t notify_packets_pending(void) {
  ...
  ssize_t ret =
      write(s_openthread_netif_glue.event_fd, &signal, sizeof(signal));
...
static void process_thread_address(const otIp6AddressInfo* address_info,
                                   bool is_added,
                                   void* context) {
  ...
  if (is_added) {
    if (is_multicast) {
      if (esp_event_post(OPENTHREAD_EVENT,
                         OPENTHREAD_EVENT_MULTICAST_GROUP_JOIN, &addr,
                         sizeof(addr), 0) != ESP_OK) {
        ESP_LOGE(OT_PLAT_LOG_TAG,
                 "Failed to post OpenThread join multicast group event");
      }
    } else {
      ip_event_add_ip6_t add_addr;
      add_addr.addr = addr;
      // if an address is not mesh local or link local, we set preferred for
      // this address.
      add_addr.preferred = is_mesh_local_addr(address_info->mAddress) ||
                                   is_link_local_addr(address_info->mAddress)
                               ? 0
                               : 1;
      if (esp_event_post(OPENTHREAD_EVENT, OPENTHREAD_EVENT_GOT_IP6, &add_addr,
                         sizeof(add_addr), 0) != ESP_OK) {
        ESP_LOGE(OT_PLAT_LOG_TAG,
                 "Failed to post OpenThread got ip6 address event");
      }
    }
  } else {
    if (is_multicast) {
      if (esp_event_post(OPENTHREAD_EVENT,
                         OPENTHREAD_EVENT_MULTICAST_GROUP_LEAVE, &addr,
                         sizeof(addr), 0) != ESP_OK) {
        ESP_LOGE(OT_PLAT_LOG_TAG,
                 "Failed to post OpenThread leave multicast group event");
      }
    } else {
      if (esp_event_post(OPENTHREAD_EVENT, OPENTHREAD_EVENT_LOST_IP6, &addr,
                         sizeof(addr), 0) != ESP_OK) {
        ESP_LOGE(OT_PLAT_LOG_TAG,
                 "Failed to post OpenThread lost ip6 address event");
      }
    }
  }
}
...
static void process_thread_receive(otMessage* message, void* context) {
  ...
  error = esp_netif_receive(s_openthread_netif_glue.base.netif, message, length,
                            NULL);

  if (error != ESP_OK) {
    ESP_LOGW(OT_PLAT_LOG_TAG, "process_thread_receive failed: %s",
  ...
  while (xQueueReceive(s_packet_queue, &msg, 0) == pdTRUE) {
    if (msg) {
      ...
      if (ot_error != OT_ERROR_NONE && ot_error != OT_ERROR_DROP) {
        ESP_LOGW(OT_PLAT_LOG_TAG,
                 "ThreadNetif Failed to send OpenThread IP6 message: %s",
...
static esp_err_t openthread_netif_transmit(void* handle,
                                           void* buffer,
                                           size_t len) {
  esp_err_t error = ESP_OK;
  ...
  if (ot_error != OT_ERROR_NONE) {
    ESP_LOGE(OT_PLAT_LOG_TAG, "Failed to copy to OpenThread message: %s",
             otThreadErrorToString(ot_error));
    ExitNow(error = ESP_ERR_NO_MEM);
  }
  ...
  if (xQueueSend(s_packet_queue, &message, 0) != pdTRUE) {
    ESP_LOGE(OT_PLAT_LOG_TAG,
             "Failed to send to Thread netif: packet queue full");
...
static esp_err_t register_openthread_event_handlers(esp_netif_t* esp_netif) {
  ...
  ESP_RETURN_ON_ERROR(esp_event_handler_register(
                          OPENTHREAD_EVENT, OPENTHREAD_EVENT_LOST_IP6,
                          esp_netif_action_remove_ip6_address, esp_netif),
                      OT_PLAT_LOG_TAG,
                      "OpenThread interface remove ip6 event register failed");
  ESP_RETURN_ON_ERROR(
      esp_event_handler_register(
          OPENTHREAD_EVENT, OPENTHREAD_EVENT_MULTICAST_GROUP_JOIN,
          esp_netif_action_join_ip6_multicast_group, esp_netif),
      OT_PLAT_LOG_TAG,
      "OpenThread interface join ip6 multicast group event register failed");
  ESP_RETURN_ON_ERROR(
      esp_event_handler_register(
          OPENTHREAD_EVENT, OPENTHREAD_EVENT_MULTICAST_GROUP_LEAVE,
          esp_netif_action_leave_ip6_multicast_group, esp_netif),
      OT_PLAT_LOG_TAG,
      "OpenThread interface leave ip6 multicast group event register failed");
  return ESP_OK;
}

static void unregister_openthread_event_handlers(void) {
  esp_event_handler_unregister(OPENTHREAD_EVENT, OPENTHREAD_EVENT_START,
                               esp_netif_action_start);
  esp_event_handler_unregister(OPENTHREAD_EVENT, OPENTHREAD_EVENT_STOP,
                               esp_netif_action_stop);
  esp_event_handler_unregister(OPENTHREAD_EVENT, OPENTHREAD_EVENT_IF_UP,
                               esp_netif_action_connected);
  esp_event_handler_unregister(OPENTHREAD_EVENT, OPENTHREAD_EVENT_IF_DOWN,
  ...
  esp_event_handler_unregister(OPENTHREAD_EVENT, OPENTHREAD_EVENT_LOST_IP6,
                               esp_netif_action_remove_ip6_address);
  esp_event_handler_unregister(OPENTHREAD_EVENT,
...
static esp_err_t openthread_netif_post_attach(esp_netif_t* esp_netif,
                                              void* args) {
  ...
  esp_netif_driver_ifconfig_t driver_ifconfig = {
      .handle = &s_openthread_netif_glue,
      .transmit = openthread_netif_transmit,
  ...
  if (error == ESP_OK) {
    error =
        esp_event_post(OPENTHREAD_EVENT, OPENTHREAD_EVENT_START, NULL, 0, 0);
  }

  return error;
}

void* esp_openthread_netif_glue_init(
    const esp_openthread_platform_config_t* config) {
  otInstance* instance = esp_openthread_get_instance();
  esp_err_t error = ESP_OK;

  if (instance == NULL || s_packet_queue ||
      s_openthread_netif_glue.event_fd >= 0) {
    return NULL;
  }

  s_packet_queue =
      xQueueCreate(config->port_config.netif_queue_size, sizeof(otMessage*));
  ...
  error = esp_openthread_platform_workflow_register(
      &esp_openthread_netif_glue_update, &esp_openthread_netif_glue_process,
      netif_glue_workflow);
exit:
  if (error != ESP_OK) {
    return NULL;
...
void esp_openthread_netif_glue_deinit(void) {
  ...
  if (esp_event_post(OPENTHREAD_EVENT, OPENTHREAD_EVENT_STOP, NULL, 0, 0) !=
      ESP_OK) {
...
void esp_openthread_netif_glue_update(
...
esp_err_t esp_openthread_netif_glue_process(
    otInstance* instance,
    const esp_openthread_mainloop_context_t* context) {
  if (s_openthread_netif_glue.event_fd >= 0 &&
      FD_ISSET(s_openthread_netif_glue.event_fd, &context->read_fds)) {
