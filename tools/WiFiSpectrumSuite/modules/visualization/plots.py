...
def generate_interference_plots(df: pd.DataFrame, csv_file: str) -> str:
    ...
    Args:
        df: DataFrame con columnas SSID, Channel, RSSI, Calidad.
        csv_file: Ruta del CSV original (se usa para derivar el nombre de salida).

    Returns:
        Ruta al archivo PNG guardado, o cadena vacía si ocurrió un error.
    """
    try:
        ...
        print(f"    Gráficas guardadas como '{output_path}'")
        return output_path

    except Exception as exc:
        print(f"    Error al generar visualizaciones: {exc}")
        return ""
...
def generate_wardriving_plots(df: pd.DataFrame, nombre_base: str) -> str:
    ...
    Args:
        df: DataFrame con columnas Channel, RSSI, Timestamp, AuthMode,
    ...
    try:
        ...
        # Scatter RSSI promedio por canal
        if "Channel" in df.columns and "RSSI" in df.columns:
